...
EXPECTED_STAGE_A_SHA256 = "f306e1bea864b6ff1f75fddc85f607e92c62487b452ddf041804f4e62cea670a"
...
def canonical_rows(path: Path) -> tuple[list[tuple[str, int]], str, str | None]:
    # Reveal-only packaging repair, matching the already-established PR #1378
    # convention: validate the full historical table, then retain the canonical
    # 2022-2026 subset. Scientific scoring below still uses 2022-2023 only.
    ...
    idcol = None
    for key in ("eventid", "event", "trajectoryid", "id", "uniquetrajectoryidentifier"):
        if key in bynorm:
            idcol = bynorm[key]
            break
    if idcol is None:
    ...
    for row in rows:
        ...
        if yearcol is not None and str(row[yearcol]).strip():
            ...
            match = re.search(r"20(19|20|21|22|23|24|25|26)", eid)
            req(match is not None, f"cannot derive canonical year from {eid}")
            year = int(match.group(0))
        parsed.append((eid, year))

    req(len({eid for eid, _ in parsed}) == len(parsed), "canonical historical IDs not unique")
    counts = Counter(year for _, year in parsed)
    req(dict(sorted(counts.items())) == EXPECTED_ALL, f"historical canonical counts changed: {dict(sorted(counts.items()))}")

    canonical = [(eid, year) for eid, year in parsed if year in EXPECTED_CANONICAL]
    ccounts = Counter(year for _, year in canonical)
    req(dict(sorted(ccounts.items())) == EXPECTED_CANONICAL, f"2022-2026 canonical counts changed: {dict(sorted(ccounts.items()))}")
    req(len(canonical) == 95 and len({eid for eid, _ in canonical}) == 95, "canonical 2022-2026 IDs not unique/count changed")
    return canonical, idcol, yearcol
...
def main() -> int:
    ...
    req(actual == EXPECTED_STAGE_A_SHA256, f"not the binding R1 Stage-A seal: {actual}")
    ...
    result = {
        ...
        "repair_scope": "canonical historical-row packaging validation only; exact scoring/classification unchanged",
        ...
        "canonical_historical_counts": {str(k): v for k, v in EXPECTED_ALL.items()},
        "canonical_2022_2026_counts": {str(k): v for k, v in EXPECTED_CANONICAL.items()},
        ...
        "second_scientific_reveal_authorized": False,
    }
    a.output.write_text(json.dumps(result, indent=2, sort_keys=True, allow_nan=False) + "\n")

    lines = [
        "# M2D fixed4-seeded drift halo v1 — final blind OrbitTrace exact-ID reveal (R2 technical repair)",
        "",
        f"Verdict: **`{verdict}`**",
        "",
        "R2 did not rerun the scientific scan or any halo construction. It consumed the immutable Stage-A artifact sealed in R1 and repaired only the already-known historical-row handling of the canonical package, following the established #1378 reveal repair convention.",
        "",
    ...
    lines.extend(
        [
            "",
            "All 8,469 parent/seed/model/covariance/event-score/halo mappings were sealed before canonical target access. R2 used exact trajectory-ID intersection only; no reranking, merging, target-aware trimming, covariance/confidence/scale change, or membership recomputation was permitted.",
        ]
    )
    (a.output.parent / "FINAL_BLIND_TARGET_REVEAL_R2.md").write_text("\n".join(lines) + "\n")
