...
struct LCT {
    ...
    void pull(int x){
        t[x].mn=t[x].val; t[x].mnid=x;
        for(int d=0;d<2;d++){ int c=t[x].ch[d]; if(c && t[c].mn < t[x].mn){ t[x].mn=t[c].mn; t[x].mnid=t[c].mnid; } }
    }
...
struct IEdge{int u,v,a,b;};
struct XEdge{int u,a,b;};
...
int main(int argc,char**argv){
    ...
    for(uint32_t ci=0;ci<C;ci++){
        ...
        for(size_t bi=0;bi<blevels.size();bi++){
            ...
            unordered_map<int, vector<int>> verts; verts.reserve(alevels.size()*2);
            for(uint32_t u=0;u<n;u++) if(d14[u]>=b && d13[u]>0) verts[d13[u]].push_back((int)u);
            unordered_map<int, vector<pair<int,int>>> fedges; fedges.reserve(alevels.size()*2);
            for(int id=(int)n+1;id<nextEdge;id++) if(edgeActive[id]){ int u=eu[id]-1,v=ev[id]-1; if(d14[u]>=b && d14[v]>=b) fedges[ewa[id]].push_back({u,v}); }
            unordered_map<int, vector<int>> bads; bads.reserve(alevels.size()*2);
            for(auto &xe:xs) if(xe.b>=b) bads[xe.a].push_back(xe.u);
            ...
            for(size_t ai=0;ai<alevels.size();ai++){
                ...
                int anext=(ai+1<alevels.size()?alevels[ai+1]:0); long double wa=(long double)(a-anext)/(long double)N13;
                inner += (long double)dsu.good * wa;
            }
            total += inner*wb;
        }
        long double score= n? total/(long double)n:0.0L;
        out<<ci<<'\t'<<n<<'\t'<<m<<'\t'<<xs.size()<<'\t'<<(double)score<<'\t'<<blevels.size()<<'\t'<<replacements<<"\n";
        cerr<<"ci="<<ci<<" n="<<n<<" m="<<m<<" score="<<(double)score<<" b="<<blevels.size()<<" repl="<<replacements<<"\n";
    }
}
