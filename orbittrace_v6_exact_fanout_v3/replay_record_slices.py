...
def main() -> int:
    ...
    shard_paths = sorted(args.exact_shards_dir.glob(f"v6_exact_v3_{args.year}_shard_*.pkl"))
    require(bool(shard_paths), "no record-slice shard files")
    shard_count: int | None = None
    seen_indices: set[int] = set()
    slices_by_center: dict[float, list[dict[str, Any]]] = defaultdict(list)
    declared_loads: list[int] | None = None

    for path in shard_paths:
        shard, _digest = load_pickle_with_sidecar(path)
        require(shard["format"] == "orbittrace-v6-exact-record-slice-shard-v3", f"slice shard format changed {path.name}")
        require(int(shard["year"]) == args.year, f"slice shard year mismatch {path.name}")
        require(shard["preexact_sha256"] == pre_sha, f"slice shard preexact mismatch {path.name}")
        require(shard["scan_rows_sha256"] == pre["scan_rows_sha256"], f"slice shard scan mismatch {path.name}")
        require(shard["firewall"]["target_interval_remains_excluded"] is True, f"slice shard firewall failed {path.name}")
        require(shard["firewall"]["labels_not_evaluated"] is True, f"slice shard label firewall failed {path.name}")
        current_count = int(shard["shard_count"])
        shard_count = current_count if shard_count is None else shard_count
        require(current_count == shard_count, "mixed slice shard counts")
        index = int(shard["shard_index"])
        require(index not in seen_indices, f"duplicate slice shard index {index}")
        seen_indices.add(index)
        loads = [int(x) for x in shard["all_shard_estimated_costs"]]
        declared_loads = loads if declared_loads is None else declared_loads
        require(loads == declared_loads, "slice scheduling loads differ across shards")
        require(int(shard["scheduled_estimated_cost"]) == loads[index], "slice shard declared cost mismatch")
        for item in shard["slices"]:
            slices_by_center[float(item["center"])].append(item)

    require(shard_count is not None and declared_loads is not None, "missing slice shard metadata")
    require(seen_indices == set(range(shard_count)), f"incomplete slice shards: {sorted(seen_indices)} / {shard_count}")
    expected_bins, expected_loads = build_record_slices(pre, shard_count)
    require(declared_loads == expected_loads, "record-slice schedule changed before replay")

    # Reassemble each center exactly in original record order and prove that every
    # contiguous piece is present once with the exact proposal-anchor sequence.
    exact_by_center: dict[float, list[dict[str, Any]]] = {}
    for center in [float(v) for v in pre["ordered_centers"]]:
        spec = pre["centers"][center]
        records = spec["records"]
        items = sorted(slices_by_center.get(center, []), key=lambda item: int(item["record_start"]))
        require(bool(items), f"no slices for center {center}")
        cursor = 0
        outputs: list[dict[str, Any]] = []
        for item in items:
            start = int(item["record_start"]); stop = int(item["record_stop"])
            require(start == cursor and stop > start, f"slice gap/overlap center {center}")
            expected_records = records[start:stop]
            require(canonical_sha(expected_records) == item["records_sha256"], f"slice input hash mismatch center {center} {start}:{stop}")
            current_outputs = item["outputs"]
            require([str(r["proposal_anchor_id"]) for r in current_outputs] == [str(r["proposal_anchor_id"]) for r in expected_records], f"slice output order mismatch center {center} {start}:{stop}")
            outputs.extend(current_outputs)
            cursor = stop
        require(cursor == len(records), f"slice coverage incomplete center {center}")
        require([str(r["proposal_anchor_id"]) for r in outputs] == [str(r["proposal_anchor_id"]) for r in records], f"merged exact order mismatch center {center}")
    ...
    v6 = load_module(args.repaired_source, f"orbittrace_v6_record_slice_replay_{args.year}")
    ...
    replayed: list[float] = []
    ordered_centers = [float(v) for v in pre["ordered_centers"]]
    ...
    def replay_exact(old_arg, records, window_events, event_lookup, support_arg, base_arg):
        ...
        require(center in pre["centers"] and center not in replayed, f"unexpected/duplicate replay center {center}")
        spec = pre["centers"][center]
    ...
    checkpoint = {
        ...
        "execution": {
            "exact_fanout_v3_record_slices": True,
            "exact_shard_count": shard_count,
            "preexact_sha256": pre_sha,
            "estimated_shard_costs": declared_loads,
        },
        "audit": audit,
        "anchors": anchors,
    ...
    print(f"PASS_V6_FANOUT_V3_YEAR_REPLAY year={args.year} anchors={len(anchors)} components={len(components)} sha={digest}", flush=True)
    return 0
