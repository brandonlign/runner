...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--max-records-per-unit", type=int, default=2048)
...
def main() -> int:
    ...
    v6 = load_module(args.repaired_source, f"orbittrace_v6_exact_unit_{args.year}_{args.shard_index}")
    ...
    shard_units, shard_loads = balanced_unit_shards(
        pre,
        args.shard_count,
        max_records_per_unit=args.max_records_per_unit,
    )
    selected = shard_units[args.shard_index]
    require(bool(selected), "empty exact unit shard")
    print(
        f"V6_SUBCENTER_BALANCE year={args.year} loads={shard_loads} "
        f"selected_load={shard_loads[args.shard_index]} units={len(selected)}",
        flush=True,
    )

    exact_units: list[dict[str, Any]] = []
    for unit in selected:
        spec = pre["centers"][unit.center]
        full_records = spec["records"]
        require(canonical_sha(full_records) == spec["records_sha256"], f"captured record hash mismatch center {unit.center}")
        records = full_records[unit.start:unit.stop]
        require(len(records) == unit.record_count, f"unit record count changed {unit.key}")
        ids = [str(value) for value in spec["window_event_ids"]]
        require(canonical_sha(ids) == spec["window_event_ids_sha256"], f"captured window hash mismatch center {unit.center}")
        require(all(event_id in event_lookup for event_id in ids), f"captured window event absent center {unit.center}")
        window_events = [event_lookup[event_id] for event_id in ids]
        canonical_window = old.window_events_for_center(scan, unit.center, base)
        require([str(row["id"]) for row in canonical_window] == ids, f"window reconstruction changed center {unit.center}")
        ...
        require(
            ...
            f"exact output order changed unit {unit.key}",
        )
        exact_units.append(
            {
                "center": unit.center,
        ...
        print(
            f"V6_SUBCENTER_EXACT_DONE year={args.year} shard={args.shard_index}/{args.shard_count} "
            f"unit={unit.key} records={len(records):,} window={len(ids):,}",
            flush=True,
        )

    payload = {
        "format": "orbittrace-v6-exact-unit-shard-v3",
        "year": args.year,
        ...
        "unit_keys": [unit.key for unit in selected],
    ...
    path = args.output / f"v6_exact_units_{args.year}_shard_{args.shard_index:02d}.pkl"
    path.write_bytes(raw)
    digest = sha256_bytes(raw)
    path.with_suffix(".sha256").write_text(digest + "\n")
    print(
        f"PASS_V6_SUBCENTER_EXACT_SHARD year={args.year} shard={args.shard_index}/{args.shard_count} "
        f"units={len(selected)} sha={digest}",
        flush=True,
    )
    return 0
