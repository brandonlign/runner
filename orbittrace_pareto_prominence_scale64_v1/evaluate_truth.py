...
def main()->int:
    ...
    req(pre['schema']=='ORBITTRACE_PARETO_PROMINENCE_SCALE64_V1_PRELABEL' and pre['scientific_role']=='PRELABEL_PARETO_PROMINENCE_SCALE64_V1','prelabel identity');req(audit['verdict']=='PASS_PARETO_PROMINENCE_SCALE64_V1_PRETRUTH' and audit['prelabel_sha256']==ph and all(audit['gates'].values()),'pretruth authorization')
    ...
    parent=load(a.parent_runner,'scale64_truth_parent');q=load(a.quality_source,'scale64_truth_gmn');q.v1.mult.YEARS=YEARS;q.v1.mult.MONTH_KEYS=MONTH_KEYS;q.v1.mult.TOP_K=100
    runtime=q.v1.mult.load_frozen_runtime();support=runtime.load_support_module(a.support_source_parts);support.YEARS=YEARS;support.MONTH_KEYS=MONTH_KEYS;support.CORPUS='orbittrace-pareto-prominence-scale64-v1-truth';support.RANKING_VARIANTS=('persistence',);req((float(support.BLIND_LOW),float(support.BLIND_HIGH))==BLIND,'firewall changed');setattr(a,'fixed4_baseline_json',a.v8_result_json)
    ...
    panels=[]
    for b in BUCKETS:
        r=subset[b];K=int(r['equal_budget_k']);par=list(r['recurrent_candidates']);succ=list(r['successor_candidates'])[:K];req(len(par)==len(succ)==K,'equal budget');req([int(x['rank']) for x in par]==list(range(1,K+1)),'parent rank');req([int(x['pareto_prominence_rank']) for x in r['successor_candidates']]==list(range(1,len(r['successor_candidates'])+1)),'Pareto rank')
        for y in YEARS:
