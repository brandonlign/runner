...
def main() -> int:
    ...
    memo = load(paths["memo"][0], "rft_uvp_memo")
    dead = load(paths["dead"][0], "rft_uvp_dead")
    uvmod = load(paths["uv"][0], "rft_uvp_uv")
    direct = load(paths["direct"][0], "rft_uvp_direct")
    puv = load(paths["puv"][0], "rft_uvp_parallel_uv")
    builder = load(paths["builder"][0], "rft_uvp_builder")
    ...
    def shard_cache(mod: Any, events: list[dict[str, Any]]):
        ...
        replica, owned, unowned, stats = builder.build_one_uv_parallel_replica(
        ...
        with gzip.open(a.shard_output, "wt", encoding="utf-8", compresslevel=6) as fh:
            ...
            fh.write("\n")
        print(json.dumps({"RFT_UV_PARALLEL_REPLICA_SHARD_COMPLETE": replica, "stats": stats}, sort_keys=True), flush=True)
        raise ShardComplete
    ...
    try:
        ...
        return 0
    raise RuntimeError("cached runner unexpectedly passed UV-parallel shard interception")
