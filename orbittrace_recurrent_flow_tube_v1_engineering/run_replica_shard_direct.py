...
def main() -> int:
    ...
    direct_path = here / "direct_replica_builder.py"
    pins = {
        memo_path: MEMO_BLOB,
        cached_path: CACHED_BLOB,
        dead_path: DEAD_MEDOID_BLOB,
        pair_path: EXACT_PAIR_BLOB,
        direct_path: DIRECT_BLOB,
    }
    for path, expected in pins.items():
        if blob(path) != expected:
            raise RuntimeError(f"engineering source pin changed: {path.name}")

    memo = load(memo_path, "rft_direct_shard_memo")
    dead = load(dead_path, "rft_direct_dead_medoid")
    pair = load(pair_path, "rft_direct_exact_pair")
    direct = load(direct_path, "rft_direct_replica_builder")
    ...
    def shard_cache(mod: Any, events: list[dict[str, Any]]):
        ...
        replica, owned, unowned, stats = direct.build_one_replica(mod, events, a.replica, dead, pair)
        ...
        with gzip.open(a.shard_output, "wt", encoding="utf-8", compresslevel=6) as fh:
            ...
            fh.write("\n")
        print(json.dumps({"RFT_DIRECT_REPLICA_SHARD_COMPLETE": replica, "stats": stats}, sort_keys=True), flush=True)
        raise ShardComplete
    ...
    try:
        ...
        return 0
    raise RuntimeError("cached runner unexpectedly passed direct shard interception")
