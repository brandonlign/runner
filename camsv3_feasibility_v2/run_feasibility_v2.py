...
def audit_year(year: int) -> dict:
    ...
    with zipfile.ZipFile(io.BytesIO(raw)) as archive:
        ...
        gates["exact_one_basename_match"] = len(matches) == 1
        if len(matches) != 1:
...
def main() -> None:
    ...
    verdict = "PASS_CAMSV3_STRUCTURAL_FEASIBILITY_V2" if all(gates.values()) else "KILL_CAMSV3_STRUCTURAL_FEASIBILITY_V2"
    result = {
        "method": "CAMSv3 structural feasibility parser v2 only",
        "sole_parser_change": "exact safe basename match instead of exact full ZIP member path",
        "years": years,
    ...
    report.extend(f"- {item['year']}: `{item['actual_member_path']}`" for item in years)
    report.extend(
        [
            "",
            "## Canonical header",
    ...
    print(json.dumps({"verdict": verdict, "gates": gates, "member_paths": [x["actual_member_path"] for x in years]}, indent=2))
