...
def orbit_vector(q: float,e: float,inc_deg: float,peri_deg: float,node_deg: float)->np.ndarray:
    ...
    # Unit perihelion/eccentricity direction in ecliptic Cartesian coordinates.
    p=np.asarray([
        cO*co-sO*so*ci,
        sO*co+cO*so*ci,
        so*si,
...
def candidates_from_sync(labels: np.ndarray,selected_nodes: tuple[int,...],events: list[dict[str,Any]],ordinary: dict[float,float],synchronous: dict[float,float])->list[dict[str,Any]]:
    ...
    for lab,node in enumerate(selected_nodes):
        ...
        req(len(members)>=MIN_CLUSTER_SIZE,f'selected orbit cluster below frozen minimum: node={node}')
        out.append({
            'family_id':parent.member_hash('ORBIT7-DSEOM1',members),
...
def main()->int:
    ...
    qmod=parent.load_module(a.quality_source,'orbit7_density_sync_frozen_gmn_utility')
    ...
    support.CORPUS='orbittrace-orbit7-density-sync-v1-development-2022-2023-target-excluded'
    ...
    def wrapped(text: str):
        ...
        # Protected rows cannot reach orbital-field indexing.
    ...
    orbit_events=[]; vectors=[]; missing_raw=0; incomplete=0; invalid=0; fallback_q=0
    eligible_by_year={y:0 for y in YEARS}
    for e0 in accessible_events:
        ...
        if not(q>0.0 and ecc>=0.0 and 0.0<=inc<=180.0):
            invalid+=1; continue
        v=orbit_vector(q,ecc,inc,peri,node)
        orbit_events.append({'id':eid,'year':int(e0['year'])})
        vectors.append(v); eligible_by_year[int(e0['year'])]+=1
    req(len(vectors)>=100,'fewer than 100 orbit-eligible events')
    X=np.vstack(vectors).astype(np.float64,copy=False)
    years=np.asarray([e['year'] for e in orbit_events],dtype=np.int64)
    req(X.shape==(len(orbit_events),7),'ORBIT7 matrix shape mismatch')
    req(np.all(np.isfinite(X)),'nonfinite ORBIT7 matrix')
    ...
    req(tree_sha(tree)==frozen_tree_sha,'density-sync kernel mutated orbit condensed tree')
    labels=eom_labels(tree,synchronous)
    nodes=selected_eom_nodes(tree,synchronous)
    req(len(nodes)==len(set(int(x) for x in labels if int(x)>=0)),'orbit selected-node/label count mismatch')
    candidates=candidates_from_sync(labels,nodes,orbit_events,ordinary,synchronous)
    ...
    prelabel={
        'scientific_role':'PRELABEL_FROZEN_ORBIT7_DENSITY_SYNC_V1',
        'representation':'ORBIT7_Q_PLANE_NORMAL_ECCENTRICITY_VECTOR',
        ...
        'eligibility':'finite q/e/i/peri/node; q>0; e>=0; 0<=i<=180; angles modulo 360',
        ...
        'accessible_events':len(accessible_events),'orbit_eligible_events':len(orbit_events),
        'orbit_eligible_by_year':{str(y):eligible_by_year[y] for y in YEARS},
    ...
    prelabel_path=a.output/'ORBIT7_DENSITY_SYNC_V1_PRELABEL.json'
    prelabel_path.write_text(json.dumps(prelabel,indent=2,sort_keys=True,allow_nan=False)+'\n')
    prelabel_sha=sha(prelabel_path)

    # Truth opens only after the orbit representation, hierarchy, memberships, order and hashes are durable.
    ...
    verdict='PASS_ORBIT7_DENSITY_SYNC_V1_GMN_DEVELOPMENT' if passed else 'FAIL_ORBIT7_DENSITY_SYNC_V1_GMN_DEVELOPMENT'
    result={
        'verdict':verdict,'scientific_role':'TARGET_EXCLUDED_GMN_2022_2023_DEVELOPMENT_ONLY',
        'prelabel_sha256':prelabel_sha,'representation':prelabel['representation'],'normalization_or_fitted_scale':None,
        'accessible_events':len(accessible_events),'orbit_eligible_events':len(orbit_events),'orbit_eligible_by_year':prelabel['orbit_eligible_by_year'],
    ...
    (a.output/'ORBIT7_DENSITY_SYNC_V1_GMN_DEVELOPMENT.json').write_text(json.dumps(result,indent=2,sort_keys=True,allow_nan=False)+'\n')
    print(json.dumps({
        'verdict':verdict,'accessible_events':len(accessible_events),'orbit_eligible_events':len(orbit_events),'orbit_eligible_by_year':prelabel['orbit_eligible_by_year'],
