...
V52_HDB_ORDER_SHA256 = '75fb44015e348c6b1bf0367e74db8e273e29862e132b5ef3305b2ddb409d8cc7'
...
def validate_v51(vector_file: Path, result_file: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    require(vector['diagnostic_recoverability_attached'] is False and vector['annual_own_family_f1_attached'] is False, 'v51 vector contains diagnostic outcomes')
    require(vector['literature_budget_used_in_statistic'] is False and vector['component_quality_topology_signal_used'] is False, 'v51 statistic contaminated by unrelated rescue signals')
    ...
    require(result['verdict'] == 'PASS_V51_V31_CONSENSUS_BOTTLENECK_DIAGNOSTIC', 'v51 authorizing diagnostic did not pass')
    require(result['direction_supported_both_years'] is True, 'v51 direction not supported both years')
...
def freeze_mode(vector_file: Path, result_file: Path, output: Path) -> int:
    ...
    vector, _ = validate_v51(vector_file, result_file)
    ...
    require(local_ranks == list(range(1, HDB_N + 1)), 'local ranks are not a complete permutation')
    require(v19_ranks == list(range(1, HDB_N + 1)), 'v19 ranks are not a complete permutation')
    ...
    for r in rows:
        ...
        b = float(r['consensus_bottleneck'])
        ...
        require(abs(b - max(lp, vp)) < 1e-15, 'v51 bottleneck arithmetic changed')
        by_id[fid] = {'family_id': fid, 'local_rank': rl, 'v19_rank': rv}

    v31_order = sorted(ids, key=lambda fid: (by_id[fid]['local_rank'] + by_id[fid]['v19_rank'], by_id[fid]['local_rank'], by_id[fid]['v19_rank'], fid))
    require(order_sha(v31_order) == V31_HDB_FUSED_SHA256, 'reconstructed exact-v31 HDB fusion changed')
    v52_order = sorted(ids, key=lambda fid: (max(by_id[fid]['local_rank'], by_id[fid]['v19_rank']), min(by_id[fid]['local_rank'], by_id[fid]['v19_rank']), fid))
    require(order_sha(v52_order) == V52_HDB_ORDER_SHA256, 'v52 HDB order identity changed')

    old = {fid: i + 1 for i, fid in enumerate(v31_order)}
    new = {fid: i + 1 for i, fid in enumerate(v52_order)}
    ...
    require((moved, moved_up, moved_down) == (217, 117, 100), 'v52 structural movement changed')
    top9_delta = len(set(v31_order[:9]) ^ set(v52_order[:9])) // 2
    top11_delta = len(set(v31_order[:11]) ^ set(v52_order[:11])) // 2
    require(top9_delta == 1 and top11_delta == 1, 'v52 frozen prefix membership consequence changed')
    ...
    for fid in ids:
        ...
        out_rows.append({
            ...
            'v52_rank': new[fid],
            'rank_displacement_v31_minus_v52': old[fid] - new[fid],
        })
    out_rows.sort(key=lambda r: int(r['v52_rank']))

    payload: dict[str, Any] = {
        'verdict': 'PASS_V52_LEXICOGRAPHIC_MINIMAX_HDB_ORDER_FREEZE',
        'scientific_role': 'COMPLETE_HDB_TOTAL_ORDER_FROZEN_FROM_BINDING_V51_RANK_VECTOR_BEFORE_V52_PANEL_OUTCOME',
        ...
        'family_count': HDB_N,
        'hdb_rule': '(max(local_rank,v19_rank), min(local_rank,v19_rank), family_id), ascending',
        'sugar_rule': 'exact v31 unchanged',
        'v31_hdb_order_sha256': V31_HDB_FUSED_SHA256,
        'v52_hdb_order_sha256': V52_HDB_ORDER_SHA256,
        'v31_hdb_order': v31_order,
        'v52_hdb_order': v52_order,
        'families': out_rows,
        ...
        'current_v52_panel_outcome_accessed': False,
    ...
    out = output / 'V52_LEXICOGRAPHIC_MINIMAX_HDB_ORDER_FREEZE.json'
...
def exact_parent_capture(sugar_root: Path, hdbscan_root: Path, truth_root: Path, ranker_source: Path, output: Path) -> tuple[dict[str, Any], dict[int, dict[str, list[str]]]]:
    ...
    require(len(captured) == 2, 'expected two exact-v31 fusion calls')
    by_n = {len(x['fused_order']): x for x in captured}
    require(set(by_n) == {267, HDB_N}, 'unexpected parent fusion route sizes')
    ...
    h = by_n[HDB_N]
    require(order_sha(h['local_order']) == V31_HDB_LOCAL_SHA256, 'exact-v31 HDB local order changed')
    require(order_sha(h['v19_order']) == V19_HDB_ORDER_SHA256, 'exact-v31 HDB v19 order changed')
    require(order_sha(h['fused_order']) == V31_HDB_FUSED_SHA256, 'exact-v31 HDB fused order changed')
    return parent, by_n
...
def evaluate_mode(sugar_root: Path, hdbscan_root: Path, truth_root: Path, ranker_source: Path, order_file: Path, output: Path) -> int:
    ...
    require(freeze['verdict'] == 'PASS_V52_LEXICOGRAPHIC_MINIMAX_HDB_ORDER_FREEZE', 'v52 order freeze verdict changed')
    require(freeze['scientific_role'] == 'COMPLETE_HDB_TOTAL_ORDER_FROZEN_FROM_BINDING_V51_RANK_VECTOR_BEFORE_V52_PANEL_OUTCOME', 'v52 order freeze role changed')
    require(freeze['v31_hdb_order_sha256'] == V31_HDB_FUSED_SHA256 and freeze['v52_hdb_order_sha256'] == V52_HDB_ORDER_SHA256, 'v52 frozen order identity changed')
    expected_canonical = str(freeze['canonical_sha256_without_self_field'])
    check = dict(freeze)
    del check['canonical_sha256_without_self_field']
    require(canonical_sha(check) == expected_canonical, 'v52 order freeze canonical identity changed')
    v52_order = list(map(str, freeze['v52_hdb_order']))
    require(len(v52_order) == HDB_N and len(set(v52_order)) == HDB_N and order_sha(v52_order) == V52_HDB_ORDER_SHA256, 'invalid v52 HDB permutation')
    require(freeze['current_v52_panel_outcome_accessed'] is False and freeze['annual_own_family_f1_used_for_order'] is False and freeze['recoverability_label_used_for_order'] is False, 'v52 order used current outcome information')
    require(freeze['literature_budget_used_for_order'] is False and freeze['boundary_identity_used'] is False and freeze['component_quality_topology_signal_used'] is False, 'v52 order used forbidden rescue signal')
    ...
    require(list(map(str, freeze['v31_hdb_order'])) == hdb_fused, 'frozen reconstructed v31 order differs from runtime exact v31')
    ...
    require(set(v52_order) == set(hids), 'v52 HDB order universe differs from #950')
    ranked_hdb = v31.v22.rerank(hfams, v52_order)
    ...
    result: dict[str, Any] = {
        'scientific_stage': 'EXPOSED_SONOTACO_V52_LEXICOGRAPHIC_MINIMAX_FUSION_V1',
        'verdict': 'PASS_V52_LEXICOGRAPHIC_MINIMAX_FUSION_ALL_PANEL_LITERATURE_SUPERIORITY_DEVELOPMENT' if passed else 'FAIL_V52_LEXICOGRAPHIC_MINIMAX_FUSION_ALL_PANEL_LITERATURE_SUPERIORITY_DEVELOPMENT',
        'sole_scientific_change': 'HDB replaces exact-v31 equal rank-sum with frozen symmetric lexicographic minimax over the same exact local and v19 ranks; Sugar exact v31 unchanged',
        'authorizing_diagnostic': 'PASS_V51_V31_CONSENSUS_BOTTLENECK_DIAGNOSTIC',
        'authorizing_v51_run': 31493423814,
        'authorizing_v51_artifact': 9101972590,
        ...
        'hdb_v52_order_sha256': order_sha(v52_order),
        'hdb_rule': '(max(local_rank,v19_rank), min(local_rank,v19_rank), family_id), ascending',
    ...
    out = output / 'V52_LEXICOGRAPHIC_MINIMAX_FUSION_RESULT.json'
    out.write_text(json.dumps(result, indent=2, sort_keys=True, allow_nan=False) + '\n')
    print(json.dumps({'verdict': result['verdict'], 'panel_wins': wins, 'panels': panels, 'hdb_v52_order_sha256': result['hdb_v52_order_sha256']}, indent=2, sort_keys=True, allow_nan=False))
    return 0
