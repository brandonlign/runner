...
QUERY = 'SELECT Code, "Obs.date", Lsun, "Lgeo-Lsun", Bgeo, Vgeo FROM "J/A+A/667/A157/catalog" WHERE Lsun < 20.0 OR Lsun > 55.0'
...
def query_csv() -> bytes:
    require(QUERY == 'SELECT Code, "Obs.date", Lsun, "Lgeo-Lsun", Bgeo, Vgeo FROM "J/A+A/667/A157/catalog" WHERE Lsun < 20.0 OR Lsun > 55.0', "Stage-2 ADQL changed")
    ...
    req = urllib.request.Request(
        ...
        data=body,
        method="POST",
        headers={"Content-Type": "application/x-www-form-urlencoded", "User-Agent": "OrbitTrace-EFN-stage2-geometry/1"},
    )
...
def main() -> int:
    ...
    expected_union = set(ids_by_year[2017]) | set(ids_by_year[2018])
    ...
    for row in reader:
        ...
        require(code in set(ids_by_year[year]), f"Stage-2 Code/year mismatch: {code} -> {year}")
    ...
    result = {
        ...
        "server_side_filter": "Lsun < 20.0 OR Lsun > 55.0",
        "solar_longitude_normalization": SOLAR_LONGITUDE_NORMALIZATION,
        "exact_360_wrapped_to_zero_by_year": {str(y): wrapped_by_year[y] for y in YEARS},
    ...
    print(json.dumps({"verdict": result["verdict"], "rows_by_year": result["rows_by_year"], "canonical_geometry_sha256": geometry_sha, "exact_360_wrapped_to_zero_by_year": result["exact_360_wrapped_to_zero_by_year"]}, sort_keys=True))
    return 0
