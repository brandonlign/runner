...
def nearest_group_mean_distance(z: np.ndarray, Ztr: np.ndarray, gtr: np.ndarray, class_mask: np.ndarray) -> tuple[float, int, int]:
    require(z.shape == (FEATURE_DIM,), 'invalid held-out vector')
    require(Ztr.ndim == 2 and Ztr.shape[1] == FEATURE_DIM, 'invalid training matrix')
    require(gtr.shape == (len(Ztr),) and class_mask.shape == (len(Ztr),), 'invalid group/class alignment')
    idx = np.where(class_mask)[0]
    require(len(idx) > 0, 'empty class reference population')
    class_groups = sorted(set(map(str, gtr[idx].tolist())))
    require(len(class_groups) > 0, 'empty class-specific strict-group set')
    best = float('inf')
    members_total = 0
    for group in class_groups:
        mask = class_mask & (gtr == group)
        members = Ztr[mask]
        require(len(members) > 0, 'empty class-specific strict group')
        members_total += int(len(members))
        d = float(np.mean(np.linalg.norm(members - z[None, :], axis=1)))
        require(np.isfinite(d) and d >= 0.0, 'invalid strict-group mean distance')
        if d < best:
            best = d
    require(np.isfinite(best), 'no finite strict-group mean distance')
    return best, len(class_groups), members_total
...
def reproduce_parent(sugar_root: Path, hdbscan_root: Path, truth_root: Path, ranker_source: Path, output: Path) -> dict[str, Any]:
    ...
    require(parent['distance'] == 'ordinary Euclidean across all 71 fold-training standardized dimensions', 'v31 parent distance changed')
...
def main() -> int:
    ...
    ranker = v22.load_module(a.ranker_source, 'frozen_839_v58_nearest_group_mean_distance')
    ...
    require(Xall.shape == (cursor, FEATURE_DIM) and len(y13all) == len(y14all) == len(groups) == cursor, 'v58 stacked input mismatch')
    ...
    for fold in range(5):
        ...
        for year, yall, out in ((2013, y13all, margin13), (2014, y14all, margin14)):
            ...
            pos_groups = sorted(set(map(str, gtr[pos].tolist())))
            neg_groups = sorted(set(map(str, gtr[neg].tolist())))
            require(len(pos_groups) > 0 and len(neg_groups) > 0, f'{year} fold {fold} lacks class-specific strict groups')
            for j, global_i in enumerate(te_idx.tolist()):
                dpos, _, _ = nearest_group_mean_distance(Zte[j], Ztr, gtr, pos)
                dneg, _, _ = nearest_group_mean_distance(Zte[j], Ztr, gtr, neg)
                out[global_i] = dneg - dpos
            annual_diag[str(year)] = {
                'positive_references': int(pos.sum()),
                'nonpositive_references': int(neg.sum()),
                'positive_strict_groups': len(pos_groups),
                'nonpositive_strict_groups': len(neg_groups),
            }
        fold_diag.append({
            'fold': fold,
    ...
    require(np.all(np.isfinite(combined)), 'nonfinite v58 combined margin')
    ...
    result = {
        'scientific_stage': 'EXPOSED_SONOTACO_V58_NEAREST_GROUP_MEAN_DISTANCE_LOCAL_GEOMETRY_V1',
        'verdict': 'PASS_V58_NEAREST_GROUP_MEAN_DISTANCE_ALL_PANEL_LITERATURE_SUPERIORITY_DEVELOPMENT' if passed else 'FAIL_V58_NEAREST_GROUP_MEAN_DISTANCE_ALL_PANEL_LITERATURE_SUPERIORITY_DEVELOPMENT',
        'sole_scientific_change': 'replace exact-v31 nearest individual family class distance with nearest strict group by arithmetic mean Euclidean distance to that class-eligible group members',
        ...
        'distance': 'ordinary Euclidean across all 71 fold-training standardized dimensions before strict-group arithmetic mean aggregation',
        'class_distance': 'minimum across strict groups of arithmetic mean member Euclidean distance within the class-specific group subset',
        'class_labels': 'exact v31 annual family-level annual_f1 > 0.5; no sibling propagation',
        ...
        'group_distance_aggregation': 'arithmetic_mean',
        'nearest_reference_unit': 'strict_group',
        'group_label_propagation_used': False,
        'group_prototype_used': False,
        'group_size_weight_used': False,
        'trimmed_or_quantile_group_distance_used': False,
        'top_m_members_used': False,
        'classifier_or_regressor_used': False,
        'k_search': False,
        'metric_search': False,
    ...
    out = a.output / 'V58_NEAREST_GROUP_MEAN_DISTANCE_RESULT.json'
