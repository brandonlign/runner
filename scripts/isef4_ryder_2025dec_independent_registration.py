#!/usr/bin/env python3
"""Independently align the third genuine Ryder Dec2025 original NAC to Dec2024.
...
def fit(src,dst,side,rot,flip,expected):
    first=np.linalg.norm(src-side//2,axis=1)
    ...
    q=scales/expected
    predicted=(np.array([side//2,side//2])@native[:,:2].T+native[:,2])
...
def main():
    ...
    if m["schema"]!="Ryder-2022-2024-2026-four-original-calibrated-source-r640-v1" or \
       d["schema"]!="Ryder-Dec2025-camera-ground-exact-original-NAC-CDR-patch-v1":
    ...
    sz=2*d["source_radius_px"]+1
    newer,nm=extract(a.dec2025_folder/"followup_2025_Dec_original_NAC_CDR_native.npy",
        ...
        for rot in (0,2):
            for flip in (False,True):
                ...
                for ratio in RATIOS:
                    ...
                    fit_result=fit(s,t,sz,rot,flip,expected)
    ...
    doc={"schema":"Ryder-independent-Dec2025-original-NAC-to-2024-terrain-registration-v1",
       ...
       "source_camera_marker_radius_2025":sz//2,
