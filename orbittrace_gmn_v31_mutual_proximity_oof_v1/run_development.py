#!/usr/bin/env python3
"""Target-excluded GMN successor testing empirical Mutual Proximity inside the passed v31 local geometry."""
...
PARENT_RECOVERED_100 = 66
PARENT_RECOVERED_50 = 41
PARENT_PRECISION = 0.7229521515453452
PARENT_MRR = 0.050244164168646674
PARENT_QUALIFIED = 95
...
def pairwise_euclidean(rows: np.ndarray) -> np.ndarray:
    ...
    out = np.linalg.norm(diffs, axis=2)
    req(out.shape == (rows.shape[0], rows.shape[0]) and np.isfinite(out).all(), "invalid reference distance matrix")
    return out


def empirical_mp_dissimilarity(query: np.ndarray, refs: np.ndarray, ref_dist: np.ndarray) -> np.ndarray:
    """Frozen empirical MP transform from PROTOCOL.md. Lower is closer."""
    n = int(refs.shape[0])
    req(n > 0, "empty MP reference set")
    dx = np.linalg.norm(refs - query[None, :], axis=1)
    req(dx.shape == (n,) and np.isfinite(dx).all(), "invalid query-reference distances")
    threshold = dx[:, None]
    joint = (dx[None, :] > threshold) & (ref_dist > threshold)
    mp = np.sum(joint, axis=1, dtype=np.int64).astype(float) / float(n)
    dmp = 1.0 - mp
    req(dmp.shape == (n,) and np.isfinite(dmp).all(), "invalid MP dissimilarities")
    req(np.all((dmp >= 0.0) & (dmp <= 1.0)), "MP dissimilarity out of range")
    return dmp
...
def main() -> int:
    ...
    np.save(a.output / "GMN_V31_MP_INTRINSIC_FEATURES.npy", X, allow_pickle=False)
    prelabel = {
        ...
        "distance_transform": "frozen empirical Mutual Proximity; labels not used in transform",
    ...
    prelabel_path = a.output / "GMN_V31_MUTUAL_PROXIMITY_PRELABEL.json"
    ...
    for fold in range(5):
        ...
        pos = y[train]
        neg = ~pos
        Drr = pairwise_euclidean(Ztr)
        raw_nearest_counts = np.zeros(Ztr.shape[0], dtype=int)
        mp_nearest_counts = np.zeros(Ztr.shape[0], dtype=int)

        for j, global_i in enumerate(test_indices.tolist()):
            raw = np.linalg.norm(Ztr - Zte[j][None, :], axis=1)
            dmp = empirical_mp_dissimilarity(Zte[j], Ztr, Drr)
            raw_nearest_counts[int(np.argmin(raw))] += 1
            mp_nearest_counts[int(np.argmin(dmp))] += 1
            dpos = float(np.min(dmp[pos]))
            dneg = float(np.min(dmp[neg]))
        ...
        fold_diag.append({
            ...
            "raw_nearest_reference_max_count": int(raw_nearest_counts.max()),
            "mp_nearest_reference_max_count": int(mp_nearest_counts.max()),
        })
    req(np.isfinite(margins).all(), "nonfinite OOF Mutual-Proximity geometry margin")
    ...
    gates = {
        "recovered_at_100_strictly_better_than_parent": int(fused["recovered_at_100"]) > PARENT_RECOVERED_100,
        "recovered_at_50_not_worse_than_parent": int(fused["recovered_at_50"]) >= PARENT_RECOVERED_50,
        "top100_precision_not_worse_than_parent": float(fused["top100_dominant_precision"]) >= PARENT_PRECISION,
        "mrr_not_worse_than_parent": float(fused["mrr"]) >= PARENT_MRR,
        "qualified_count_exact_parent": int(fused["qualified_matches"]) == PARENT_QUALIFIED,
    }
    passed = all(gates.values())
    verdict = "PASS_GMN_V31_MUTUAL_PROXIMITY_OOF" if passed else "FAIL_GMN_V31_MUTUAL_PROXIMITY_OOF"

    result = {
        "verdict": verdict,
        "scientific_role": "TARGET_EXCLUDED_GMN_2022_2023_METHOD_DEVELOPMENT_SUCCESSOR_ONLY",
        ...
        "reference_definition": "exact frozen parent recoverability reference target",
        "strict_whole_shower_oof": True,
        "fold_count": 5,
        "nearest_k": 1,
        "base_distance": "ordinary Euclidean after fold-training z-score",
        "distance_transform": "d_MP(x,y)=1-|{j in R:d(x,j)>d(x,y) and d(y,j)>d(x,y)}|/|R|",
        "mp_strict_inequalities": True,
        "mp_reference_population": "all fold-training families; held-out query not inserted",
        "mp_parametric_approximation": False,
        "mp_pseudocount": False,
        "margin": "d_MP_nonpositive-d_MP_positive",
        "diversity": {"lambda": DIVERSITY_LAMBDA, "scale": DIVERSITY_SCALE},
        "fusion": "equal rank-sum with immutable P19 hard order",
        "hard_baseline": metric_subset(baseline),
        "mutual_proximity_local_geometry_only": metric_subset(local_metrics),
        "mutual_proximity_equal_rank_fusion": metric_subset(fused),
        "parent_benchmark": {
            "recovered_at_100": PARENT_RECOVERED_100,
            "recovered_at_50": PARENT_RECOVERED_50,
            "top100_dominant_precision": PARENT_PRECISION,
            "mrr": PARENT_MRR,
            "qualified_matches": PARENT_QUALIFIED,
        },
        "pass_gates": gates,
        "reported_nonbinding": {
            "recovered_at_25": fused.get("recovered_at_25"),
            "recovered_at_500": fused.get("recovered_at_500"),
        },
        "fold_diagnostics": fold_diag,
        "k_search": False,
        "metric_search": False,
        "empirical_parametric_search": False,
        ...
        "fusion_search": False,
        "hubness_threshold_search": False,
        ...
        "dms_scientific_access": False,
        "blind_exclusion": list(BLIND),
        "claim_boundary": "GMN target-excluded development only. PASS authorizes only a separately frozen exposed-SonotaCo comparison; FAIL permanently rejects this exact empirical-MP v1.",
    }
    out = a.output / "GMN_V31_MUTUAL_PROXIMITY_OOF_RESULT.json"
    out.write_text(json.dumps(result, indent=2, sort_keys=True, allow_nan=False) + "\n")
    print(json.dumps({
        "verdict": verdict,
        "fused100": fused["recovered_at_100"],
        "fused50": fused["recovered_at_50"],
        "fused25": fused.get("recovered_at_25"),
        "fused500": fused.get("recovered_at_500"),
        "fused_precision": fused["top100_dominant_precision"],
        "fused_mrr": fused["mrr"],
        "qualified": fused["qualified_matches"],
        "prelabel_sha256": prelabel_sha,
        "margin_sha256": array_sha(margins),
        "gates": gates,
