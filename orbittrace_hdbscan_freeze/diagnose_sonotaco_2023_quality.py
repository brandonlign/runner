...
def main():
    ...
    rows=core=0; parse_fail=Counter(); passes=Counter(); patterns=Counter(); raw_labels=Counter(); pass_labels=Counter()
    ...
    for raw in reader:
        ...
        label=str(r.get('Shower') or '').strip(); label='SPORADIC' if label.upper() in SPO else label
