...
EXPECTED_P3_SOURCE = Path('/tmp/run_p3_crossfit.py')
...
def load_p3() -> Any:
    require(EXPECTED_P3_SOURCE.is_file(), f'P3 runtime missing: {EXPECTED_P3_SOURCE}')
    spec = importlib.util.spec_from_file_location('orbittrace_p3_crossfit', EXPECTED_P3_SOURCE)
    require(spec is not None and spec.loader is not None, 'cannot load frozen P3 runtime')
...
def self_test() -> None:
    ...
    for prefix in ('# ', '#  ', '\ufeff#  ', ' \t#\t'):
        header = prefix + ';'.join(names)
        fields, positions = compatible_exact_header_positions(header + '\nrow;ignored')
    ...
    print('PASS_P3_HEADER_WHITESPACE_COMPAT_SELF_TEST')
