...
ROOT="$PWD"
OUT="$ROOT/output/m2d-fixed4-consensus-core-v1"
IN="$ROOT/input/m2d-fixed4-consensus"
...
test "$(git hash-object orbittrace_m2d_fixed4_consensus_core_v1/PROTOCOL.md)" = 'ed62d5149cde2c2070eb21520e77503e719ba9e0'
test "$(git hash-object orbittrace_m2d_fixed4_consensus_core_v1/build_pretruth.py)" = '140f21736ea6615fe111e02d91eaa99b19422da7'
test "$(git hash-object orbittrace_m2d_fixed4_consensus_core_v1/evaluate_truth.py)" = 'da56bcac1fc78ade58b4323179a5a980f3b1566e'
test "$(git hash-object orbittrace_topomodal_hierarchy_scale_v1/run_diagnostic.py)" = 'c1efa8da34dea140726a4c2fe4943eb29a304538'
python -m py_compile orbittrace_m2d_fixed4_consensus_core_v1/build_pretruth.py orbittrace_m2d_fixed4_consensus_core_v1/evaluate_truth.py
...
echo '== download immutable target-excluded pretruth/runtime artifacts =='
...
echo '== export label-free target-excluded geometry =='
...
echo '== build and seal exact fixed4 consensus cores before truth =='
python -u orbittrace_m2d_fixed4_consensus_core_v1/build_pretruth.py \
  ...
  --scorer-parts frozen-v8/mondrian_clique_development/source_parts_v2 \
  --v8-result-json "$IN/v8/pooled_year_centroid_v8_development.json" \
  --output "$OUT/pretruth/M2D_FIXED4_CONSENSUS_CORE_V1_PRETRUTH.json"
sha256sum "$OUT/pretruth/M2D_FIXED4_CONSENSUS_CORE_V1_PRETRUTH.json" > "$OUT/pretruth/PRETRUTH_SHA256.txt"
git rev-parse HEAD > "$OUT/pretruth/execution_commit.txt"
python - <<'PY'
import json,hashlib
p='output/m2d-fixed4-consensus-core-v1/pretruth/M2D_FIXED4_CONSENSUS_CORE_V1_PRETRUTH.json'
...
assert r['fixed4_calibration_or_score_threshold_used'] is False
print('PRETRUTH_SEALED_BEFORE_TRUTH',h)
print(json.dumps(r['overall_summary'],sort_keys=True))
PY

# The scientific core is now frozen entirely as stable event IDs. Remove geometry
# so the truth evaluator cannot recompute or modify it.
rm -f "$OUT/pretruth/GMN_LABEL_FREE_GEOMETRY.json"
test -s "$OUT/pretruth/PRETRUTH_SHA256.txt"

echo '== binding same-discovery truth evaluation =='
python -u orbittrace_m2d_fixed4_consensus_core_v1/evaluate_truth.py \
  --fair-pretruth "$IN/fair/GMN_INTERNAL_MASS_SPARSE_LITERATURE_FAIRNESS_V1_PRETRUTH.json" \
  --core-pretruth "$OUT/pretruth/M2D_FIXED4_CONSENSUS_CORE_V1_PRETRUTH.json" \
  ...
  --scorer-parts frozen-v8/mondrian_clique_development/source_parts_v2 \
  --v8-result-json "$IN/v8/pooled_year_centroid_v8_development.json" \
  --output "$OUT/truth/M2D_FIXED4_CONSENSUS_CORE_V1_RESULT.json"
sha256sum "$OUT/truth/M2D_FIXED4_CONSENSUS_CORE_V1_RESULT.json" > "$OUT/truth/RESULT_SHA256.txt"
python --version > "$OUT/truth/python_version.txt"
python -m pip freeze > "$OUT/truth/environment.txt"
python - <<'PY'
import json,hashlib
p='output/m2d-fixed4-consensus-core-v1/truth/M2D_FIXED4_CONSENSUS_CORE_V1_RESULT.json'
r=json.load(open(p))
print('BINDING_VERDICT',r['verdict'])
for comp in ('sugar2017','hdbscan2025'):
 a=r['aggregates'][comp]; s=a['paired_same_discovery']
 print(comp,'paired',s)
 print(comp,'gates',a['gates'])
