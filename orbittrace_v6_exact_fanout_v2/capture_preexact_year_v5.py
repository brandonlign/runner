...
def main() -> int:
    ...
    v6 = load_module(args.repaired_source, f"orbittrace_v6_fast_capture_{args.year}")
    ...
    def capture_calibrate(*call_args, **call_kwargs):
        result = original_calibrate(*call_args, **call_kwargs)
        require(len(result) == 4, "calibration return shape changed")
        proposal_cal, v3_cal, fixed4_cal, summary = result
    ...
    def capture_exact(old_arg, records, window_events, event_lookup, support_arg, base_arg):
        ...
        print(f"V6_FAST_CAPTURE center={center:.1f} records={len(records):,} events={len(ids):,}", flush=True)
        return []

    v6.calibrate_year_v6 = capture_calibrate
    v6.exact_rescore_window_v6 = capture_exact
    try:
        # Executes the immutable prefix exactly once. Empty exact outputs mean the
        # returned audit contains exact prefilter/proposal/calibration counters,
        # while no retained family or truth evaluation can be produced here.
        prefix_audit, captured_anchors, captured_components = v6.scan_year_v6(
    ...
    require(captured_anchors == [] and captured_components == [], "capture unexpectedly retained scientific structures")
    require(int(prefix_audit["retained_detected_anchors"]) == 0, "capture unexpectedly retained exact anchors")
    require(int(prefix_audit["components"]) == 0, "capture unexpectedly built components")
    ordered_centers = sorted(centers)
    total_records = sum(len(centers[center]["records"]) for center in ordered_centers)
    require(total_records == int(prefix_audit["deduplicated_exact_proposals"]), "captured proposal count/audit mismatch")
    require(total_records > 0, "no exact proposals captured")
    require(sorted(calibration_state["v3_cal"]) == list(prefix_audit["supported_bins"]), "captured calibration/audit bin mismatch")

    checkpoint = {
        "format": "orbittrace-v6-preexact-fastfinalize-v5",
        "year": args.year,
        ...
        "firewall": {
            ...
            "exact_outputs_not_available_during_capture": True,
        },
    }
    raw = pickle.dumps(checkpoint, protocol=pickle.HIGHEST_PROTOCOL)
    path = args.output / f"v6_preexact_v5_{args.year}.pkl"
    path.write_bytes(raw)
    digest = sha256_bytes(raw)
    path.with_suffix(".sha256").write_text(digest + "\n")
    (args.output / f"v6_preexact_v5_{args.year}.json").write_text(json.dumps({
        "year": args.year,
        "checkpoint_sha256": digest,
        "centers": len(ordered_centers),
        "total_records": total_records,
        "supported_bins": prefix_audit["supported_bins"],
        "proposal_candidates_scored": prefix_audit["proposal_candidates_scored"],
        "prefilter_candidates": prefix_audit["prefilter_candidates"],
        "scan_rows_sha256": scan_sha,
        "calibration_rows_sha256": calibration_sha,
    }, indent=2, sort_keys=True) + "\n")
    print(f"PASS_V6_FASTFINALIZE_PREEXACT_CAPTURE year={args.year} centers={len(ordered_centers)} records={total_records:,} sha={digest}", flush=True)
    return 0
