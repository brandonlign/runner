...
def main()->int:
    ...
    require(summary['verdict']=='PASS_V31_RELATIVE_SOURCEBLIND_CANONICAL_SONOTACO_PRETRUTH_CATALOGUE_FREEZE','pretruth catalogue not frozen')
    ...
    require(summary['categorical_columns']==[0] and summary['relative_transform']==TRANSFORM,'relative transform boundary changed')
    require(summary['panel_specific_candidate_generation'] is False and summary['panel_specific_ranking'] is False,'panel-specific pretruth behavior entered')
    catalogue=json.loads(a.catalogue.read_text())
    require(sha(a.catalogue)==summary['primary_output_sha256'],'pretruth catalogue hash changed')
    ...
    require(catalogue['feature_dimension']==21 and catalogue['source_specific_features_used'] is False,'source-specific ranking information entered v31')
    require(catalogue['categorical_columns']==[0] and catalogue['relative_transform']==TRANSFORM,'relative representation changed')
    families=catalogue['families']
    require(len(families)==catalogue['family_count'] and [int(f['rank']) for f in families]==list(range(1,len(families)+1)),'ranked family catalogue invalid')
    final=json.loads((a.truth_root/'V15_FINAL_LITERATURE_RESULT.json').read_text())
    require(final['verdict']=='FAIL_FINAL_LITERATURE_SUPERIORITY','immutable truth/comparator package changed')
    panels=[]
    for route,year in PANELS:
        ...
        budget=int(frozen['candidate_budget']['comparator_budget']); cur=evaluate(families,truth,budget); lit=frozen['comparator_summary']
        cm=float(cur['macro_f1']); cr=int(cur['recovered_f1_gt_0_5']); lm=float(lit['macro_f1']); lr=int(lit['recovered_f1_gt_0_5']); win=bool(cm>lm and cr>=lr)
    ...
    result={
        'scientific_stage':'V31_RELATIVE_SOURCEBLIND_CANONICAL_SONOTACO_EXPOSED_LITERATURE_DEVELOPMENT',
        'single_canonical_detector_catalogue':True,'panel_specific_candidate_generation':False,'panel_specific_ranking':False,
        'candidate_catalogue_sha256':sha(a.catalogue),'candidate_order_sha256':catalogue['v31_order_sha256'],
        'candidate_counts':catalogue['candidate_counts'],'membership_diagnostics':catalogue['membership_diagnostics'],
        'feature_dimension':21,'source_specific_features_used':False,'categorical_columns':[0],'relative_transform':TRANSFORM,
        'panels':panels,'panel_wins':wins,
        'verdict':'PASS_V31_RELATIVE_SOURCEBLIND_CANONICAL_SONOTACO_ALL_PANEL_LITERATURE_SUPERIORITY_DEVELOPMENT' if passed else 'FAIL_V31_RELATIVE_SOURCEBLIND_CANONICAL_SONOTACO_ALL_PANEL_LITERATURE_SUPERIORITY_DEVELOPMENT',
        'sonotaco_role':'EXPOSED_DEVELOPMENT_ONLY','model_retrained_on_sonotaco':False,'matched_comparator_rows_used_as_detector_input':False,
        'parameter_search':False,'post_result_second_search':False,'source_quota_selected':False,'partial_source_feature_restoration':False,'alternate_transform_search':False,
        'maarsy_scientific_access':False,'dms_scientific_access':False,'target_information_access':False,
    }
    (a.output/'V31_RELATIVE_SOURCEBLIND_CANONICAL_SONOTACO_EXPOSED_RESULT.json').write_text(json.dumps(result,indent=2,sort_keys=True,allow_nan=False)+'\n')
