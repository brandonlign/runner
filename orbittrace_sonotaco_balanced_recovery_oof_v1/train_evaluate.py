...
VARIANT='balanced_recovery_oof_v19_rank_sum'
...
def positive_probability(model:ExtraTreesClassifier,X:np.ndarray)->np.ndarray:
    ...
    require(0 in classes and 1 in classes,'recovery classifier missing a class')
    j=classes.index(1); out=np.asarray(model.predict_proba(X)[:,j],dtype=np.float64)
    require(out.shape==(len(X),) and np.all(np.isfinite(out)) and np.all((out>=0.0)&(out<=1.0)),'invalid recovery probabilities')
    return out
...
def main()->int:
    ...
    # Exact repaired v22-v25 pretruth scientific identity before exposed truth is interpreted.
    for route in ROUTES:
    ...
    ranker=v23.load_module(a.ranker_source,'frozen_839_balanced_recovery')
    ...
    for route in ROUTES:
        ...
        for i,(f,b) in enumerate(zip(fams,best)):
            ...
            y.append(target); gs.append(group); annual.append((float(f13),float(f14)))

        yarr=np.asarray(y,dtype=np.int8)
        require(set(np.unique(yarr).tolist()).issubset({0,1}) and np.unique(yarr).size==2,f'{route} balanced-recovery target degenerate')
        route_offsets[route]=(cursor,cursor+len(ids)); cursor+=len(ids)
        Xs.append(X); targets.append(yarr); groups.extend(gs)
        route_data[route]={'meta':meta,'families':fams,'ids':ids,'centroids':C,'best':best,'annual':annual,'eligible':eligible}
        target_diag[route]={
            'families':len(ids),
            'eligible_recurrent_showers':len(eligible),
            'families_with_best_recurrent_label':int(sum(b['best_label'] is not None for b in best)),
            'balanced_recovery_positive_families':int(np.sum(yarr==1)),
            'balanced_recovery_negative_families':int(np.sum(yarr==0)),
            'positive_fraction':float(np.mean(yarr)),
            'target_definition':'1 iff F1_2013>0.5 AND F1_2014>0.5 for exact v22-v25 best_label',
    ...
    require(np.unique(yall).size==2,'stacked target degenerate')
    ...
    for fold in range(5):
        ...
        require(np.unique(yall[tr]).size==2,f'training fold {fold} lacks both recovery classes')
        ...
        fold_diag.append({
            ...
            'train_positive':int(np.sum(yall[tr]==1)),'test_positive':int(np.sum(yall[te]==1)),
    ...
    full_freeze={'verdict':'NOT_FROZEN_BALANCED_RECOVERY_OOF_FAIL','model_sha256':None}
    if passed:
        full=recovery_model(); full.fit(Xall,yall,sample_weight=weights); full.set_params(n_jobs=1)
        path=a.output/'sonotaco_balanced_recovery_classifier.joblib'; joblib.dump(full,path)
        full_freeze={
            'verdict':'PASS_FULL_EXPOSED_SONOTACO_BALANCED_RECOVERY_MODEL_FREEZE',
    ...
    (a.output/'BALANCED_RECOVERY_FULL_MODEL_FREEZE.json').write_text(json.dumps(full_freeze,indent=2,sort_keys=True)+'\n')

    result={
        'scientific_stage':'EXPOSED_SONOTACO_STRICT_GROUP_BALANCED_RECOVERY_OOF_RANKING_DEVELOPMENT_V1',
        'verdict':'PASS_BALANCED_RECOVERY_ALL_PANEL_LITERATURE_SUPERIORITY_DEVELOPMENT' if passed else 'FAIL_BALANCED_RECOVERY_ALL_PANEL_LITERATURE_SUPERIORITY_DEVELOPMENT',
        'sole_scientific_change_from_v25_pairwise':'pairwise balanced-quality preference -> direct benchmark-defined two-year recovery classification',
        ...
        'strict_whole_shower_oof':True,'shared_model_across_routes':True,
        ...
        'group_weighting':'exact #839 inverse strict-group weights',
        ...
        'feature_search':False,'target_search':False,'model_search':False,'hyperparameter_search':False,'fusion_search':False,'diversity_search':False,'source_quota_selected':False,'post_result_second_search':False,
    ...
    (a.output/'SONOTACO_BALANCED_RECOVERY_OOF_RESULT.json').write_text(json.dumps(result,indent=2,sort_keys=True,allow_nan=False)+'\n')
