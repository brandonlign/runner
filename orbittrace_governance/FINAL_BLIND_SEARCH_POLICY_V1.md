# OrbitTrace final blind-search policy — v1
...
No final target-containing execution is permitted until the exact frozen candidate has satisfied all of:
...
## Stage A — target-free full ranking
...
- the complete primary ranked family list, not merely the top K;
- every family ID and rank;
- exact member stable event IDs by year;
- all primary scores needed to reproduce the ordering;
- candidate/family provenance and scientific-source hashes.

Stage A may not use catalogue shower labels, OrbitTrace IDs, target coordinates, target orbit/radiant/activity information, prior recovery information, or any target-specific exception.

After the complete ranking is frozen, it cannot be reranked, merged, expanded, filtered, or rescored based on Stage B.

## Stage B — exact-ID-only reveal

Stage B receives only the withheld **stable event-ID sets by year** needed for exact set intersection with Stage-A frozen members.

Stage B is forbidden from receiving or using:

- target coordinates;
- radiant/speed values;
- orbital elements;
- activity profile or solar-longitude center;
- target name/identity during matching;
- nearest-neighbor or geometric matching;
- member expansion;
- family merging;
- post-reveal reranking;
- manual candidate selection.

The reveal operation is exact stable-ID set overlap only.

## Frozen overlap requirement

A Stage-A family is an eligible OrbitTrace recovery only if it contains:

- at least **4 exact withheld IDs in 2022**;
...
- `PARTIAL_FINAL_BLIND_ORBITTRACE_RECOVERY`: an eligible recovery exists at primary rank **26–100**.
...
After Stage B opens target IDs, the final candidate is frozen permanently. No target-informed parameter change, membership adjustment, ranking change, threshold relaxation, alternate family selection rule, or rerun can be presented as a blind discovery.

Any later target-informed analysis is characterization only.
