#!/usr/bin/env python3
"""Frozen target-excluded GMN v31 successor using Manhattan nearest-reference geometry."""
...
def l1_distances(refs: np.ndarray, point: np.ndarray) -> np.ndarray:
    refs = np.asarray(refs, dtype=float)
    point = np.asarray(point, dtype=float)
    req(refs.ndim == 2 and point.ndim == 1 and refs.shape[1] == len(point), "L1 shape mismatch")
    out = np.sum(np.abs(refs - point[None, :]), axis=1)
    req(np.isfinite(out).all() and np.all(out >= 0.0), "invalid Manhattan distance")
    return out
...
def run_self_tests() -> int:
    refs = np.asarray([[0.0, 0.0], [2.0, -1.0], [1.0, 4.0]], dtype=float)
    point = np.asarray([1.0, 1.0], dtype=float)
    got = l1_distances(refs, point)
    expected = np.asarray([2.0, 3.0, 3.0], dtype=float)
    req(np.array_equal(got, expected), f"L1 analytic distance self-test failed: {got}")

    # L1 should reduce the relative leverage of one large coordinate compared with L2.
    a = np.asarray([6.0, 0.0], dtype=float)
    b = np.asarray([3.0, 3.0], dtype=float)
    origin = np.zeros(2, dtype=float)
    l1a = float(l1_distances(a[None, :], origin)[0])
    l1b = float(l1_distances(b[None, :], origin)[0])
    l2a = float(np.linalg.norm(a))
    l2b = float(np.linalg.norm(b))
    req(l1a == l1b == 6.0 and l2a > l2b, "coordinate-leverage self-test failed")

    ranks = {"a": 2, "b": 1}
    idx = nearest_index(np.asarray([3.0, 3.0]), ["a", "b"], ranks)
    req(idx == 1, "deterministic nearest tie self-test failed")

    print(json.dumps({
        "verdict": "PASS_MANHATTAN_LOCAL_GEOMETRY_ENGINEERING_SELF_TESTS",
        "analytic_l1_distances": got.tolist(),
        "coordinate_leverage_check": True,
...
def main() -> int:
    ...
    euclidean_margin = np.zeros(EXPECTED_N, dtype=float)
    manhattan_raw = np.zeros(EXPECTED_N, dtype=float)
    ...
    for fold in range(5):
        ...
        req(len(P) == len(pos_ids) and len(N) == len(neg_ids) and len(P) > 0 and len(N) > 0,
            f"class reference alignment changed fold {fold}")

        fold_l1: list[float] = []
        fold_l2: list[float] = []
        for j, global_i in enumerate(test_indices.tolist()):
            z = Zte[j]
            dpos_l2_all = np.linalg.norm(P - z[None, :], axis=1)
            dneg_l2_all = np.linalg.norm(N - z[None, :], axis=1)
            dpos_l1_all = l1_distances(P, z)
            dneg_l1_all = l1_distances(N, z)
            req(np.isfinite(dpos_l2_all).all() and np.isfinite(dneg_l2_all).all(), "nonfinite Euclidean provenance distance")

            dpos_l2 = float(np.min(dpos_l2_all))
            dneg_l2 = float(np.min(dneg_l2_all))
            dpos_l1 = float(np.min(dpos_l1_all))
            dneg_l1 = float(np.min(dneg_l1_all))
            m2 = dneg_l2 - dpos_l2
            m1 = dneg_l1 - dpos_l1
            euclidean_margin[global_i] = m2
            manhattan_raw[global_i] = m1
            fold_l2.append(m2)
            fold_l1.append(m1)

            ip = nearest_index(dpos_l1_all, pos_ids, hard_rank)
            inn = nearest_index(dneg_l1_all, neg_ids, hard_rank)
            evidence_rows.append({
                "family_id": ids[global_i],
                "fold": fold,
                "nearest_positive_family_id_l1": pos_ids[ip],
                "nearest_nonpositive_family_id_l1": neg_ids[inn],
                "nearest_positive_distance_l1": dpos_l1,
                "nearest_nonpositive_distance_l1": dneg_l1,
                "raw_manhattan_margin": m1,
                "parent_euclidean_margin": m2,
            })
        ...
        fold_diag.append({
            ...
            "parent_euclidean_margin_min": float(np.min(fold_l2)),
            "parent_euclidean_margin_median": float(np.median(fold_l2)),
            "parent_euclidean_margin_max": float(np.max(fold_l2)),
            "raw_manhattan_margin_min": float(np.min(fold_l1)),
            "raw_manhattan_margin_median": float(np.median(fold_l1)),
            "raw_manhattan_margin_max": float(np.max(fold_l1)),
        })

    req(euclidean_margin.shape == (EXPECTED_N,) and np.isfinite(euclidean_margin).all(), "invalid parent Euclidean margin vector")
    req(manhattan_raw.shape == (EXPECTED_N,) and np.isfinite(manhattan_raw).all(), "invalid raw Manhattan margin vector")
    req(array_sha(euclidean_margin) == PARENT_MARGIN_SHA, "parent Euclidean OOF margin did not reproduce exactly")
    req(len(evidence_rows) == EXPECTED_N and len({r["family_id"] for r in evidence_rows}) == EXPECTED_N,
        "Manhattan evidence universe changed")

    parent_scale = float(np.median(np.abs(euclidean_margin)))
    manhattan_scale = float(np.median(np.abs(manhattan_raw)))
    req(np.isfinite(parent_scale) and parent_scale > 0.0, "invalid parent median absolute margin")
    req(np.isfinite(manhattan_scale) and manhattan_scale > 0.0, "invalid Manhattan median absolute margin")
    unit_factor = float(parent_scale / manhattan_scale)
    req(np.isfinite(unit_factor) and unit_factor > 0.0, "invalid Manhattan unit factor")
    manhattan_scaled = manhattan_raw * unit_factor
    req(np.isfinite(manhattan_scaled).all(), "nonfinite scaled Manhattan margin")
    ...
    local_idx = q.diversity_order(manhattan_scaled, cm, DIVERSITY_LAMBDA, DIVERSITY_SCALE, tie)
    ...
    verdict = "PASS_GMN_V31_MANHATTAN_LOCAL_GEOMETRY_V1" if passed else "FAIL_GMN_V31_MANHATTAN_LOCAL_GEOMETRY_V1"

    evidence = {
        "scientific_role": "TARGET_EXCLUDED_GMN_OOF_MANHATTAN_LOCAL_GEOMETRY_EVIDENCE",
        "candidate_count": EXPECTED_N,
        "parent_euclidean_margin_sha256": array_sha(euclidean_margin),
        "raw_manhattan_margin_sha256": array_sha(manhattan_raw),
        "scaled_manhattan_margin_sha256": array_sha(manhattan_scaled),
        "parent_median_absolute_margin": parent_scale,
        "manhattan_median_absolute_margin": manhattan_scale,
    ...
    (a.output / "GMN_V31_MANHATTAN_LOCAL_GEOMETRY_EVIDENCE.json").write_text(
    ...
    result = {
        ...
        "manhattan_local_only": metric_subset(local_metrics),
        "manhattan_equal_rank_fusion": metric_subset(fused),
        "pass_gates": gates,
        "parent_median_absolute_margin": parent_scale,
        "manhattan_median_absolute_margin": manhattan_scale,
        "unit_factor": unit_factor,
        "unit_preservation": "median(abs(parent Euclidean margin))/median(abs(raw Manhattan margin))",
        "strict_whole_shower_oof": True,
        "fold_count": 5,
        "parent_standardization": "fold-training mean/population-standard-deviation z-score",
        "classifier": "binary k=1 nearest positive versus nearest nonpositive reference gap",
        "metric": "Manhattan L1 in standardized 23D parent representation",
        "local_score": "d_nonpositive_L1 - d_positive_L1, positively rescaled only for inherited diversity units",
        "nearest_k": 1,
    ...
    (a.output / "GMN_V31_MANHATTAN_LOCAL_GEOMETRY_V1_RESULT.json").write_text(
