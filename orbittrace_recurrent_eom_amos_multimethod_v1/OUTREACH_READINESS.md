# AMOS 2023/2024 external-validation outreach readiness

**Status: POSITIVE acquisition readiness; scientific result: NONE.**

This record contains only current public project/contact metadata. It does not access AMOS event-level values, shower labels, uncertainties, orbit elements, the protected OrbitTrace target region, MAARSY, or DMS.

## Current official contact verification

Checked 2026-08-14 against current Comenius University / AMOS pages:

- AMOS lists **prof. RNDr. Juraj Tóth, PhD.** as project lead / principal investigator.
- Current official project/contact email: **Juraj.Toth@fmph.uniba.sk**.
- The current AMOS English project page states that a pair of stations detects roughly **5,000–8,000 common meteors per year**, depending on station separation and observing conditions. This is sufficient scale to make the frozen 2023/2024 complete-sample request scientifically meaningful if the reduced trajectory fields can be provided.

Official metadata sources checked:

- https://fmph.uniba.sk/microsites/daa/daa/veda-a-vyskum-dev/amos/
- https://fmph.uniba.sk/en/microsites/daa/division-of-astronomy-and-astrophysics/research/amos/
- https://fmph.uniba.sk/daa/

No event list, trajectory table, shower catalogue, downloadable event asset, or scientific row was opened during this verification.

## Frozen outreach payload

The send-ready request remains exactly:

`orbittrace_recurrent_eom_amos_multimethod_v1/DATA_REQUEST_READY_WITH_COMPARATORS.md`

Recipient:

`Juraj.Toth@fmph.uniba.sk`

Subject:

`Research request: blinded AMOS 2023–2024 multi-station meteor solutions`

The request preserves the existing staged blind-transfer contract:

1. Stage 1: `event_id,utc_time,solar_longitude_deg` only;
2. local inclusive `[20°,55°]` removal and retained-ID freeze;
3. Stage 2 retained geometry only;
4. optional Stage 2B comparator-only fields, never admitted to recurrent-EOM features/ranking;
5. Stage 3 shower associations only after all candidate/comparator rankings are hash-frozen.

## Next action boundary

The repository and protocol are ready for outreach. **No email is sent by this record.** Sending external correspondence should use the exact frozen draft above; receipt of any reply or data does not authorize protocol, feature, threshold, metric, gate, year, or representation changes.
