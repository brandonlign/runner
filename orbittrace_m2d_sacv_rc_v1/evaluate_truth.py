...
def main()->int:
    ...
    for n in ('fair-pretruth','rc-pretruth','internal-prelabel','quality-source','support-source-parts','candidate-payload','baseline-payload','scorer-parts','v8-result-json','output'):ap.add_argument('--'+n,type=Path,required=True)
    ...
    fair=json.loads(a.fair_pretruth.read_text());rc=json.loads(a.rc_pretruth.read_text());req(rc['scientific_role']=='TARGET_EXCLUDED_SACV_RECURRENCE_COMPONENT_MEMBERSHIPS_FROZEN_BEFORE_SHOWER_TRUTH','wrong rc role');req(rc['fair_pretruth_sha256']==FAIR_SHA,'rc parent changed');req(rc['shower_truth_used'] is False and rc['target_information_access'] is False and rc['target_region_events_accessed'] is False and rc['post_result_parameter_search'] is False,'rc firewall')
    fs={(int(s['denominator']),int(s['bucket'])):s for s in fair['subsets']};xs={(int(s['denominator']),int(s['bucket'])):s for s in rc['subsets']};req(set(fs)==set(xs)=={(d,b) for d in DENOMS for b in BUCKETS},'panel mismatch')
    for key in fs:
    ...
    q=load(a.quality_source,'rc_truth_q');q.v1.mult.YEARS=YEARS;q.v1.mult.MONTH_KEYS=MONTH_KEYS;q.v1.mult.TOP_K=100;rt=q.v1.mult.load_frozen_runtime();support=rt.load_support_module(a.support_source_parts);support.YEARS=YEARS;support.MONTH_KEYS=MONTH_KEYS;support.CORPUS='orbittrace-m2d-sacv-rc-v1-gmn-truth';support.RANKING_VARIANTS=('persistence',);req((float(support.BLIND_LOW),float(support.BLIND_HIGH))==BLIND,'blind changed');setattr(a,'fixed4_baseline_json',a.v8_result_json);_c,base,_s=support.load_sources(a);scan,_cal,hidden,sources=support.parse_catalogue(base);req(sorted(scan)==list(YEARS) and [x['key'] for x in sources]==list(MONTH_KEYS),'source set changed')
