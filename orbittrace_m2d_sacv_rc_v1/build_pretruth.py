...
class Runtime:
    ...
    def proc(self,c,rank):
        ids=sorted(map(str,c['event_ids']));req(all(x in self.byid for x in ids),f'missing parent geometry rank {rank}');src={2022:self.enumerate_sources(ids,2022),2023:self.enumerate_sources(ids,2023)};nodes=[]
        for y in YEARS:
            for h in src[y]:h['members_all']=self.members(ids,h['center'],h['radius']);nodes.append((y,h['id']))
        bykey={(y,h['id']):h for y in YEARS for h in src[y]};edges=[];adj={n:set() for n in nodes}
        for a in src[2022]:
            for b in src[2023]:
                d=float(np.linalg.norm(a['center']-b['center']))
                if d>a['radius']+1e-12 or d>b['radius']+1e-12:continue
                ab=len(self.members(ids,a['center'],a['radius'],2023));ba=len(self.members(ids,b['center'],b['radius'],2022))
                if ab<MIN_SUPPORT or ba<MIN_SUPPORT:continue
                u=(2022,a['id']);v=(2023,b['id']);adj[u].add(v);adj[v].add(u);edges.append({'a':a['id'],'b':b['id'],'d':d,'ab':ab,'ba':ba,'min_cross_support':min(ab,ba),'sum_cross_support':ab+ba,'min_excess':min(float(a['excess']),float(b['excess'])),'sum_excess':float(a['excess'])+float(b['excess'])})
        ...
        def pub(h):return {k:v for k,v in h.items() if k not in ('center','members_all')}
        ...
        return {'rank':rank,'family_id':str(c['family_id']),'family_hash':str(c['family_hash']),'parent_n':len(ids),'refined':bool(sel),'output_n':len(o),'ratio':len(o)/len(ids) if ids else 0.0,'output_ids':o,'hypothesis_counts':{str(y):len(src[y]) for y in YEARS},'validated_edge_count':len(edges),'component_count':len(components),'selected_component_index':0 if sel else None,'selected_component_edge_count':len(sel['edges']) if sel else 0,'selected_component_node_count':len(sel['nodes']) if sel else 0,'components':pcs,'hypotheses':{str(y):[pub(h) for h in src[y]] for y in YEARS}}
...
def main():
    ...
    for s in fair['subsets']:
        ...
        subsets.append({'denominator':d,'bucket':b,'parent_candidate_count':len(parents),'extractions':rows});print(json.dumps({'panel':f'd{d}_b{b}','candidates':len(rows),'refined':sum(x['refined'] for x in rows),'mean_ratio':float(np.mean([x['ratio'] for x in rows])),'validated_edges':sum(x['validated_edge_count'] for x in rows),'components':sum(x['component_count'] for x in rows)},sort_keys=True),flush=True)
    payload={'schema':'ORBITTRACE_M2D_SACV_RC_V1_GMN_PRETRUTH','scientific_role':'TARGET_EXCLUDED_SACV_RECURRENCE_COMPONENT_MEMBERSHIPS_FROZEN_BEFORE_SHOWER_TRUTH','fair_pretruth_sha256':FAIR_SHA,'geometry_sha256':sha(a.geometry),'years':list(YEARS),'blind_exclusion':BLIND,'configuration':{'rmax':RMAX,'minimum_support':MIN_SUPPORT,'contamination_max':CONTAM_MAX,'analog_offsets_deg':DELTAS[1:].tolist(),'physical_scales':{'solar_deg':5.0,'radiant_deg':4.0,'speed_fraction':0.10},'annual_hypotheses':'all SACV-v1-admissible per-center widest-radius hypotheses','crossyear_edges':'exact SACV-v1 reciprocal validation for every annual-hypothesis pair','component_membership':'union of all SACV balls in selected validated recurrence component','component_selection':'max validated edge count, then node count, summed minimum cross-support, summed minimum excess, deterministic member hash','fallback':'exact_parent_only_if_no_validated_recurrence_component'},'subsets':subsets,'summary':{'candidate_occurrences':len(allrows),'refined_occurrences':sum(x['refined'] for x in allrows),'mean_parent_n':float(np.mean([x['parent_n'] for x in allrows])),'mean_output_n':float(np.mean([x['output_n'] for x in allrows])),'validated_edges':sum(x['validated_edge_count'] for x in allrows),'recurrent_components':sum(x['component_count'] for x in allrows)},'parent_discovery_membership_changed':False,'parent_rank_changed':False,'shower_truth_used':False,'target_information_access':False,'target_region_events_accessed':False,'sonotaco_scientific_access':False,'post_result_parameter_search':False};a.output.write_text(json.dumps(payload,separators=(',',':'),sort_keys=True,allow_nan=False)+'\n');print(json.dumps({'verdict':'PASS_M2D_SACV_RC_V1_GMN_PRETRUTH','sha256':sha(a.output),'summary':payload['summary']},indent=2,sort_keys=True));return 0
