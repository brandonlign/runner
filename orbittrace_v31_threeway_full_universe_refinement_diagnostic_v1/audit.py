...
def compare(threeway: list[dict[str, Any]], joint_only: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        'threeway': a,
        'joint_only': b,
        'threeway_minus_joint_only_recoverable_fraction': (
            float(pa - pb) if pa is not None and pb is not None else None
        ),
        'risk_ratio': rr,
        'risk_ratio_defined': bool(rr is not None),
        'direction_pass': direction,
    }


def validate_cross(vector: dict[str, Any], cross_path: Path) -> dict[str, Any]:
    require(sha(cross_path) == CROSS_RESULT_SHA, '#1093 result SHA changed')
    ...
    require(cross['new_rank_or_score_evaluated'] is False, '#1093 unexpectedly evaluated a score')
    require(cross['selector_evaluated'] is False and cross['replacement_rule_evaluated'] is False and cross['successor_selected'] is False, '#1093 unexpectedly evaluated a selector')
    ...
    require(len(vmap) == N_HDB, 'frozen vector duplicate family')
    checked = 0
    for year in ('2013', '2014'):
        rows = cross['annual_diagnostics'][year]['groups']
        require(len(rows) == 18, f'#1093 {year} representative count changed')
        for r in rows:
            fid = str(r['representative_family_id'])
            require(fid in vmap, f'#1093 representative missing from vector: {fid}')
            vr = vmap[fid]
            require(int(vr['v31_rank']) == int(r['v31_rank']), f'#1093 v31 rank mismatch: {fid}')
            require(bool(vr['crossroute_positive']) == bool(r['crossroute_positive']), f'#1093 cross-route sign mismatch: {fid}')
            a = vr['crossroute_rank_gap']
            b = r['crossroute_rank_gap']
            require((a is None) == (b is None), f'#1093 null-gap mismatch: {fid}')
            if a is not None:
                require(abs(float(a) - float(b)) <= 1e-15, f'#1093 rank-gap mismatch: {fid}')
            checked += 1
...
def main() -> int:
    ...
    require(int(vector['joint_family_count']) == JOINT_COUNT, 'three-way joint count changed')
    ...
    validation = validate_cross(vector, a.cross_result)
    ...
    require([str(f['family_id']) for f in fams] == ids, '#950 HDB family identity changed')
    ...
    require(set(vmap) == set(ids), '#950/vector family universe mismatch')
    ...
    for year in (2013, 2014):
        ...
        direction_flags.append(year_gate)
        annual[str(year)] = {
    ...
    passed = bool(strict_subset and all(direction_flags))
    result = {
        ...
        'vector_canonical_sha256': expected_vector_sha,
        'family_count': N_HDB,
        ...
        'interpretation_gate': 'strict nonempty subset of 60 and defined RR>1 with higher recoverability fraction for threeway than joint-only at family and diagnostic-group levels in both years',
