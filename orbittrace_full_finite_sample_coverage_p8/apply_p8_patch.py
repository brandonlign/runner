...
CONST_ANCHOR = '''P7_ROBUST_FLOOR_MIN_SEEDS = 19
'''
CONST_REPL = '''P7_ROBUST_FLOOR_MIN_SEEDS = 19
P8_COVERAGE_ALPHA = P3_NEGATIVE_TAIL_MAX
'''

RANK_ANCHOR = '''            membership_floor_rank = 2 if len(sorted_seed_probabilities) >= P7_ROBUST_FLOOR_MIN_SEEDS else 1
...
'''
RANK_REPL = '''            membership_floor_rank = max(1, int(np.floor(P8_COVERAGE_ALPHA * (len(sorted_seed_probabilities) + 1))))
            membership_floor = float(sorted_seed_probabilities[membership_floor_rank - 1])
            require(membership_floor >= seed_floor, "P8 coverage membership floor below inherited minimum seed floor")
            require(membership_floor_rank <= len(sorted_seed_probabilities), "P8 coverage rank outside held-out seed vector")
'''
...
RECORD_REPL = '''                "membership_floor": membership_floor,
                "membership_floor_rank": membership_floor_rank,
                "membership_floor_alpha": P8_COVERAGE_ALPHA,
                "membership_floor_rule": "kth-smallest held-out recurrent-seed probability where k=max(1,floor(P3_NEGATIVE_TAIL_MAX*(seed_count+1)))",
'''
...
'''
PAYLOAD_REPL = '''        "p7_membership_floor": "superseded by P8 maximal finite-sample order statistic under inherited alpha",
        "p7_robust_floor_min_seeds": P7_ROBUST_FLOOR_MIN_SEEDS,
        "p7_order_statistic_tuning": False,
        "p8_membership_floor": "kth-smallest held-out recurrent-seed probability where k=max(1,floor(P3_NEGATIVE_TAIL_MAX*(seed_count+1)))",
        "p8_coverage_alpha": P8_COVERAGE_ALPHA,
        "p8_order_statistic_tuning": False,
        "no_known_shower_truth_used": True,
'''

PROPOSAL_ANCHOR = '''                "membership_floor": float(gate["membership_floor"]),
                "membership_floor_rank": int(gate["membership_floor_rank"]),
                "scoring_fold": scoring_fold,
'''
PROPOSAL_REPL = '''                "membership_floor": float(gate["membership_floor"]),
                "membership_floor_rank": int(gate["membership_floor_rank"]),
                "membership_floor_alpha": float(gate["membership_floor_alpha"]),
                "scoring_fold": scoring_fold,
'''
...
'''
GATE_REPL = '''        "p8_coverage_alpha_exactly_inherited_p3_tail": abs(P8_COVERAGE_ALPHA - P3_NEGATIVE_TAIL_MAX) < 1e-15 and abs(P8_COVERAGE_ALPHA - 0.10) < 1e-15,
        "p8_maximal_finite_sample_rank_exact": all(
            int(r["membership_floor_rank"]) == max(1, int(np.floor(P8_COVERAGE_ALPHA * (int(r["seed_count"]) + 1))))
            for r in reliability.values()
        ),
        "p8_coverage_bound_when_feasible": all(
            (int(r["seed_count"]) < 9 and int(r["membership_floor_rank"]) == 1 and 1.0 / (int(r["seed_count"]) + 1) > P8_COVERAGE_ALPHA)
            or (
                int(r["seed_count"]) >= 9
                and int(r["membership_floor_rank"]) / (int(r["seed_count"]) + 1) <= P8_COVERAGE_ALPHA + 1e-15
                and (int(r["membership_floor_rank"]) + 1) / (int(r["seed_count"]) + 1) > P8_COVERAGE_ALPHA - 1e-15
            )
            for r in reliability.values()
        ),
        ...
        "p8_no_p6_unreliable_direction_can_propose": all(
            bool(reliability[f"{p['family_id']}|{p['source_year']}|{p['target_year']}"]["reliable"])
            for ps in proposals_by_event.values() for p in ps
        ),
'''
...
VERDICT_REPL = '''    verdict = (
        "PASS_FULL_FINITE_SAMPLE_COVERAGE_MEMBERSHIP_P8_DEVELOPMENT"
        if all(gates.values())
        else "FAIL_FULL_FINITE_SAMPLE_COVERAGE_MEMBERSHIP_P8_NO_GO"
    )
'''
...
'''
CLASS_REPL = '''        "classification": "P7 lineage with maximal finite-sample held-seed order statistic permitted by the inherited P3 alpha=0.10 exclusion budget; immutable promoted-v8 cores and rank",
'''
...
'''
CONFIG_REPL = '''            "p7_membership_floor": "superseded by P8 maximal finite-sample order statistic under inherited alpha",
            "p7_robust_floor_min_seeds": P7_ROBUST_FLOOR_MIN_SEEDS,
            "p7_order_statistic_tuning": False,
            "p7_parameter_search": False,
            "p8_membership_floor": "kth-smallest held-out recurrent-seed probability where k=max(1,floor(P3_NEGATIVE_TAIL_MAX*(seed_count+1)))",
...
'''
DIAG_REPL = '''            "p8_coverage_alpha": P8_COVERAGE_ALPHA,
            ...
            "p8_rank_ge5_reliable_directions": sum(bool(r["reliable"]) and int(r["membership_floor_rank"]) >= 5 for r in reliability.values()),
            "p8_membership_floor_rank_max": max(int(r["membership_floor_rank"]) for r in reliability.values()),
...
'''
JSON_REPL = '''    (args.output / "full_finite_sample_coverage_membership_p8_development.json").write_text(json.dumps(result, indent=2) + "\\n")
'''
MD_ANCHOR = '''    (args.output / "FINITE_SAMPLE_ROBUST_FLOOR_MEMBERSHIP_P7_DEVELOPMENT.md").write_text(
'''
MD_REPL = '''    (args.output / "FULL_FINITE_SAMPLE_COVERAGE_MEMBERSHIP_P8_DEVELOPMENT.md").write_text(
'''
TITLE_ANCHOR = '''        "# OrbitTrace P7 finite-sample robust held-seed floor membership development\\n\\n"
'''
TITLE_REPL = '''        "# OrbitTrace P8 full finite-sample coverage held-seed floor membership development\\n\\n"
'''
...
'''
PRINT_REPL = '''    print((args.output / "FULL_FINITE_SAMPLE_COVERAGE_MEMBERSHIP_P8_DEVELOPMENT.md").read_text(), flush=True)
'''
...
def main() -> int:
    ...
    for before, after, label in (
        (CONST_ANCHOR, CONST_REPL, "P8 inherited-alpha constant"),
        (RANK_ANCHOR, RANK_REPL, "P8 maximal finite-sample rank"),
        (RECORD_ANCHOR, RECORD_REPL, "P8 reliability floor record"),
        (PAYLOAD_ANCHOR, PAYLOAD_REPL, "P8 crossfit declaration"),
        (PROPOSAL_ANCHOR, PROPOSAL_REPL, "P8 proposal floor record"),
    ...
    print("P8_PATCH_SCOPE=exact P7 plus maximal held-seed order statistic under inherited P3 alpha=0.10; all reliability/geometry/rank unchanged; no parameter search")
    return 0
