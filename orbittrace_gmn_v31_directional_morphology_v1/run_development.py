#!/usr/bin/env python3
"""Frozen GMN v31 + one cross-year directional-morphology coordinate."""
...
def residual_vector(e,c):
    ...
    req(r.shape==(3,) and np.isfinite(r).all(),'invalid residual vector')
    return r

def shape_tensor(vectors):
    R=np.asarray(vectors,dtype=float)
    req(R.ndim==2 and R.shape[0]>=1 and R.shape[1]==3 and np.isfinite(R).all(),'invalid annual residual matrix')
    M=(R.T@R)/float(R.shape[0])
    tr=float(np.trace(M))
    req(math.isfinite(tr) and tr>=0.0,'invalid second-moment trace')
    if tr==0.0:
        return np.zeros((3,3),dtype=float)
    S=M/tr
    req(S.shape==(3,3) and np.isfinite(S).all(),'invalid normalized shape tensor')
    req(np.allclose(S,S.T,rtol=0.0,atol=1e-15),'shape tensor not symmetric')
    req(abs(float(np.trace(S))-1.0)<=1e-12,'shape tensor trace changed')
    return S

def morphology_discrepancy(v0,v1):
    S0=shape_tensor(v0); S1=shape_tensor(v1)
    d=float(np.linalg.norm(S0-S1,ord='fro')/math.sqrt(2.0))
    req(math.isfinite(d) and d>=0.0 and d<=1.0+1e-12,'invalid morphology discrepancy')
    return d
...
def main():
    ...
    for f in hard:
        ...
        for year in YEARS:
            ...
            vectors=[]
            for eid in eids:
                e=lookup.get(eid); req(e is not None,f'member absent {eid}'); vectors.append(residual_vector(e,c))
            tensors.append(shape_tensor(vectors)); cc.append(len(vectors))
        d=morphology_discrepancy([residual_vector(lookup[str(e)],f['centroids'][str(YEARS[0])]) for e in f['event_ids'] if int(str(e)[:4])==YEARS[0]], [residual_vector(lookup[str(e)],f['centroids'][str(YEARS[1])]) for e in f['event_ids'] if int(str(e)[:4])==YEARS[1]])
        morph.append(d); counts.append(cc); tensor_sha_rows.append(arrsha(np.stack(tensors,axis=0)))
    mv=np.asarray(morph,float); req(mv.shape==(N,) and np.isfinite(mv).all() and np.all(mv>=0) and np.all(mv<=1.0+1e-12),'morphology vector invalid')
    X24=np.column_stack([X,mv]); cand=oof_margin(X24,folds,y,ids,hard_rank)
    ...
    pos=mv[y]; neg=mv[~y]
    result={'verdict':'PASS_GMN_V31_DIRECTIONAL_MORPHOLOGY_V1' if passed else 'FAIL_GMN_V31_DIRECTIONAL_MORPHOLOGY_V1','scientific_role':'TARGET_EXCLUDED_GMN_2022_2023_V31_SUCCESSOR_DEVELOPMENT_ONLY','first_valid_outcome_binding':True,'sole_scientific_change':'append cross-year Frobenius discrepancy of annual trace-normalized 3x3 signed physical-residual second-moment tensors to exact v31 23D representation','candidate_count':N,'feature_dimension_parent':23,'feature_dimension_candidate':24,'parent_feature_sha256':arrsha(X),'centroid_sha256':arrsha(cm),'parent_margin_sha256':arrsha(parent),'directional_morphology_sha256':arrsha(mv),'candidate_margin_sha256':arrsha(cand),'candidate_fused_order_sha256':ordersha(c_fused),'hard_control':HARD,'parent_control':PARENT,'parent_reproduced_metrics':trim(parent_metrics),'directional_morphology_local_only':trim(local_metrics),'directional_morphology_equal_rank_fusion':trim(metrics),'pass_gates':gates,'morphology_summary_all':{'min':float(mv.min()),'median':float(np.median(mv)),'max':float(mv.max())},'morphology_summary_positive':{'min':float(pos.min()),'median':float(np.median(pos)),'max':float(pos.max())},'morphology_summary_nonpositive':{'min':float(neg.min()),'median':float(np.median(neg)),'max':float(neg.max())},'per_family':[{'family_id':fid,'morphology_discrepancy':float(mv[i]),'annual_tensor_pair_sha256':tensor_sha_rows[i],'n2022':counts[i][0],'n2023':counts[i][1]} for i,fid in enumerate(ids)],'feature_search':False,'tensor_statistic_search':False,'eigenvalue_or_axis_search':False,'radial_statistic_reuse':False,'activity_statistic_reuse':False,'metric_search':False,'k_search':False,'scaling_search':False,'reference_change':False,'diversity_search':False,'fusion_search':False,'post_result_second_search':False,'blind_exclusion':[20.0,55.0],'sonotaco_2013_2014_access':False,'target_information_access':False,'target_region_events_accessed':False,'maarsy_scientific_access':False,'dms_scientific_access':False,'sonotaco_benchmark_authorized_by_this_result':bool(passed)}
    (a.output/'GMN_V31_DIRECTIONAL_MORPHOLOGY_V1_RESULT.json').write_text(json.dumps(result,indent=2,sort_keys=True,allow_nan=False)+'\n'); print(json.dumps({k:v for k,v in result.items() if k!='per_family'},indent=2,sort_keys=True,allow_nan=False)); return 0
