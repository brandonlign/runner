...
def _split_contiguous(records: list[dict[str, Any]], count: int) -> list[list[dict[str, Any]]]:
    ...
    for index in range(count):
        ...
        chunks.append(records[start:stop]); start=stop
...
def install(v6: Any, *, workers: int = 4, min_parallel_records: int = 256) -> dict[str, Any]:
    ...
    cpu_count=max(1,int(os.cpu_count() or 1)); worker_count=min(max(1,int(workers)),cpu_count,4)
    ...
    def replacement(old,records,window_events,event_lookup,support,base):
        ...
        _ORIGINAL=original; _CONTEXT=(old,window_events,event_lookup,support,base)
        try:
            ctx=mp.get_context("fork")
            with ctx.Pool(processes=len(chunks)) as pool:
                chunk_results=pool.map(_run_chunk,chunks,chunksize=1)
        finally:
            _CONTEXT=None; _ORIGINAL=None
        ...
        if [str(r["proposal_anchor_id"]) for r in merged] != [str(r["proposal_anchor_id"]) for r in records]:
    ...
    return {"workers":worker_count,"min_parallel_records":threshold,"start_method":"fork","scientific_body":"immutable original exact_rescore_window_v6 called per contiguous chunk"}
