...
def verify_zenodo_metadata()->dict[str,Any]:
    response=requests.get(ZENODO_RECORD_URL,timeout=120,headers={"Accept":"application/json","User-Agent":"OrbitTrace-P1-MAARSY-2018-2019/1.0"}); response.raise_for_status()
...
def stream_geometry(output:Path)->tuple[dict[int,list[dict[str,Any]]],dict[str,Any]]:
    ...
    with requests.get(CONTENT_URL,timeout=(60,600),stream=True,headers={"User-Agent":"OrbitTrace-P1-MAARSY-2018-2019/1.0","Accept-Encoding":"identity"}) as response:
...
def read_needed_orbits(needed_ids:set[str],output:Path)->tuple[dict[str,dict[str,float]],dict[str,Any]]:
    ...
    with requests.get(CONTENT_URL,timeout=(60,600),stream=True,headers={"User-Agent":"OrbitTrace-P1-MAARSY-2018-2019-orbit/1.0","Accept-Encoding":"identity"}) as response:
