...
def _run_chunk(chunk: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    # Deliberately call the immutable original scalar function. No scientific
    # operation is reimplemented here.
...
def install(v6: Any, *, workers: int | None = None, min_parallel_records: int = 256) -> dict[str, Any]:
    """Replace only v6.exact_rescore_window_v6 with an order-preserving executor.

    Every worker invokes the exact original scalar function on a contiguous
    subset. The original function body, distance calculations, stable-nearest
    handling, v3 scoring, fixed4 scoring, and returned records are untouched.
    Linux/fork is required so the dynamically loaded frozen modules and window
    context are inherited without serialization/reconstruction.
    """
    ...
    requested = cpu_count if workers is None else max(1, int(workers))
    worker_count = min(requested, cpu_count, 4)
    ...
    def replacement(old, records, window_events, event_lookup, support, base):
        ...
        output_ids = [str(row["proposal_anchor_id"]) for row in merged]
        if output_ids != input_ids:
            raise RuntimeError("parallel exact-rescore changed proposal order")
        print(
            f"V6_PARALLEL_EXACT workers={len(chunks)} proposals={len(records):,} "
    ...
    return {
        ...
        "min_parallel_records": threshold,
        "start_method": "fork",
        "scientific_body": "immutable original exact_rescore_window_v6 called per contiguous chunk",
    }
