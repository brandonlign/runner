...
PRE_WORKFLOW_NAME='OrbitTrace P15 matched pretruth checkpoints'
PRE_ARTIFACT_NAME='orbittrace-p15-matched-pretruth-checkpoints'
...
progress 'ONE-FILE EVALUATOR ACTIVATION / EXACT PRETRUTH BRANCH'
git fetch --no-tags origin "$BASE_SHA" "$HEAD_SHA"
mapfile -t files < <(git diff --name-only "$BASE_SHA" "$HEAD_SHA")
test "${#files[@]}" -eq 1
test "${files[0]}" = "$MARKER"
marker="$(git show "$HEAD_SHA:$MARKER")"
test "$(printf '%s\n' "$marker" | sed -n '1p')" = 'EXECUTE_P15_MATCHED_EVALUATOR_FROM_UNIQUE_PRETRUTH'
PRE_BRANCH="$(printf '%s\n' "$marker" | sed -n '2p')"
test "$PRE_BRANCH" = 'agent/orbittrace-p15-matched-pretruth-explicit-run-v2'
test "$(printf '%s\n' "$marker" | wc -l)" -eq 2

git checkout --detach "$BASE_SHA"
test ! -e "$MARKER"

progress 'PIN MERGED EVALUATOR/FIREWALL SOURCE'
...
progress 'VERIFY SOLE ADMISSIBLE ARTIFACT-ONLY P15 DEVELOPMENT PASS'
download_exact "$DEV_ARTIFACT" "$DEV_RUN" "$DEV_NAME" "$DEV_DIGEST" p15-dev
...
progress 'DETERMINISTICALLY DISCOVER UNIQUE SUCCESSFUL PRETRUTH RUN'
curl -L --fail --retry 3 -H "Authorization: Bearer $GH_TOKEN" -H 'Accept: application/vnd.github+json' \
  "https://api.github.com/repos/$REPO/actions/runs?branch=$PRE_BRANCH&event=pull_request&per_page=100" -o /tmp/p15-pre-runs.json
python - "$PRE_BRANCH" "$PRE_WORKFLOW_NAME" <<'PY_RUN'
import json,sys
branch,name=sys.argv[1:]
runs=json.load(open('/tmp/p15-pre-runs.json'))['workflow_runs']
h=[r for r in runs if r.get('head_branch')==branch and r.get('name')==name and r.get('status')=='completed' and r.get('conclusion')=='success']
assert len(h)==1,[(r.get('id'),r.get('name'),r.get('status'),r.get('conclusion'),r.get('head_sha')) for r in h]
r=h[0]
open('/tmp/p15-pre-run-id','w').write(str(r['id']))
open('/tmp/p15-pre-head-sha','w').write(str(r['head_sha']))
print('UNIQUE_PRETRUTH_RUN',r['id'],r['head_sha'])
PY_RUN
PRE_RUN="$(cat /tmp/p15-pre-run-id)"
PRE_HEAD_SHA="$(cat /tmp/p15-pre-head-sha)"

git fetch --no-tags origin "$PRE_HEAD_SHA"
marker_text="$(git show "$PRE_HEAD_SHA:orbittrace_support_safe_halo_p15/MATCHED_PRETRUTH_RUN.md)"
test "$(printf '%s\n' "$marker_text" | sed -n '1p')" = 'EXECUTE_P15_MATCHED_PRETRUTH_AFTER_ARTIFACT_ONLY_DEVELOPMENT_PASS'
test "$(printf '%s\n' "$marker_text" | sed -n '2p')" = '31329529635'
test "$(printf '%s\n' "$marker_text" | sed -n '3p')" = '9042508082'
test "$(printf '%s\n' "$marker_text" | sed -n '4p')" = "$DEV_DIGEST"
test "$(printf '%s\n' "$marker_text" | sed -n '5p')" = "$DEV_RESULT_SHA"
test "$(printf '%s\n' "$marker_text" | sed -n '6p')" = '23d309f6702ed0aa6769381963ea64701ae59c97376a0bae536b527fbc978fe6'
test "$(printf '%s\n' "$marker_text" | wc -l)" -eq 6

progress 'PIN UNIQUE PRETRUTH ARTIFACT AND BOTH CHECKPOINT HASHES'
curl -L --fail --retry 3 -H "Authorization: Bearer $GH_TOKEN" -H 'Accept: application/vnd.github+json' \
  "https://api.github.com/repos/$REPO/actions/runs/$PRE_RUN/artifacts?per_page=100" -o /tmp/p15-pre-artifacts.json
python - "$PRE_RUN" "$PRE_ARTIFACT_NAME" <<'PY_ART'
import json,sys
run,name=sys.argv[1:]
a=json.load(open('/tmp/p15-pre-artifacts.json'))['artifacts']
h=[x for x in a if x['name']==name and not x['expired']]
assert len(h)==1,[(x['id'],x['name'],x.get('digest'),x['expired']) for x in h]
x=h[0]; assert x['workflow_run']['id']==int(run),x
assert isinstance(x.get('digest'),str) and x['digest'].startswith('sha256:'),x
open('/tmp/p15-pre-artifact-id','w').write(str(x['id']))
open('/tmp/p15-pre-artifact-digest','w').write(x['digest'])
print('UNIQUE_PRETRUTH_ARTIFACT',x['id'],x['digest'])
PY_ART
PRE_ARTIFACT="$(cat /tmp/p15-pre-artifact-id)"
PRE_DIGEST="$(cat /tmp/p15-pre-artifact-digest)"
download_exact "$PRE_ARTIFACT" "$PRE_RUN" "$PRE_ARTIFACT_NAME" "$PRE_DIGEST" p15-pre
...
for panel in hdbscan sugar; do
  ...
  actual="$(sha256sum "pretruth/checkpoints/${panel}.pkl" | awk '{print $1}')"
  test "$(cat "pretruth/checkpoints/${panel}.pkl.sha256")" = "$actual"
  printf '%s_CHECKPOINT_SHA256=%s\n' "${panel^^}" "$actual"
  printf '%s' "$actual" > "/tmp/${panel}-checkpoint-sha"
done
HDB_CP_SHA="$(cat /tmp/hdbscan-checkpoint-sha)"
SUGAR_CP_SHA="$(cat /tmp/sugar-checkpoint-sha)"
...
progress 'WRITE PRETRUTH PROVENANCE BEFORE ANY TRUTH INDEXING'
mkdir -p output
python - "$PRE_RUN" "$PRE_HEAD_SHA" "$PRE_ARTIFACT" "$PRE_DIGEST" "$HDB_CP_SHA" "$SUGAR_CP_SHA" <<'PY_PROV'
import json,sys
run,head,aid,digest,hdb,sugar=sys.argv[1:]
p={
 'classification':'P15 matched pretruth provenance before truth',
 ...
 'pretruth_run':int(run),'pretruth_head_sha':head,'pretruth_artifact_id':int(aid),'pretruth_artifact_digest':digest,
 'hdbscan_checkpoint_sha256':hdb,'sugar_checkpoint_sha256':sugar,
 'competitor_cluster_values_accessed':False,'known_shower_truth_accessed':False,
 'external_data_access':False,'target_information_access':False,
}
open('output/p15_pretruth_provenance_before_truth.json','w').write(json.dumps(p,indent=2,sort_keys=True)+'\n')
print('P15_PRETRUTH_PROVENANCE_BEFORE_TRUTH',json.dumps(p,sort_keys=True))
PY_PROV

progress 'STAGE EXACT POSTFREEZE SOURCES — VALUES REMAIN UNINDEXED'
mkdir -p input/{v3,evaluator,archives} /tmp/p15-eval/{parser,mapping}
...
python - "$PRE_RUN" "$PRE_HEAD_SHA" "$PRE_ARTIFACT" "$PRE_DIGEST" "$HDB_CP_SHA" "$SUGAR_CP_SHA" <<'PY_FINAL'
...
prov=json.load(open('output/p15_pretruth_provenance_before_truth.json'))
prov.update({
 ...
 'matched_verdict':r['verdict'],'external_validation_authorized':bool(r['external_validation_authorized']),
