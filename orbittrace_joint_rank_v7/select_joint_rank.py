#!/usr/bin/env python3
"""Evaluate the preregistered OrbitTrace-v7 joint-rank sparse-rescue family."""
...
PRODUCT_BUDGETS = tuple(range(32, 257, 8))
DEVELOPMENT_YEARS = (2025, 2023, 2024, 2018)
KS = (4, 6, 8, 12)
FPR_CAP = 0.055
SECTOR_FPR_CAP = 0.08
...
FROZEN_REFERENCES: dict[int, dict[str, Any]] = {
    ...
    2018: {
        "fixed4_k4": 0.18181818181818182,
        "brown_recall": {"6": 0.3484848484848485, "8": 0.6893939393939394, "12": 0.8863636363636364},
    },
}

EXPECTED_AUC = {
    2025: {"v3": 0.8368602079503676, "brown": 0.8285055721507353},
    2023: {"v3": 0.8362627802414387, "brown": 0.8319715832101502},
    2024: {"v3": 0.8558692990511172, "brown": 0.8503142217630854},
    2018: {"v3": 0.7790392083715948, "brown": 0.7707113751147842},
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--year2018", required=True, type=Path)
...
def load_year(directory: Path, year: int) -> dict[str, Any]:
    if year == 2025:
        ...
        benchmark = json.loads((directory / "SONOTACO_2018_V6_PROSPECTIVE_RESULT.json").read_text())
        negative = read_jsonl_gz(directory / "negative_records.jsonl.gz")
        positive = read_jsonl_gz(directory / "positive_records.jsonl.gz")
        if benchmark["verdict"] != "FAIL_V6_SONOTACO_2018_PROSPECTIVE_VALIDATION":
            raise RuntimeError("2018 successor development must preserve the frozen v6 failure")
        v3_auc = float(benchmark["raw_auc"][METHOD_V3])
        brown_auc = float(benchmark["raw_auc"][METHOD_BROWN])
        getter = lambda row, method: float(row["p_v6"][method])
    else:
        raise RuntimeError(f"unsupported development year {year}")

    expected = EXPECTED_AUC[year]
    if abs(v3_auc - expected["v3"]) > 1e-12 or abs(brown_auc - expected["brown"]) > 1e-12:
...
def evaluate(payload: dict[str, Any], r_primary: int, r_fixed4: int, product_budget: int) -> dict[str, Any]:
    neg_detected = (payload["neg_v3"] <= r_primary) | (
        (payload["neg_f4"] <= r_fixed4)
        & (payload["neg_v3"].astype(np.int32) * payload["neg_f4"].astype(np.int32) <= product_budget)
    )
    pos_detected = (payload["pos_v3"] <= r_primary) | (
        (payload["pos_f4"] <= r_fixed4)
        & (payload["pos_v3"].astype(np.int32) * payload["pos_f4"].astype(np.int32) <= product_budget)
    )
    ...
    recall = {str(k): float(np.mean(pos_detected[payload["ks"] == k])) for k in KS}
...
def selection_key(row: dict[str, Any]) -> tuple[float, float, float, int, int, int]:
    return (
        -float(row["minimum_recall_margin"]),
        float(row["maximum_pooled_fpr"]),
        float(row["maximum_worst_sector_fpr"]),
        int(row["product_budget"]),
        -int(row["r_primary"]),
        int(row["r_fixed4"]),
    )
...
def main() -> None:
    ...
    payloads = {
        ...
        2018: load_year(args.year2018, 2018),
    }

    grid_path = args.output / "V7_COMPLETE_GRID.csv"
    feasible_rows: list[dict[str, Any]] = []
    grid_count = 0
    with grid_path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.writer(handle)
        header = [
            "r_primary", "r_fixed4", "product_budget", "feasible",
        ...
        for r_primary in PRIMARY_RANKS:
            for r_fixed4 in FIXED4_RANKS:
                for product_budget in PRODUCT_BUDGETS:
                    ...
                    max_sector = 0.0
                    for year in DEVELOPMENT_YEARS:
                        measured = evaluate(payloads[year], r_primary, r_fixed4, product_budget)
                        per_year[year] = measured
                        feasible = feasible and bool(measured["feasible"])
                        margins.extend(float(value) for value in measured["recall_margins"].values())
                        max_fpr = max(max_fpr, float(measured["pooled_fpr"]))
                        max_sector = max(max_sector, float(measured["worst_sector_fpr"]))
                    row = {
                        "r_primary": r_primary,
                        "r_fixed4": r_fixed4,
                        "product_budget": product_budget,
                        "feasible": bool(feasible),
                        "minimum_recall_margin": min(margins),
                        "maximum_pooled_fpr": max_fpr,
                        "maximum_worst_sector_fpr": max_sector,
                        "years": per_year,
                    }
                    if feasible:
                        feasible_rows.append(row)
                    out = [
                        r_primary, r_fixed4, product_budget, int(feasible),
                        row["minimum_recall_margin"], max_fpr, max_sector,
                    ]
                    for year in DEVELOPMENT_YEARS:
    ...
    expected_grid = len(PRIMARY_RANKS) * len(FIXED4_RANKS) * len(PRODUCT_BUDGETS)
    if grid_count != expected_grid or expected_grid != 19604:
    ...
    verdict = "PASS_V7_JOINT_RANK_DEVELOPMENT" if selected is not None else "FAIL_V7_JOINT_RANK_DEVELOPMENT"
    result = {
        "verdict": verdict,
        "configuration": {
            ...
            "product_budgets": [min(PRODUCT_BUDGETS), max(PRODUCT_BUDGETS), 8],
            "grid_rules": grid_count,
            "fpr_cap": FPR_CAP,
            "sector_fpr_cap": SECTOR_FPR_CAP,
            "recall_tolerance": RECALL_TOLERANCE,
            "rule": "(rank_v3 <= r_primary) OR ((rank_fixed4 <= r_fixed4) AND (rank_v3 * rank_fixed4 <= B))",
            "selector": "maximin recall margin; minimum worst-year pooled FPR; minimum worst-sector FPR; smallest product budget; largest primary rank; smallest fixed4 rank",
        },
        "frozen_references": FROZEN_REFERENCES,
        "continuous_auc_guard": EXPECTED_AUC,
        "feasible_rules": len(feasible_rows),
        "selected": selected,
        "prospective_reservation": "SonotaCo 2017; no performance access before a passing development freeze",
    }
    (args.output / "V7_DEVELOPMENT_SELECTION.json").write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")

    lines = [
        "# OrbitTrace v7 joint-rank rescue development",
        "",
        f"Verdict: **`{verdict}`**",
        "",
    ...
    if selected is not None:
        lines.extend([
            f"Selected ranks: **primary={selected['r_primary']}/513, fixed4={selected['r_fixed4']}/513, product budget={selected['product_budget']}**",
            "",
            f"Minimum recall margin: **{selected['minimum_recall_margin']:.6f}**",
            f"Maximum pooled FPR: **{selected['maximum_pooled_fpr']:.6f}**",
            "",
        ])
        for year in DEVELOPMENT_YEARS:
        ...
        lines.extend([
            "No rule in the complete preregistered family satisfies all frozen cross-year gates.",
            "",
            "This is a preserved no-go for a unified joint-rank rescue; it does not invalidate the separately validated v3 ranking or fixed4 sparse channel.",
            "",
        ])
    lines.extend([
        "The complete 19,604-rule grid is preserved in `V7_COMPLETE_GRID.csv`.",
        "",
        "SonotaCo 2017 remains untouched and reserved; a failed development result does not authorize access.",
    ])
    (args.output / "V7_DEVELOPMENT_SELECTION.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    print("\n".join(lines))
