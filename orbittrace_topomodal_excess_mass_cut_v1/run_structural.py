...
def cross_scale(coarse:list[frozenset[str]],fine:list[frozenset[str]],fine_universe:frozenset[str])->dict[str,Any]:
    ...
    scores=[max((jaccard(f,c) for c in restricted),default=0.0) for f in fine]
...
def excess_cut(structural:Any,events:list[dict[str,Any]])->tuple[list[dict[str,Any]],dict[str,Any]]:
    ...
    ds=diagram_sorted(np.asarray(model.diagram_,dtype=float)); req(len(ds)==len(children),'diagram/merge count')
    N=L+len(children); members:[Any]=[None]*N; member_ix:[Any]=[None]*N; parent=np.full(N,-1,dtype=np.int64); kids:[Any]=[None]*N; active_peak=np.full(N,np.nan); active_key:[Any]=[None]*N; merge_level=np.full(N,np.nan)
    for leaf in range(L):
        ix=np.flatnonzero(labels==leaf); req(len(ix)>0,'empty leaf'); member_ix[leaf]=frozenset(int(i) for i in ix); members[leaf]=frozenset(ids[int(i)] for i in ix); pk=float(np.max(rho[ix])); active_peak[leaf]=pk; active_key[leaf]=min(ids[int(i)] for i in ix if float(rho[int(i)])==pk)
    for off,pair in enumerate(children):
        node=L+off; a,b=int(pair[0]),int(pair[1]); req(0<=a<node and 0<=b<node and parent[a]==-1 and parent[b]==-1,'hierarchy'); ia,ib=member_ix[a],member_ix[b]; ma,mb=members[a],members[b]; req(ia is not None and ib is not None and ia.isdisjoint(ib),'child ix membership'); req(ma is not None and mb is not None and ma.isdisjoint(mb),'child ID membership')
        pa,pb=float(active_peak[a]),float(active_peak[b]); ka,kb=str(active_key[a]),str(active_key[b]); winner=(a if (pa>pb or (pa==pb and ka<kb)) else b)
        member_ix[node]=frozenset(ia|ib); members[node]=frozenset(ma|mb); kids[node]=(a,b); parent[a]=node; parent[b]=node; active_peak[node]=active_peak[winner]; active_key[node]=active_key[winner]
    # Exact node-specific merge level from the vertex-weighted radius graph. In a superlevel-set
    # graph the two child components first connect at max_{crossing edge} min(rho_u,rho_v).
    reconstructed=[]
    for off,pair in enumerate(children):
        node=L+off; a,b=int(pair[0]),int(pair[1]); ia=set(member_ix[a]); ib=set(member_ix[b]); smaller,other=(ia,ib) if len(ia)<=len(ib) else (ib,ia); saddle=-math.inf
        for i in smaller:
            for j in neigh[i]:
                if j in other:saddle=max(saddle,min(float(rho[i]),float(rho[j])))
        req(math.isfinite(saddle),'children lack crossing graph edge'); merge_level[node]=float(saddle)
        pa,pb=float(active_peak[a]),float(active_peak[b]); ka,kb=str(active_key[a]),str(active_key[b]); loser=(b if (pa>pb or (pa==pb and ka<kb)) else a); birth=float(active_peak[loser]); req(birth+1e-12>=saddle,f'saddle above dying peak {node}: {saddle}>{birth}'); reconstructed.append([birth,float(saddle)])
    roots=np.flatnonzero(parent==-1); req(len(roots)==roots_expected and sum(len(members[int(r)]) for r in roots)==n,'roots')
    rec=diagram_sorted(np.asarray(reconstructed,dtype=float)); req(rec.shape==ds.shape,'diagram shape'); req(np.allclose(rec,ds,rtol=0,atol=1e-12),f'diagram reconstruction mismatch {float(np.max(np.abs(rec-ds))) if rec.size else 0.0}'); diagerr=float(np.max(np.abs(rec-ds))) if rec.size else 0.0
    ...
    for node in range(N):
        m=members[node]; ix=member_ix[node]; req(m is not None and ix is not None,'missing node'); p=int(parent[node]); lo=0.0 if p==-1 else float(merge_level[p]); up=float(active_peak[node]) if kids[node] is None else float(merge_level[node]); req(math.isfinite(lo) and math.isfinite(up) and up+1e-12>=lo,f'lifetime {node}: {lo} {up}'); lo=max(0.0,lo); up=max(lo,up); lower[node]=lo; upper[node]=up; arr=np.asarray(sorted(ix),dtype=np.int64); stability[node]=float(np.sum(np.maximum(0.0,np.minimum(rho[arr],up)-lo))); req(math.isfinite(stability[node]) and stability[node]>=0,'stability')
    ...
    def solve(node:int)->tuple[float,tuple[int,...]]:
        ...
        ch=kids[node]; child_score=0.0; child_nodes=[]
        if ch is not None:
            for c in ch:
                sc,sn=solve(int(c)); child_score+=sc; child_nodes.extend(sn)
        reportable=len(members[node])>=MIN_SUPPORT; self_score=float(stability[node]) if reportable else -1.0
        if reportable and self_score>=child_score:best_score[node]=self_score; selected[node]=(node,)
        else:best_score[node]=child_score; selected[node]=tuple(child_nodes)
