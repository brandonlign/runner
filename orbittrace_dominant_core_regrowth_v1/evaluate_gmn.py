...
DCR_SCHEMA = "ORBITTRACE_DOMINANT_CORE_REGROWTH_V1_PRETRUTH"
DCR_ROLE = "TARGET_EXCLUDED_GMN_DCR_V1_DEVELOPMENT_RANKING"
...
def renamed_comparison(row: dict[str, Any]) -> dict[str, Any]:
    out = dict(row)
    out["dcr"] = out.pop("bwm")
    out["dcr_available_candidates"] = out.pop("bwm_available_candidates")
    out["dcr_capacity_shortfall"] = out.pop("bwm_capacity_shortfall")
    out["dcr_evaluated_candidates"] = out.pop("bwm_evaluated_candidates")
    return out
...
def main() -> int:
    ...
    req(sha(a.dcr_pretruth) == a.expected_pretruth_sha, "sealed DCR ranking changed")
    req(sha(a.frozen_bwm_evaluator) == BWM_EVALUATOR_SHA, "frozen evaluator changed")
    pre = json.loads(a.dcr_pretruth.read_text())
    req(pre.get("schema") == DCR_SCHEMA and pre.get("scientific_role") == DCR_ROLE, "wrong DCR identity")
    req(pre.get("structural_pass") is True, "DCR structural gate failed")
    req(pre.get("configuration", {}).get("dominance_rule") == "2 * seed_member_count > parent_member_count", "DCR selector changed")
    req(pre.get("configuration", {}).get("new_tuned_parameters") == [], "unexpected DCR tuned parameters")
    req(pre.get("configuration", {}).get("one_candidate_per_parent") is True, "DCR candidate cardinality rule changed")
    req(
    ...
    for subset in compat["subsets"]:
        subset["bwm_candidates"] = subset["dcr_candidates"]
    ss = pre["size_summary"]
    compat["size_summary"] = {
        "bwm_max_top_budget_member_count": ss["dcr_max_top_budget_member_count"],
        "bwm_mean_top_budget_member_count": ss["dcr_mean_top_budget_member_count"],
        "bwm_p90_top_budget_member_count": ss["dcr_p90_top_budget_member_count"],
        "bwm_size_biased_top_budget_member_burden": ss["dcr_size_biased_top_budget_member_burden"],
    ...
    with tempfile.TemporaryDirectory(prefix="dcr-eval-") as td:
        td_path = Path(td)
        compat_path = td_path / "BWM_COMPAT_FROM_DCR.json"
    ...
    for name, rr in raw["routes"].items():
        ...
        cap["dcr_shortfall_panels"] = cap.pop("bwm_shortfall_panels")
        cap["dcr_total_shortfall"] = cap.pop("bwm_total_shortfall")
        routes[name] = {"dcr": rr["bwm"], "support_pruned": rr["support_pruned"], "literature": rr["literature"], "capacity": cap}
    scales = {name: {"dcr": rr["bwm"], "support_pruned": rr["support_pruned"]} for name, rr in raw["scales"].items()}
    comparisons = [renamed_comparison(row) for row in raw["comparisons"]]
    gates = dict(raw["gates"])
    verdict = "PASS_DOMINANT_CORE_REGROWTH_V1_GMN_DEVELOPMENT" if all(gates.values()) else "FAIL_DOMINANT_CORE_REGROWTH_V1_GMN_DEVELOPMENT"
    out = {
        "schema": "ORBITTRACE_DOMINANT_CORE_REGROWTH_V1_GMN_RESULT",
        "scientific_role": "TARGET_EXCLUDED_GMN_DCR_V1_DEVELOPMENT_RESULT",
        "development_status": pre["development_status"],
        "verdict": verdict,
        "dcr_pretruth_sha256": sha(a.dcr_pretruth),
        ...
        "interpretation_boundary": "DCR v1 was designed after BWM, CMR, and FOCR GMN development results. This GMN score is development evidence only. A pass requires frozen non-GMN transfer before generalization or OrbitTrace characterization.",
    }
