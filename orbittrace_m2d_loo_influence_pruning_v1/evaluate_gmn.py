...
LOIP_SCHEMA = "ORBITTRACE_M2D_LOO_INFLUENCE_PRUNING_V1_PRETRUTH"
LOIP_ROLE = "TARGET_EXCLUDED_GMN_M2D_LOIP_V1_DEVELOPMENT_RANKING"
...
def renamed_comparison(row: dict[str, Any]) -> dict[str, Any]:
    out = dict(row)
    out["loip"] = out.pop("bwm")
    out["loip_available_candidates"] = out.pop("bwm_available_candidates")
    out["loip_capacity_shortfall"] = out.pop("bwm_capacity_shortfall")
    out["loip_evaluated_candidates"] = out.pop("bwm_evaluated_candidates")
    return out


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--loip-pretruth", type=Path, required=True)
    ...
    req(sha(a.loip_pretruth) == a.expected_pretruth_sha, "sealed LOIP ranking changed")
    ...
    req(pre.get("configuration", {}).get("one_candidate_per_parent") is True, "LOIP cardinality changed")
    req(pre.get("mechanism_summary", {}).get("ranking_changed_from_support_pruned") is False, "LOIP parent order changed")
    req(
    ...
    for subset in compat["subsets"]:
        subset["bwm_candidates"] = subset["loip_candidates"]
    ss = pre["size_summary"]
    compat["size_summary"] = {
        "bwm_max_top_budget_member_count": ss["loip_max_top_budget_member_count"],
        "bwm_mean_top_budget_member_count": ss["loip_mean_top_budget_member_count"],
        "bwm_p90_top_budget_member_count": ss["loip_p90_top_budget_member_count"],
        "bwm_size_biased_top_budget_member_burden": ss["loip_size_biased_top_budget_member_burden"],
    ...
    with tempfile.TemporaryDirectory(prefix="loip-eval-") as td:
        td_path = Path(td)
        compat_path = td_path / "BWM_COMPAT_FROM_LOIP.json"
    ...
    for name, rr in raw["routes"].items():
        ...
        cap["loip_shortfall_panels"] = cap.pop("bwm_shortfall_panels")
        cap["loip_total_shortfall"] = cap.pop("bwm_total_shortfall")
        routes[name] = {
            "loip": rr["bwm"],
    ...
    scales = {
        name: {"loip": rr["bwm"], "support_pruned": rr["support_pruned"]}
        for name, rr in raw["scales"].items()
    }
    comparisons = [renamed_comparison(row) for row in raw["comparisons"]]

    inherited_gates = dict(raw["gates"])
    extraction_gates = {
        "sugar_macro_precision_strictly_higher_than_support_pruned": routes["sugar2017"]["loip"]["macro_precision"] > routes["sugar2017"]["support_pruned"]["macro_precision"],
        "hdb_macro_precision_strictly_higher_than_support_pruned": routes["hdbscan2025"]["loip"]["macro_precision"] > routes["hdbscan2025"]["support_pruned"]["macro_precision"],
    }
    gates = {**inherited_gates, **extraction_gates}
    verdict = "PASS_M2D_LOO_INFLUENCE_PRUNING_V1_GMN_DEVELOPMENT" if all(gates.values()) else "FAIL_M2D_LOO_INFLUENCE_PRUNING_V1_GMN_DEVELOPMENT"

    out = {
        "schema": "ORBITTRACE_M2D_LOO_INFLUENCE_PRUNING_V1_GMN_RESULT",
        "scientific_role": "TARGET_EXCLUDED_GMN_M2D_LOIP_V1_DEVELOPMENT_RESULT",
        "development_status": pre["development_status"],
        "verdict": verdict,
        "loip_pretruth_sha256": sha(a.loip_pretruth),
        ...
        "interpretation_boundary": "LOIP v1 is exposed target-excluded GMN development evidence. Promotion requires every inherited support-pruned/literature gate plus strictly higher Sugar and HDB macro precision. Any pass must transfer frozen to the designated non-GMN validation stage before full-GMN OrbitTrace characterization.",
    }
