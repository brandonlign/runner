#!/usr/bin/env python3
"""Run the single frozen OrbitTrace-v5 prospective validation on SonotaCo 2024.

This runner is intentionally narrow. It evaluates only the frozen fixed4 sparse
score, the frozen Brown-family episode wavelet, and the frozen OrbitTrace v3
multi-anchor ranking. V5 uses all 512 calibration nulls per supported bin while
predecessor reference p-values use the deterministic first-128 prefix.
"""
...
def main() -> None:
    ...
    if eligibility.get("verdict") != "PASS_SONOTACO_2024_V5_ELIGIBILITY_AUDIT":
        raise RuntimeError("invalid 2024 eligibility freeze verdict")
    ...
    if constants != {
        ...
        "v3_max_rank": 20,
        "fixed4_max_rank": 10,
        "decision_rule": "(p_v3 <= 20/513) OR (p_fixed4 <= 10/513)",
    }:
        raise RuntimeError(f"eligibility constants mismatch: {constants}")

    base = load_module("v5_2024_base", decode_file(args.baseline_payload), BASELINE_SOURCE_SHA256)
    scorer = load_module(
        "v5_2024_scorer",
        decode_parts(args.scorer_parts, ["part00.b64", "part01.b64", "part02.b64", "part03.b64"]),
        SCORER_SOURCE_SHA256,
    )
    candidate = load_module("v5_2024_candidate", decode_file(args.candidate_payload), CANDIDATE_SOURCE_SHA256)
    ...
    if confirmation_hash != eligibility["parser"]["sonotaco_2024_confirmation_source_sha256"]:
        raise RuntimeError(f"2024 parser hash mismatch: {confirmation_hash}")
    confirmation = load_module("v5_2024_confirmation", confirmation_payload)
    parse_2024 = getattr(confirmation, "parse_sonotaco_2024_events")
    labeled, sporadic, parser_audit = parse_2024(args.archive, args.audit, base)
    if not all(bool(value) for value in parser_audit["gates"].values()):
        raise RuntimeError(f"2024 parser audit failed: {parser_audit['gates']}")
    ...
    for row in negative_rows:
        ...
        row["v5_detected"] = bool(decision.detected(row["p_v5"][METHOD_V3], row["p_v5"][METHOD_FIXED4]))
    ...
    v5_grid_exact = all(
        on_grid(row["p_v5"][method], V5_DENOMINATOR)
    ...
    v5_fpr = rate(row["v5_detected"] for row in negative_rows)
    sector_fpr = {
        str(sector): rate(
            row["v5_detected"]
            for row in negative_rows
    ...
    v5_recall = {
        str(k): rate(row["v5_detected"] for row in positive_rows if int(row["k"]) == int(k))
        for k in scorer.ALL_K
    }
    ...
    exact_decision_rule = (
        ...
        and decision.V3_MAX_RANK == 20
        and decision.FIXED4_MAX_RANK == 10
    ...
    gates = {
        ...
        "v5_pvalues_exact_denominator_513_grid": v5_grid_exact,
        ...
        "v5_pooled_fpr_at_most_0055": v5_fpr <= FPR_CAP + 1e-15,
        "v5_worst_sector_fpr_at_most_008": worst_sector_fpr <= SECTOR_FPR_CAP + 1e-15,
        "v5_k4_recall_at_least_predecessor_fixed4": v5_recall["4"] + 1e-15 >= fixed4_reference_recall["4"],
        "v5_k6_within_003_of_predecessor_brown": v5_recall["6"] + 1e-15 >= brown_reference_recall["6"] - RECALL_TOLERANCE,
        "v5_k8_within_003_of_predecessor_brown": v5_recall["8"] + 1e-15 >= brown_reference_recall["8"] - RECALL_TOLERANCE,
        "v5_k12_within_003_of_predecessor_brown": v5_recall["12"] + 1e-15 >= brown_reference_recall["12"] - RECALL_TOLERANCE,
        "v5_decision_rule_exact_20_10_over_513": exact_decision_rule,
    }
    verdict = (
        "PASS_V5_SONOTACO_2024_PROSPECTIVE_VALIDATION"
        if all(gates.values())
        else "FAIL_V5_SONOTACO_2024_PROSPECTIVE_VALIDATION"
    )

    result = {
        "verdict": verdict,
        "classification": "single preregistered prospective validation of frozen OrbitTrace-v5 sparse-episode detector",
        "configuration": {
            "year": YEAR,
            "corpus": CORPUS,
            ...
            "calibration_per_bin_v5": CALIBRATION_512,
            "calibration_denominator_v5": V5_DENOMINATOR,
        ...
        "gates": gates,
        "claim_boundary": (
            "A pass promotes v5 as a prospectively validated sparse-episode detector only; "
            "blind catalogue rediscovery and OrbitTrace target recovery remain separate tests."
        ),
    }

    (args.output / "SONOTACO_2024_V5_PROSPECTIVE_RESULT.json").write_text(
    ...
    lines = [
        "# OrbitTrace v5 SonotaCo 2024 prospective validation",
        "",
        f"Verdict: **`{verdict}`**",
        "",
        "## Continuous ranking",
        ...
        "## Frozen v5 decision",
        "",
        f"- pooled FPR: **{v5_fpr:.6f}**",
        f"- worst-sector FPR: **{worst_sector_fpr:.6f}**",
        "- recall k=4/6/8/12: **" + " / ".join(f"{v5_recall[str(k)]:.6f}" for k in scorer.ALL_K) + "**",
        "",
    ...
    lines.extend([
        "",
        "This is the one preregistered SonotaCo 2024 prospective execution. No same-corpus retuning is authorized.",
        "",
    ...
    (args.output / "SONOTACO_2024_V5_PROSPECTIVE_RESULT.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
