#!/usr/bin/env python3
"""Audit the SonotaCo-2024 v5 episode universe without detector scoring."""
...
def main() -> None:
    ...
    base = load_module(
        "sonotaco_2024_eligibility_base",
        decode_file(args.baseline_payload),
        BASELINE_SOURCE_SHA256,
    )
    scorer = load_module(
        "sonotaco_2024_eligibility_scorer",
    ...
    confirmation = load_module("sonotaco_2024_confirmation", confirmation_payload)
    parser = getattr(confirmation, "parse_sonotaco_2024_events")
    ...
    gates = {
        ...
        "v5_denominator_exact_513": CALIBRATION_DENOMINATOR == 513,
        "v5_threshold_ranks_frozen": V3_MAX_RANK == 20 and FIXED4_MAX_RANK == 10,
        "no_detector_score_computed": True,
    }
    verdict = (
        "PASS_SONOTACO_2024_V5_ELIGIBILITY_AUDIT"
        if all(gates.values())
        else "FAIL_SONOTACO_2024_V5_ELIGIBILITY_AUDIT"
    )
    result: dict[str, Any] = {
        "verdict": verdict,
        "classification": (
            "label mapping, episode eligibility, and null-window support audit after v5 development freeze; "
            "no fixed4 score, wavelet coefficient, v3 coefficient, v5 decision, AUROC, recall, or FPR computed"
        ),
        "year": YEAR,
        "corpus": CORPUS,
    ...
    (args.output / "sonotaco_2024_v5_eligibility_audit.json").write_text(
