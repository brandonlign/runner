...
DENOMS = (128, 1024)
BUCKETS = (0, 1, 2, 3)
SEALED_PRETRUTH_SHA = "57a6fd0fa680fb56b3d6a8a984682213e0235baadf14b27f241927b2dbb4b50f"
...
CAPACITY_SEMANTICS = "PR1377_EXACT: comparator_capacity_k=len(published_clusters); method=method_candidates[:k]; shortfall allowed and recorded, never padded"
...
def main() -> int:
    ...
    req(sha(a.refined_pretruth) == SEALED_PRETRUTH_SHA, "sealed refined pretruth changed")
    ...
    req(pre.get("configuration", {}).get("new_tuned_parameters") == [], "post-truth method parameter appeared")
    ...
    req(all(len(s["refined_candidates"]) >= int(s["equal_budget_k"]) and len(s["baseline_candidates"]) >= int(s["equal_budget_k"]) for s in subsets.values()), "frozen baseline M2D budget capacity gate failed")

    q = load(a.quality_source, "support_pruned_capacity_repair_q")
    ...
    support.CORPUS = "orbittrace-m2d-support-pruned-cut-v1-gmn-truth-capacity-repair"
        ...
        for b in BUCKETS:
            ...
            for y in YEARS:
                ...
                for comp in ("sugar2017", "hdbscan2025"):
                    ...
                    k = len(comparator)
                    # Exact PR #1377 fairness semantics: request comparator capacity k,
                    # take method[:k], and if the method has fewer candidates simply
                    # evaluate all available candidates. Never pad, fabricate, or fail.
                    rcands = refined[:k]
                    bcands = baseline[:k]
                    rm = evaluate(rcands, hidden, annual)
                    ...
                    comparisons.append({
                        ...
                        "refined_available_candidates": len(refined),
                        "baseline_available_candidates": len(baseline),
                        "refined_capacity_shortfall": max(0, k - len(refined)),
                        "baseline_capacity_shortfall": max(0, k - len(baseline)),
                        "refined_evaluated_candidates": len(rcands),
                        "baseline_evaluated_candidates": len(bcands),
                        "refined": rm,
                        "baseline_m2d": bm,
                        "literature": cm,
                    })
    ...
    for comp in ("sugar2017", "hdbscan2025"):
        ...
        routes[comp] = {
            "refined": agg(rows, "refined"),
            "baseline_m2d": agg(rows, "baseline_m2d"),
            "literature": agg(rows, "literature"),
            "capacity": {
                "refined_shortfall_panels": sum(int(r["refined_capacity_shortfall"] > 0) for r in rows),
                "baseline_shortfall_panels": sum(int(r["baseline_capacity_shortfall"] > 0) for r in rows),
                "refined_total_shortfall": sum(int(r["refined_capacity_shortfall"]) for r in rows),
                "baseline_total_shortfall": sum(int(r["baseline_capacity_shortfall"]) for r in rows),
            },
        }
    ...
    result = {
        "schema": "ORBITTRACE_M2D_SUPPORT_PRUNED_CUT_V1_GMN_CAPACITY_REPAIR_RESULT",
        "scientific_role": "TARGET_EXCLUDED_GMN_SUPPORT_PRUNED_M2D_DEVELOPMENT_EVALUATOR_REPAIR_ONLY",
        "verdict": verdict,
        ...
        "repair_scope": "evaluator-only; removes erroneous capacity assertion and reproduces PR1377 truncation/shortfall semantics; no method or pretruth reexecution",
        "routes": routes,
        "scales": scales,
        "size_summary": size,
        "gates": gates,
        "comparisons": comparisons,
        "scientific_pretruth_reexecuted": False,
    ...
    print(json.dumps({"verdict": verdict, "capacity_semantics": CAPACITY_SEMANTICS, "routes": routes, "scales": scales, "size_summary": size, "gates": gates, "result_sha256": sha(a.output)}, indent=2, sort_keys=True), flush=True)
    return 0
