...
The complete candidate ranking must come only from GitHub Actions run `32312758548`, the audit-only r2 repair frozen in `SONOTACO_RANK_AUDIT_REPAIR_R2.md`. That run derives its executable from exact original ranking source Git blob `5cdf7e8b7d202a6335d832b89a8f81c7ff781d57` by replacing only the verification-only brute-audit index set; all candidate memberships, exact C++ M2D scores, ranking rules and tiebreaks are unchanged. It uses support-pretruth SHA-256 `6ae27f985340eaa41870ab4c4f8cd15d6a1cd97e03ef828254f4c24d7896176a` with truth absent. The ranked artifact must contain exactly 907 candidates and carry its own SHA-256 seal. If that ranking run does not finish successfully, this evaluation is not authorized.
...
- sealed exact M2D ranking: artifact `orbittrace-m2d-support-pruned-cut-v1-sonotaco-ranked-pretruth-r2`, run `32312758548`; its exact file SHA must be read from and checked against the artifact's `RANKED_PRETRUTH_SHA256.txt` before truth access;
...
2. Wait for and verify the exact audit-repaired rank run and its frozen source/head identity.
3. Download rows, support pretruth, and sealed ranked pretruth only.
4. Verify all pretruth identities, candidate count, rank sequence, truth flags, and exact ranked SHA while `input/truth` does not exist.
5. Only after those checks pass, download the immutable truth artifact.
6. Run `evaluate_sonotaco_ranked.py` exactly once.
7. Preserve the resulting PASS/FAIL artifact and provenance.
