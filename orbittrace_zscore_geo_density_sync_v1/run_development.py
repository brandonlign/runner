...
def candidates_from_sync(
    ...
    for lab, node in enumerate(selected_nodes):
        ...
        out.append({
            "family_id": parent.member_hash("ZSGEO-DSEOM1", members),
            "node_id": int(node),
...
def main() -> int:
    ...
    qmod = parent.load_module(a.quality_source, "zscore_geo_dseom_frozen_gmn_utility")
    ...
    support.CORPUS = "orbittrace-zscore-geo-density-sync-v1-development-2022-2023-target-excluded"
    ...
    # Sole scientific transformation: pooled target-excluded, unlabeled component-wise population Z-score.
    mu = np.mean(X, axis=0, dtype=np.float64)
    sigma = np.std(X, axis=0, ddof=0, dtype=np.float64)
    req(np.all(np.isfinite(mu)), "non-finite GEO means")
    req(np.all(np.isfinite(sigma)) and np.all(sigma > 0.0), "invalid GEO standard deviations")
    Z = (X - mu) / sigma
    zmean = np.mean(Z, axis=0, dtype=np.float64)
    zstd = np.std(Z, axis=0, ddof=0, dtype=np.float64)
    req(np.all(np.abs(zmean) < 1e-12), f"standardized mean audit failed: {zmean.tolist()}")
    req(np.all(np.abs(zstd - 1.0) < 1e-12), f"standardized std audit failed: {zstd.tolist()}")
    ...
    req(tree_sha(tree) == frozen_tree_sha, "density-sync kernel mutated standardized condensed tree")
    ...
    prelabel = {
        "scientific_role": "PRELABEL_FROZEN_ZSCORE_GEO_DENSITY_SYNC_V1",
        "representation": "POOLED_TARGET_EXCLUDED_ZSCORE_GEO6",
        "normalization": "componentwise_population_zscore_ddof0",
        "normalization_scope": "accessible_target_excluded_GMN_2022_2023_pooled",
        "transfer_rule": "recompute_unlabeled_componentwise_zscore_on_each_accessible_survey_corpus",
        "geo6_means": [float(x) for x in mu],
        "geo6_population_stds": [float(x) for x in sigma],
        "standardized_means_audit": [float(x) for x in zmean],
        "standardized_population_stds_audit": [float(x) for x in zstd],
    ...
    prelabel_path = a.output / "ZSCORE_GEO_DENSITY_SYNC_V1_PRELABEL.json"
    prelabel_path.write_text(json.dumps(prelabel, indent=2, sort_keys=True, allow_nan=False) + "\n")
    prelabel_sha = sha(prelabel_path)

    # Known-shower truth is indexed only after the exact normalized hierarchy and ordered memberships are durable.
    ...
    verdict = "PASS_ZSCORE_GEO_DENSITY_SYNC_V1_GMN_DEVELOPMENT" if passed else "FAIL_ZSCORE_GEO_DENSITY_SYNC_V1_GMN_DEVELOPMENT"
    ...
    (a.output / "ZSCORE_GEO_DENSITY_SYNC_V1_GMN_DEVELOPMENT.json").write_text(
