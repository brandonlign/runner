...
def main() -> int:
    ...
    req(pre["schema"] == "ORBITTRACE_TOPOMODAL_MAP_EQUATION_RANKING_V1_PRELABEL", "wrong prelabel schema")
    req(pre["scientific_role"] == "PRELABEL_TOPOMODAL_MAP_EQUATION_RANKING_V1", "wrong prelabel role")
    ...
    req(pre["configuration"]["score"] == "binary_two_level_map_equation_compression_gain_L1_minus_L2", "score changed after freeze")
    req(pre["configuration"]["ranking"] == "compression_gain_desc_then_family_hash_asc", "ranking changed after freeze")
    req(pre["configuration"]["teleportation"] is False, "teleportation introduced")
    ...
    req(len(pre["subsets"]) == 8, "wrong prelabel subset count")
    ...
    for row in pre["subsets"]:
        topo = row["topomodal_candidates"]
        recurrent = row["recurrent_candidates"]
        req(len(topo) >= len(recurrent), "topomodal list shorter than equal budget")
        req([int(x["rank"]) for x in topo] == list(range(1, len(topo) + 1)), "topomodal rank discontinuity")
        req([int(x["rank"]) for x in recurrent] == list(range(1, len(recurrent) + 1)), "recurrent rank discontinuity")
        expected_order = sorted(topo, key=lambda r: (-float(r["compression_gain"]), str(r["family_hash"])))
        req([str(x["family_id"]) for x in expected_order] == [str(x["family_id"]) for x in topo], "prelabel map rank order invalid")

    parent_runner = load_module(a.parent_runner, "topomodal_map_eval_parent")
    ...
    qmod = load_module(a.quality_source, "topomodal_map_eval_gmn_utility")
    ...
    support.CORPUS = "orbittrace-topomodal-map-equation-ranking-v1-target-excluded-evaluator"
    ...
    for denominator in (COARSE_D, FINE_D):
        for bucket in BUCKETS:
            ...
            topo = list(frozen["topomodal_candidates"])
            K = len(recurrent)
            topo_equal = topo[:K]
            req(len(topo_equal) == K, "equal-budget truncation failed")
            for year in YEARS:
                ...
                panels.append(
                    {
                        ...
                        "parent": parent_m,
                        "topomodal_map_equal_budget": succ_m,
                        "topomodal_map_full_diagnostic": full_m,
    ...
    for denominator in (COARSE_D, FINE_D):
        ...
        req(len(ps) == 8, f"wrong truth panel count d={denominator}")
        parent_agg = aggregate(ps, "parent")
        succ_agg = aggregate(ps, "topomodal_map_equal_budget")
        ...
        scale_results[str(denominator)] = {
            "parent": parent_agg,
            "topomodal_map_equal_budget": succ_agg,
    ...
    fine_s = scale_results[str(FINE_D)]["topomodal_map_equal_budget"]
    coarse_p = scale_results[str(COARSE_D)]["parent"]
    coarse_s = scale_results[str(COARSE_D)]["topomodal_map_equal_budget"]
    gates = {
    ...
    verdict = "PASS_TOPOMODAL_MAP_EQUATION_RANKING_V1" if all(gates.values()) else "FAIL_TOPOMODAL_MAP_EQUATION_RANKING_V1"
    result = {
        "schema": "ORBITTRACE_TOPOMODAL_MAP_EQUATION_RANKING_V1",
    ...
    out = a.output / "TOPOMODAL_MAP_EQUATION_RANKING_V1.json"
