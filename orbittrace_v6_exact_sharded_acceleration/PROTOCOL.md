...
2. Run the original repaired `scan_year_v6` prefix independently for 2022 and 2023 and stop at the first call to the original `exact_rescore_window_v6`, after the frozen code has completed all proposal windows and global proposal-anchor deduplication. Source inspection after the first technical activation showed this frozen scan does not expose its proposal stage through a hookable `proposal_window_v6` function. Therefore proposal-window outputs are **not** intercepted or altered; the unchanged proposal stage is allowed to recompute during year replay. This adds roughly the original proposal-stage cost but avoids any invasive rewrite of frozen proposal logic.
...
5. Replay each year through the original repaired `scan_year_v6`. Exact calibration outputs captured at the exact-rescore boundary may be replayed after input hashes match. The unchanged proposal stage recomputes normally. Exact-rescore outputs from step 4 are replayed only after proposal-anchor order and exact window-event IDs match. All post-exact p-values, detection logic, anchor caps, and component construction execute inside the original repaired function.
...
Every serialized stage has a SHA-256 sidecar. Catalogue rows, calibration rows, source identities, exact proposal-anchor order, exact window event IDs, shard coverage, and year replay order are checked before reuse. If a future source exposes a uniquely identifiable proposal function and proposal outputs are captured, their call fingerprints must also match before replay. Any mismatch is a technical failure and produces no scientific verdict.
