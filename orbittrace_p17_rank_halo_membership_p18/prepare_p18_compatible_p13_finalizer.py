...
def main()->int:
    ...
    text=replace_once(text,OLD_CP,NEW_CP,'checkpoint membership semantics')
    text=replace_once(text,OLD_Q,NEW_Q,'evaluated membership semantics')

    restored=text.replace(NEW_Q,OLD_Q,1).replace(NEW_CP,OLD_CP,1).replace(TECH_TRANSPORT,PARENT_TRANSPORT,1)
    if restored!=original:
        raise RuntimeError('P18 finalizer differs outside the three preregistered compatibility surfaces')
    ...
    gates=(
        ...
        'sparse_stream_superiority_required_for_promotion',
    ...
    out.write_text(text)
    print('PASS_P18_P13_FINALIZER_EXACT_THREE_SURFACE_COMPATIBILITY_PATCH')
    return 0
