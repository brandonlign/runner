...
EXPECTED_P11_SOURCE = Path('/tmp/run_p11.py')
...
def install_same_request_transport_resilience() -> None:
    ...
    def resilient_get_monthly_file_content_by_date(*args: Any, **kwargs: Any) -> Any:
        ...
        if key in cache:
            print(f"P11_GMN_INPROCESS_CACHE_HIT args={args!r} kwargs={kwargs!r}", flush=True)
            return cache[key]
        last_error: BaseException | None = None
        for attempt in range(3):
            try:
                print(f"P11_GMN_FETCH_ATTEMPT={attempt + 1}/3 args={args!r} kwargs={kwargs!r}", flush=True)
                ...
                if attempt >= 2:
                    raise
                delay = 2 ** attempt
                print(f"P11_GMN_TRANSIENT_RETRY delay_s={delay} error={exc!r}", flush=True)
                time.sleep(delay)
        raise RuntimeError(f"unreachable P11 GMN retry state: {last_error!r}")

    mt.get_monthly_file_content_by_date = resilient_get_monthly_file_content_by_date
    print('PASS_P11_SAME_REQUEST_GMN_TRANSPORT_RESILIENCE_MAX3', flush=True)


def load_p11() -> Any:
    require(EXPECTED_P11_SOURCE.is_file(), f'P11 runtime missing: {EXPECTED_P11_SOURCE}')
    spec = importlib.util.spec_from_file_location('orbittrace_p11_runtime', EXPECTED_P11_SOURCE)
    require(spec is not None and spec.loader is not None, 'cannot load P11 runtime')
...
def self_test() -> None:
    ...
    print('PASS_P11_HEADER_WHITESPACE_COMPAT_SELF_TEST')
