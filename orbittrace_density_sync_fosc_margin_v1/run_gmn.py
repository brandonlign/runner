...
def main() -> int:
    ...
    def density_wrapper(tree, years):
        ...
        optimal, alternatives, margins = fosc_optimal_values_and_selected_margins(tree, synchronous, selected)
    ...
    def candidate_wrapper(labels, selected_nodes, events, ordinary, synchronous):
        req(state["margins"] is not None, "margin state missing before candidate construction")
        req(tuple(int(x) for x in selected_nodes) == state["selected_nodes"], "selected node tuple changed between extraction and margin calculation")
        ...
        req(len(successor) == len(baseline), "margin ranking changed candidate count")
        base_multi = membership_multiset_sha(baseline)
        succ_multi = membership_multiset_sha(successor)
        req(base_multi == succ_multi, "margin ranking changed candidate membership universe")
        req({r["family_id"] for r in baseline} == {r["family_id"] for r in successor}, "margin ranking changed candidate identities")
        ...
        margin_values = np.asarray([float(state["margins"][int(r["node_id"])]) for r in baseline], dtype=float)
        prelabel = {
            "schema": "DENSITY_SYNC_FOSC_MARGIN_V1_PRELABEL",
            "scientific_role": "PRELABEL_FROZEN_DENSITY_SYNC_FOSC_MARGIN_V1",
            ...
            "fosc_decision_margin": {str(k): float(v) for k, v in sorted(state["margins"].items())},
            "fosc_descendant_alternative": {str(k): float(v) for k, v in sorted(state["alternatives"].items())},
            "margin_summary": {
    ...
    def metrics_wrapper(candidates, hidden, annual_ids):
        ...
        req(state["baseline_candidates"] is not None, "#1263 baseline candidate order missing at truth evaluation")
    ...
    # Monkeypatch only the three narrow interfaces needed to (1) observe the exact
    # already-computed #1263 tree/objective, (2) reorder the exact candidate list,
    # and (3) evaluate the untouched #1263 order with the same labels.  The exact
    # parent main() retains parsing, HDBSCAN, truth timing and metric code.
    ...
    strict_100 = any(
        int(successor_metrics[year]["recovered_at_100"]) > int(state["baseline_metrics"][year]["recovered_at_100"])
        for year in ("2022", "2023")
    )
    passed = bool(
        ...
        and state["membership_multiset_sha"] == membership_multiset_sha(state["baseline_candidates"])
    )
    verdict = (
    ...
    result = {
        "schema": "DENSITY_SYNC_FOSC_MARGIN_V1_GMN_DEVELOPMENT",
        "verdict": verdict,
        ...
        "sole_successor_change": "rank_exact_pr1263_selected_candidates_by_fosc_decision_margin",
    ...
    # Preserve the exact-parent wrapper products, but rename them so their
    # internal recurrent-vs-margin verdict cannot be mistaken for this method's
    # binding #1263-vs-margin gate.
    ...
    print(json.dumps({
        ...
        "parent": {y: {k: v for k, v in state["baseline_metrics"][y].items() if k != "first_rank_by_label"} for y in ("2022", "2023")},
        "successor": {y: {k: v for k, v in successor_metrics[y].items() if k != "first_rank_by_label"} for y in ("2022", "2023")},
        "annual_gates": annual_gates,
