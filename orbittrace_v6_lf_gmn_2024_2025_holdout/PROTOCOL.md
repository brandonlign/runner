...
- GMN 2024/2025 tests later-year temporal generalization under the exact native geometry/schema family used for development.
...
Execution fanout/checkpointing may be used only as exact-equivalent infrastructure.
...
The holdout uses the same two-pass firewall as v6-LF development.

### Pre-truth

Only stable ID and geometry are read. The shower-label column value must remain unread through:

- input filtering;
- all-event calibration construction;
- calibration/null generation;
- candidate proposal generation;
- exact rescoring;
- component construction;
- 2024/2025 recurrent-family construction;
- primary-family ranking;
- durable serialization and SHA-256 freeze of the complete ranked primary/rescue family payload.

### Truth

Only after the frozen pre-truth SHA exists may the same monthly files be re-read and the native shower-label values accessed for evaluation.

The post-freeze truth event-ID universe must exactly equal the pre-truth event-ID universe.
...
The exact same v6-LF all-event calibration rule is mandatory.

## Integrity / power gates

All must pass before scientific interpretation:

- complete 12-month retrieval for both 2024 and 2025;
...
The holdout comparison is against the already-promoted v8 development floor because those metrics were fixed before v6-LF existed.
...
These are deliberately modest preservation gates rather than development-replication gates. They require the later-year catalogue to retain most of the established v8-level discovery power while allowing real temporal changes in shower population, observing cadence and label support.

The thresholds are fixed now and may not be changed after the holdout result.
...
A scientific FAIL rejects v6-LF promotion even if development and SonotaCo literature comparison look favorable. No adjustment on GMN 2024/2025 is permitted.

## Promotion rule

For v6-LF to become the preferred final detector, require:

1. `PASS_V6_LABEL_FREE_ALL_EVENT_NULL_DEVELOPMENT`;
2. matched SonotaCo classification `BROAD_CATALOGUE_SUPERIORITY` or `SPARSE_STREAM_SUPERIORITY` against the frozen Sugar/HDBSCAN panels;
3. `PASS_V6_LF_GMN_2024_2025_TEMPORAL_HOLDOUT`.

Only then may the already-frozen Stage-A/Stage-B exact-ID final OrbitTrace firewall be executed with v6-LF.
