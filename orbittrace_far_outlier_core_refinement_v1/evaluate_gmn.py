...
FOCR_SCHEMA = "ORBITTRACE_FAR_OUTLIER_CORE_REFINEMENT_V1_PRETRUTH"
FOCR_ROLE = "TARGET_EXCLUDED_GMN_FOCR_V1_DEVELOPMENT_RANKING"
...
def renamed_comparison(row: dict[str, Any]) -> dict[str, Any]:
    out = dict(row)
    out["focr"] = out.pop("bwm")
    out["focr_available_candidates"] = out.pop("bwm_available_candidates")
    out["focr_capacity_shortfall"] = out.pop("bwm_capacity_shortfall")
    out["focr_evaluated_candidates"] = out.pop("bwm_evaluated_candidates")
    return out


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--focr-pretruth", type=Path, required=True)
    ...
    req(sha(a.focr_pretruth) == a.expected_pretruth_sha, "sealed FOCR ranking changed")
    req(sha(a.frozen_bwm_evaluator) == BWM_EVALUATOR_SHA, "frozen evaluator changed")
    pre = json.loads(a.focr_pretruth.read_text())
    req(pre.get("schema") == FOCR_SCHEMA and pre.get("scientific_role") == FOCR_ROLE, "wrong FOCR identity")
    req(pre.get("structural_pass") is True, "FOCR structural gate failed")
    req(pre.get("configuration", {}).get("outer_fence") == "Q3 + 3*IQR", "FOCR selector changed")
    req(pre.get("configuration", {}).get("new_tuned_parameters") == [], "unexpected FOCR tuned parameters")
    req(
    ...
    for subset in compat["subsets"]:
        subset["bwm_candidates"] = subset["focr_candidates"]
    ss = pre["size_summary"]
    compat["size_summary"] = {
        "bwm_max_top_budget_member_count": ss["focr_max_top_budget_member_count"],
        "bwm_mean_top_budget_member_count": ss["focr_mean_top_budget_member_count"],
        "bwm_p90_top_budget_member_count": ss["focr_p90_top_budget_member_count"],
        "bwm_size_biased_top_budget_member_burden": ss["focr_size_biased_top_budget_member_burden"],
    ...
    with tempfile.TemporaryDirectory(prefix="focr-eval-") as td:
        td_path = Path(td)
        compat_path = td_path / "BWM_COMPAT_FROM_FOCR.json"
    ...
    for name, rr in raw["routes"].items():
        ...
        cap["focr_shortfall_panels"] = cap.pop("bwm_shortfall_panels")
        cap["focr_total_shortfall"] = cap.pop("bwm_total_shortfall")
        routes[name] = {"focr": rr["bwm"], "support_pruned": rr["support_pruned"], "literature": rr["literature"], "capacity": cap}
    scales = {name: {"focr": rr["bwm"], "support_pruned": rr["support_pruned"]} for name, rr in raw["scales"].items()}
    comparisons = [renamed_comparison(row) for row in raw["comparisons"]]
    gates = dict(raw["gates"])
    verdict = "PASS_FOCR_V1_GMN_DEVELOPMENT" if all(gates.values()) else "FAIL_FOCR_V1_GMN_DEVELOPMENT"
    out = {
        "schema": "ORBITTRACE_FAR_OUTLIER_CORE_REFINEMENT_V1_GMN_RESULT",
        "scientific_role": "TARGET_EXCLUDED_GMN_FOCR_V1_DEVELOPMENT_RESULT",
        "development_status": pre["development_status"],
        "verdict": verdict,
        "focr_pretruth_sha256": sha(a.focr_pretruth),
        ...
        "interpretation_boundary": "FOCR was designed after BWM/CMR GMN development results. This GMN score is development evidence only. A pass requires frozen transfer testing before any generalization claim.",
    }
