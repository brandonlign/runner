...
VARIANTS=('background_odds_two_head_quality','background_odds_two_head_v19_rank_sum')
PREFERENCE={'background_odds_two_head_quality':2,'background_odds_two_head_v19_rank_sum':1}
...
def main()->int:
    ...
    p.add_argument('--sugar-v27-root',type=Path,required=True); p.add_argument('--hdbscan-v27-root',type=Path,required=True)
    ...
    a=p.parse_args(); a.output.mkdir(parents=True,exist_ok=True); require(v23.sha(a.ranker_source)==RANKER_SOURCE_SHA,'#839 ranker source changed')
    v22roots={'sugar':a.sugar_v22_root,'hdbscan':a.hdbscan_v22_root}; v27roots={'sugar':a.sugar_v27_root,'hdbscan':a.hdbscan_v27_root}

    for route in ROUTES:
        m=json.loads((v27roots[route]/'V27_PRETRUTH_FEATURE_MANIFEST.json').read_text())
        require(m['scientific_stage']=='V27_FIXED_MEMBERSHIP_B1_BACKGROUND_ODDS_PRETRUTH_FEATURE_FREEZE' and m['comparator']==route,'wrong v27 pretruth route')
        require(m['feature_dimension']==FEATURE_DIM and m['base_feature_dimension']==71 and m['background_odds_dimension']==2,'v27 feature definition changed')
        require(m['b1_event_acceptance_cutoff_used'] is False and m['membership_changed'] is False and m['aggregation_search'] is False and m['b1_parameter_search'] is False,'v27 evidence boundary changed')
        require(m['protected_interval_rows']==0 and m['truth_accessed'] is False and m['target_information_access'] is False and m['maarsy_scientific_access'] is False and m['dms_scientific_access'] is False,'v27 pretruth firewall violation')
        x=np.load(v27roots[route]/'features_v27.npy',allow_pickle=False); require(x.shape==(len(m['family_ids']),FEATURE_DIM) and v23.array_sha(x)==m['feature_sha256'],'v27 feature payload identity failed')
        odds=np.load(v27roots[route]/'background_odds_evidence.npy',allow_pickle=False); require(odds.shape==(len(m['family_ids']),2) and v23.array_sha(odds)==m['background_odds_evidence_sha256'],'v27 odds payload identity failed')
    ...
    ranker=v23.load_module(a.ranker_source,'frozen_839_v27_train')
    route_data={}; Xs=[]; y13s=[]; y14s=[]; groups=[]; route_offsets={}; cursor=0; target_diag={}
    for route in ROUTES:
        base=json.loads((v22roots[route]/'V22_PRETRUTH_FEATURE_MANIFEST.json').read_text()); fam_payload=json.loads((v22roots[route]/'family_memberships.json').read_text()); aug=json.loads((v27roots[route]/'V27_PRETRUTH_FEATURE_MANIFEST.json').read_text())
        ...
        X=np.load(v27roots[route]/'features_v27.npy',allow_pickle=False); C=np.load(v22roots[route]/'centroids.npy',allow_pickle=False); require(X.shape==(len(ids),FEATURE_DIM) and C.shape==(len(ids),8),'v27 training array shape changed')
        ...
        best=[v23.combined_best_label(f,hidden,eligible) for f in fams]; y13=[]; y14=[]; gs=[]
        ...
        route_offsets[route]=(cursor,cursor+len(ids)); cursor+=len(ids); Xs.append(X); y13s.append(y13a); y14s.append(y14a); groups.extend(gs); route_data[route]={'base':base,'families':fams,'ids':ids,'centroids':C,'eligible':eligible}
    ...
    Xall=np.vstack(Xs); y13all=np.concatenate(y13s); y14all=np.concatenate(y14s); groups=list(map(str,groups)); require(Xall.shape==(cursor,FEATURE_DIM) and len(groups)==cursor,'stacked v27 shape mismatch')
    ...
    for fold in range(5):
        ...
        tg=set(groups[i] for i in np.where(tr)[0]); eg=set(groups[i] for i in np.where(te)[0]); require(tg.isdisjoint(eg),f'group leakage fold {fold}'); fold_diag.append({'fold':fold,'train_examples':int(tr.sum()),'test_examples':int(te.sum()),'train_groups':len(tg),'test_groups':len(eg)})
    ...
    for route in ROUTES:
        ...
        variants[route]={'background_odds_two_head_quality':v23.rerank(rd['families'],qorder),'background_odds_two_head_v19_rank_sum':v23.rerank(rd['families'],fused),'v19_control':v23.rerank(rd['families'],v19order)}
    ...
    freeze={'verdict':'NOT_FROZEN_V27_OOF_FAIL','head_2013_sha256':None,'head_2014_sha256':None}
    if passed:
        h13=ranker.model(); h14=ranker.model(); h13.fit(Xall,y13all,sample_weight=weights); h14.fit(Xall,y14all,sample_weight=weights); h13.set_params(n_jobs=1); h14.set_params(n_jobs=1); p13=a.output/'v27_sonotaco_head_2013.joblib'; p14=a.output/'v27_sonotaco_head_2014.joblib'; joblib.dump(h13,p13); joblib.dump(h14,p14); freeze={'verdict':'PASS_V27_FULL_SONOTACO_TWO_HEAD_MODEL_FREEZE','head_2013_sha256':v23.sha(p13),'head_2014_sha256':v23.sha(p14),'feature_dimension':FEATURE_DIM,'training_examples':cursor,'training_groups':len(set(groups)),'training_feature_sha256':v23.array_sha(Xall),'target_2013_sha256':v23.array_sha(y13all),'target_2014_sha256':v23.array_sha(y14all),'in_sample_full_fit_score_used_for_promotion':False}
    (a.output/'V27_FULL_MODEL_FREEZE.json').write_text(json.dumps(freeze,indent=2,sort_keys=True)+'\n')
    result={'scientific_stage':'V27_EXPOSED_SONOTACO_B1_BACKGROUND_ODDS_EVIDENCE_TWO_HEAD_OOF_RANKING_DEVELOPMENT','sole_scientific_change_from_v24':'append two annual mean continuous B1 stream-vs-local-field log posterior-odds features on exact fixed expanded membership','feature_dimension':FEATURE_DIM,'b1_membership_cutoff_used':False,'membership_changed':False,'aggregation_search':False,'v24_two_head_objective_unchanged':True,'folds':fold_diag,'target_diagnostics':target_diag,'order_diagnostics':order_diag,'v19_control_reproduction_pass':True,'v19_control':control,'all_results':rows,'winner':winner,'verdict':'PASS_V27_EXPOSED_BACKGROUND_ODDS_ALL_PANEL_LITERATURE_SUPERIORITY_DEVELOPMENT' if passed else 'FAIL_V27_BACKGROUND_ODDS_ALL_PANEL_LITERATURE_SUPERIORITY_DEVELOPMENT','full_model_freeze':freeze,'sonotaco_role':'EXPOSED_DEVELOPMENT_ONLY','full_fit_in_sample_score_used':False,'post_result_second_search':False,'maarsy_scientific_access':False,'dms_scientific_access':False,'target_information_access':False}
    (a.output/'V27_EXPOSED_BACKGROUND_ODDS_RESULT.json').write_text(json.dumps(result,indent=2,sort_keys=True,allow_nan=False)+'\n'); print(json.dumps({'verdict':result['verdict'],'winner':winner,'full_model_freeze':freeze},indent=2,sort_keys=True,allow_nan=False)); return 0
