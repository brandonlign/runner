...
def audit_year(year: int) -> dict:
    expected_hash, expected_member, expected_rows = FROZEN[year]
    ...
    with zipfile.ZipFile(io.BytesIO(raw)) as archive:
        ...
        gates["exact_member"] = csv_members == [expected_member]
        if expected_member not in names:
            raise RuntimeError(f"{year}: missing pinned member; members={names}")
        data = archive.read(expected_member)
...
def main() -> None:
    ...
    verdict = "PASS_CAMSV3_STRUCTURAL_FEASIBILITY" if all(gates.values()) else "KILL_CAMSV3_STRUCTURAL_FEASIBILITY"
    result = {
        "method": "CAMSv3 structural feasibility only",
        "years": years,
    ...
    result_path = out / "camsv3_structural_feasibility.json"
    result_path.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")

    report = [
        "# CAMSv3 independent-survey structural feasibility",
        "",
        f"**Verdict:** `{verdict}`",
        "",
    ...
    print(json.dumps({"verdict": verdict, "gates": gates, "header": canonical_header}, indent=2))
    if verdict != "PASS_CAMSV3_STRUCTURAL_FEASIBILITY":
        raise SystemExit(1)
