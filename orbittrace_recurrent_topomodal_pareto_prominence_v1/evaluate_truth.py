...
def main()->int:
    ...
    req(pre['schema']=='ORBITTRACE_RECURRENT_TOPOMODAL_PARETO_PROMINENCE_V1_PRELABEL','wrong prelabel schema');req(pre['scientific_role']=='PRELABEL_RECURRENT_TOPOMODAL_PARETO_PROMINENCE_V1','wrong prelabel role');req(pre['source_prelabel_sha256']==SOURCE_PRELABEL_SHA256,'wrong source prelabel')
    req(pre['configuration']=={'objectives':['corroborating_parent_rank_minimize','modal_prominence_rank_minimize'],'modal_prominence_order':'modal_contrast_desc_native_support_rank_asc_family_hash_asc','pareto':'ordinary_nondominated_layers','final_order':'pareto_layer_modal_prominence_rank_parent_rank_native_support_rank_family_hash','equal_budget':'exact_recurrent_parent_count_per_panel'},'configuration changed')
    ...
    req(audit['schema']=='ORBITTRACE_RECURRENT_TOPOMODAL_PARETO_PROMINENCE_V1_PRETRUTH','wrong pretruth schema');req(audit['scientific_role']=='ZERO_LABEL_PRETRUTH_AUTHORIZATION','wrong pretruth role');req(audit['verdict']=='PASS_RECURRENT_TOPOMODAL_PARETO_PROMINENCE_V1_PRETRUTH','pretruth did not pass');req(audit['prelabel_sha256']==pre_sha,'pretruth/prelabel mismatch');req(len(audit['gates'])==12 and all(bool(v) for v in audit['gates'].values()),'pretruth gates')
    ...
    for row in pre['subsets']:
        s=list(row['successor_candidates']);p=list(row['recurrent_candidates']);K=int(row['equal_budget_k']);req(len(p)==K and len(s)>=K,'budget/count changed');req([int(x['rank']) for x in p]==list(range(1,K+1)),'parent rank')
        req([int(x['pareto_prominence_rank']) for x in s]==list(range(1,len(s)+1)),'successor rank');req(all(x['catalogue_source']=='recurrent_topomodal_pareto_prominence' for x in s),'wrong successor source')
        annual=set(row['annual_event_ids']['2022'])|set(row['annual_event_ids']['2023']);req(len(annual)==int(row['event_count']),'universe count');req(all(set(x['event_ids']).issubset(annual) for x in s+p),'candidate outside panel')
    parent=load_module(a.parent_runner,'pareto_prominence_truth_parent');q=load_module(a.quality_source,'pareto_prominence_truth_gmn')
    ...
    support.YEARS=YEARS;support.MONTH_KEYS=MONTH_KEYS;support.CORPUS='orbittrace-recurrent-topomodal-pareto-prominence-v1';support.RANKING_VARIANTS=('persistence',)
    req((float(support.BLIND_LOW),float(support.BLIND_HIGH))==BLIND,'firewall changed');setattr(a,'fixed4_baseline_json',a.v8_result_json);_candidate,baseline,_scorer=support.load_sources(a);scan,_cal,hidden,sources=support.parse_catalogue(baseline)
    ...
    for d in (128,1024):
      for b in BUCKETS:
        frozen=subset[(d,b)];K=int(frozen['equal_budget_k']);succ=list(frozen['successor_candidates'])[:K];par=list(frozen['recurrent_candidates']);req(len(succ)==len(par)==K,'equal budget drift')
        for year in YEARS:
          annual=set(frozen['annual_event_ids'][str(year)]);pm=compact(parent.metrics(par,hidden,annual));sm=compact(parent.metrics(succ,hidden,annual));req(int(pm['eligible_labels'])==int(sm['eligible_labels']),'eligibility changed')
