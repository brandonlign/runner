#!/usr/bin/env python3
"""Audit the frozen SonotaCo-2020 episode universe without computing detector scores."""
...
def main() -> None:
    ...
    base = load_module(
        "sonotaco_2020_eligibility_base",
        decode_file(args.baseline_payload),
        BASELINE_SOURCE_SHA256,
    )
    scorer = load_module(
        "sonotaco_2020_eligibility_scorer",
    ...
    confirmation = load_module("sonotaco_2020_confirmation", confirmation_payload)
    parser = getattr(confirmation, "parse_sonotaco_2020_events")
    ...
    gates = {
        ...
        "calibration_resolution_matches_frozen_rescue": (
            scorer.CALIBRATION_NEGATIVES_PER_BIN == CALIBRATION_PER_BIN
            and RESCUE_ALPHA == 1.0 / 129.0
        ),
        "no_detector_score_computed": True,
    }
    verdict = (
        "PASS_SONOTACO_2020_LABEL_ELIGIBILITY_AUDIT"
        if all(gates.values())
        else "FAIL_SONOTACO_2020_LABEL_ELIGIBILITY_AUDIT"
    )
    result: dict[str, Any] = {
        "verdict": verdict,
        "classification": (
            "label mapping, episode eligibility, and null-window support audit after dual-channel "
            "method freeze; no fixed4 score, wavelet coefficient, dual decision, AUROC, or recall computed"
        ),
        "year": YEAR,
        "corpus": CORPUS,
        ...
        "prohibited_access": {
            ...
            "dual_channel_decisions": False,
            "auc_or_recall": False,
        },
    }
    (args.output / "sonotaco_2020_label_eligibility_audit.json").write_text(
