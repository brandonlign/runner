...
def main()->int:
    ...
    qmod=parent.load_module(a.quality_source,'gmn_orbit_coverage_audit_utility')
    ...
    support.CORPUS='orbittrace-gmn-orbit-coverage-audit-v1-target-excluded'
    ...
    def wrapped(text: str):
        ...
        frame=original(text); frame_count+=1
        req('unique_trajectory_identifier' in frame.columns,'raw GMN ID column missing')
        for _,row in frame.iterrows():
            eid=str(row['unique_trajectory_identifier'])
            req(eid not in raw_by_id,f'duplicate raw trajectory ID: {eid}')
            raw_by_id[eid]={k:(row[k] if k in frame.columns else None) for k in FIELDS}
        return frame
    ...
    by_year_ids={}
    for y in YEARS:
        ids=[str(row['id']) for row in scan[y]]
        by_year_ids[y]=ids; accessible_ids.extend(ids)
    ...
    matched=[eid for eid in accessible_ids if eid in raw_by_id]
    counts={}
    for field in FIELDS:
        counts[field]=sum(finite(raw_by_id[eid].get(field)) for eid in matched)
    q_selected='q_au' if counts['q_au']>=counts['q_au_'] else 'q_au_'
    required=(q_selected,'e','i_deg','peri_deg','node_deg')
    complete=sum(all(finite(raw_by_id[eid].get(f)) for f in required) for eid in matched)
    valid=0
    for eid in matched:
        row=raw_by_id[eid]
    ...
    year_complete={}
    for y in YEARS:
        ids=[eid for eid in by_year_ids[y] if eid in raw_by_id]
        year_complete[str(y)]={
            'accessible':len(by_year_ids[y]),
            'matched_raw_id':len(ids),
            'complete_required_orbit':sum(all(finite(raw_by_id[eid].get(f)) for f in required) for eid in ids),
        }

    result={
        'verdict':'PASS_GMN_ORBIT_COVERAGE_AUDIT_V1',
        'scientific_role':'TARGET_EXCLUDED_ORBIT_FIELD_COVERAGE_ONLY_NO_TRUTH',
        'raw_frame_count':frame_count,
        'accessible_events':len(accessible_ids),
        'raw_id_matches':len(matched),
        'finite_field_counts':counts,
        'selected_perihelion_distance_field':q_selected,
        'required_orbit_fields':list(required),
        'complete_required_orbit':complete,
        'physically_valid_required_orbit':valid,
        'coverage_fraction':complete/len(accessible_ids),
        'valid_fraction':valid/len(accessible_ids),
    ...
    (a.output/'GMN_ORBIT_COVERAGE_AUDIT_V1.json').write_text(json.dumps(result,indent=2,sort_keys=True,allow_nan=False)+'\n')
    print(json.dumps({k:result[k] for k in ('verdict','accessible_events','raw_id_matches','selected_perihelion_distance_field','complete_required_orbit','coverage_fraction','physically_valid_required_orbit','valid_fraction','year_coverage')},indent=2,sort_keys=True))
    return 0
