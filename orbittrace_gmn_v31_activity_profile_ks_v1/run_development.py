#!/usr/bin/env python3
"""Frozen GMN v31 + one cross-year activity-profile KS coordinate."""
...
def ks2(a,b):
    a=np.sort(np.asarray(a,float)); b=np.sort(np.asarray(b,float)); req(len(a)>0 and len(b)>0,'empty KS sample'); z=np.unique(np.concatenate([a,b])); fa=np.searchsorted(a,z,side='right')/len(a); fb=np.searchsorted(b,z,side='right')/len(b); d=float(np.max(np.abs(fa-fb))); req(0<=d<=1 and math.isfinite(d),'invalid KS'); return d
...
def main():
    ...
    for f in hard:
        vals=[]; cc=[]
        for year in YEARS:
            c=f.get('centroids',{}).get(str(year)); req(c is not None,f'missing centroid {f["family_id"]} {year}'); eids=[str(e) for e in f['event_ids'] if int(str(e)[:4])==year]; req(eids,f'no members {f["family_id"]} {year}'); u=[]
            for eid in eids:
                e=lookup.get(eid); req(e is not None,f'member absent {eid}'); u.append(signed_delta(e['sol'],c['sol']))
            vals.append(u); cc.append(len(u))
        ksv.append(ks2(vals[0],vals[1])); counts.append(cc)
    ks=np.asarray(ksv,float); req(ks.shape==(N,) and np.isfinite(ks).all() and np.all((ks>=0)&(ks<=1)),'KS vector invalid'); X24=np.column_stack([X,ks]); cand=oof_margin(X24,folds,y,ids,hard_rank)
    ...
    pos=ks[y]; neg=ks[~y]; result={'verdict':'PASS_GMN_V31_ACTIVITY_PROFILE_KS_V1' if passed else 'FAIL_GMN_V31_ACTIVITY_PROFILE_KS_V1','scientific_role':'TARGET_EXCLUDED_GMN_2022_2023_V31_SUCCESSOR_DEVELOPMENT_ONLY','first_valid_outcome_binding':True,'sole_scientific_change':'append exact cross-year two-sample KS statistic of annual-centroid-centered member solar-longitude offsets to exact v31 23D representation','candidate_count':N,'feature_dimension_parent':23,'feature_dimension_candidate':24,'parent_feature_sha256':arrsha(X),'centroid_sha256':arrsha(cm),'parent_margin_sha256':arrsha(parent),'activity_ks_sha256':arrsha(ks),'candidate_margin_sha256':arrsha(cand),'candidate_fused_order_sha256':ordersha(c_fused),'hard_control':HARD,'parent_control':PARENT,'parent_reproduced_metrics':trim(parent_metrics),'activity_ks_local_only':trim(local_metrics),'activity_ks_equal_rank_fusion':trim(metrics),'pass_gates':gates,'ks_summary_all':{'min':float(ks.min()),'median':float(np.median(ks)),'max':float(ks.max())},'ks_summary_positive':{'min':float(pos.min()),'median':float(np.median(pos)),'max':float(pos.max())},'ks_summary_nonpositive':{'min':float(neg.min()),'median':float(np.median(neg)),'max':float(neg.max())},'per_family':[{'family_id':fid,'ks':float(ks[i]),'n2022':counts[i][0],'n2023':counts[i][1]} for i,fid in enumerate(ids)],'feature_search':False,'activity_statistic_search':False,'histogram_or_bandwidth':False,'ks_pvalue_used':False,'metric_search':False,'k_search':False,'scaling_search':False,'reference_change':False,'diversity_search':False,'fusion_search':False,'post_result_second_search':False,'blind_exclusion':[20.0,55.0],'sonotaco_2013_2014_access':False,'target_information_access':False,'target_region_events_accessed':False,'maarsy_scientific_access':False,'dms_scientific_access':False,'sonotaco_benchmark_authorized_by_this_result':bool(passed)}
    (a.output/'GMN_V31_ACTIVITY_PROFILE_KS_V1_RESULT.json').write_text(json.dumps(result,indent=2,sort_keys=True,allow_nan=False)+'\n'); print(json.dumps({k:v for k,v in result.items() if k!='per_family'},indent=2,sort_keys=True,allow_nan=False)); return 0
