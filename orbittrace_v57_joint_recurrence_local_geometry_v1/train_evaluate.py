...
def main() -> int:
    ...
    ranker = v22.load_module(a.ranker_source, 'frozen_839_v57_joint_recurrence_geometry')
    ...
    require(Xall.shape == (cursor, FEATURE_DIM) and len(y13all) == len(y14all) == len(groups) == cursor, 'v57 stacked input mismatch')
    folds = np.asarray([v22.v1.deterministic_fold(g) for g in groups], dtype=int)
    joint_positive_all = (y13all > RECOVERY) & (y14all > RECOVERY)
    require(joint_positive_all.any() and (~joint_positive_all).any(), 'global v57 joint recurrence class empty')

    margin = np.zeros(cursor, dtype=float)
    ...
    for fold in range(5):
        ...
        pos = joint_positive_all[tr]
        neg = ~pos
        require(pos.any() and neg.any(), f'fold {fold} lacks joint-positive or joint-nonpositive references')
        P = Ztr[pos]; N = Ztr[neg]
        te_idx = np.where(te)[0]
        for j, global_i in enumerate(te_idx.tolist()):
            dpos = float(np.min(np.linalg.norm(P - Zte[j], axis=1)))
            dneg = float(np.min(np.linalg.norm(N - Zte[j], axis=1)))
            margin[global_i] = dneg - dpos
        fold_diag.append({
            'fold': fold,
            'train_examples': int(tr.sum()),
            'test_examples': int(te.sum()),
            'joint_positive_references': int(pos.sum()),
            'joint_nonpositive_references': int(neg.sum()),
            'zero_variance_features': int(np.sum(sd == 0.0)),
        })

    require(np.all(np.isfinite(margin)), 'nonfinite v57 joint margin')
    variants: dict[str, Any] = {}; order_diag: dict[str, Any] = {}; v19_control: list[dict[str, Any]] = []
    for route in v24.ROUTES:
        lo, hi = offsets[route]; rd = route_data[route]; ids = rd['ids']; scores = margin[lo:hi]
        ...
        route_joint = joint_positive_all[lo:hi]
        order_diag[route] = {
            'joint_margin_sha256': v22.array_sha(scores),
            'local_diversity_order_sha256': order_sha(local_order),
            'fused_order_sha256': order_sha(fused),
            'route_joint_positive_families': int(route_joint.sum()),
            'route_joint_nonpositive_families': int((~route_joint).sum()),
    ...
    result = {
        'scientific_stage': 'EXPOSED_SONOTACO_V57_JOINT_RECURRENCE_LOCAL_GEOMETRY_V1',
        'verdict': 'PASS_V57_JOINT_RECURRENCE_LOCAL_GEOMETRY_ALL_PANEL_LITERATURE_SUPERIORITY_DEVELOPMENT' if passed else 'FAIL_V57_JOINT_RECURRENCE_LOCAL_GEOMETRY_ALL_PANEL_LITERATURE_SUPERIORITY_DEVELOPMENT',
        'sole_scientific_change': 'replace exact-v31 two annual positive-reference heads plus annual min with one family-level joint-recurrence positive-reference pool requiring own-family F1>0.5 in both years and one ordinary-Euclidean k=1 margin',
        ...
        'joint_positive_predicate': 'annual_f1_2013 > 0.5 AND annual_f1_2014 > 0.5 for the same fixed family',
        'joint_nonpositive_predicate': 'NOT joint_positive',
        'recovery_f1_threshold': RECOVERY,
        'nearest_k': 1,
        'local_margin': 'd_joint_nonpositive-d_joint_positive',
        'annual_margins_used': False,
        'annual_combiner_used': False,
        ...
        'joint_positive_count_total': int(joint_positive_all.sum()),
        'joint_nonpositive_count_total': int((~joint_positive_all).sum()),
        'joint_predicate_preexisting_benchmark_event': True,
        'classifier_or_regressor_used': False,
        'group_positive_propagation_used': False,
        'or_union_positive_used': False,
        'alternate_f1_threshold_used': False,
        'soft_quality_weight_used': False,
        'annual_margin_blend_used': False,
        'k_search': False,
        'metric_search': False,
    ...
    out = a.output / 'V57_JOINT_RECURRENCE_LOCAL_GEOMETRY_RESULT.json'
