...
def main() -> int:
    ...
    for name in (
        "intrinsic-runner", "prelabel", "parent-runner", "quality-source", "support-source-parts",
        "candidate-payload", "baseline-payload", "scorer-parts", "v8-result-json",
    ):
    ...
    intrinsic = load_module(a.intrinsic_runner, "rdtm_eval_intrinsic")
    ...
    req(pre["schema"] == "ORBITTRACE_RECURRENT_DENSITY_TOPOMODAL_V1_PRELABEL", "prelabel schema")
    req(pre["scientific_role"] == "PRELABEL_RECURRENT_DENSITY_TOPOMODAL_V1", "prelabel role")
    ...
    req(pre["configuration"]["graph"] == "exact_radius_1_physical_embedding_1284", "graph changed")
    req(pre["configuration"]["density"] == "min(radius_year_count_over_year_total_2022,radius_year_count_over_year_total_2023)", "density changed")
    req(pre["configuration"]["candidate_universe"] == "complete_tomato_hierarchy_support_ge_4", "candidate universe changed")
    req(pre["configuration"]["ranking"] == "exact_1284_native_rule_on_recurrent_density", "ranking changed")
    ...
    for r in pre["subsets"]:
        ...
        req(int(r["successor_summary"]["positive_recurrent_density_count"]) + int(r["successor_summary"]["zero_recurrent_density_count"]) == int(r["events_total"]), "density count identity")
        req(float(r["successor_summary"]["rho_rec_max"]) >= 0.0 and float(r["successor_summary"]["rho_rec_mean"]) >= 0.0, "density summary")

    parent = load_module(a.parent_runner, "rdtm_eval_parent")
    ...
    q = load_module(a.quality_source, "rdtm_eval_gmn")
    ...
    support.CORPUS = "orbittrace-recurrent-density-topomodal-v1-evaluator"
    ...
    for denominator in (128, 1024):
        for bucket in BUCKETS:
            ...
            for year in YEARS:
                ...
                truth_panels.append({
                    "denominator": denominator, "bucket": bucket, "year": year, "equal_budget_k": K,
                    "parent": pm, "successor": sm,
    ...
    for denominator in (128, 1024):
        ...
        scales[str(denominator)] = {
            "parent": pa, "successor": sa,
            "qualified_nonlower_panels": non, "qualified_strict_win_panels": win, "qualified_loss_panels": 8 - non,
        }
    ...
    verdict = "PASS_RECURRENT_DENSITY_TOPOMODAL_V1" if all(gates.values()) else "FAIL_RECURRENT_DENSITY_TOPOMODAL_V1"

    result = {
        "schema": "ORBITTRACE_RECURRENT_DENSITY_TOPOMODAL_V1",
        "scientific_role": "TARGET_EXCLUDED_GMN_2022_2023_SPARSE_RECOVERY_DEVELOPMENT",
        "verdict": verdict, "prelabel_sha256": pre_sha,
        "structural_source_run_id": 31955621864, "structural_source_artifact_id": 9265889512,
        "structural_result_sha256": STRUCTURAL_RESULT_SHA256,
        "intrinsic_source_commit": "312b1b718ae105813de242355142a74e7d377d65", "intrinsic_source_blob": INTRINSIC_SOURCE_BLOB,
        "panels": truth_panels, "scale_aggregates": scales, "gates": gates,
    ...
    out = a.output / "RECURRENT_DENSITY_TOPOMODAL_V1.json"
