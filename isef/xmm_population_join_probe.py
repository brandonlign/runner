...
q="""SELECT TOP 100000 s.srcid AS sid,s.ra AS sra,s.dec AS sdec,d.obsid AS dobsid
...
try:
 ...
 with urllib.request.urlopen(urllib.request.Request(u,headers={'User-Agent':'ISEF-XMM-popjoin-probe/1.1'}),timeout=180) as r: raw=r.read()
 t=Table.read(io.BytesIO(raw),format='votable')
 cols=list(t.colnames)
 if 'sid' not in cols or 'dobsid' not in cols:
  out={'success':False,'error':'ALIASES_NOT_PRESERVED','rows':len(t),'columns':cols}
 else:
  src=[str(x).strip() for x in t['sid']]; obs=[str(x).strip() for x in t['dobsid']]
  out={'success':True,'rows':len(t),'unique_srcids':len(set(src)),'nonblank_obsid_rows':sum(x not in ('','--','None') for x in obs),'hit_top_cap':len(t)>=100000,'columns':cols,'privacy':'No identities or coordinates emitted.'}
