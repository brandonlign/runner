...
def tap(adql):
    ...
    for url in TAPS:
        try:
            req=urllib.request.Request(url,data=body,headers={'User-Agent':'isef-euclid-positive-controls/1.2','Content-Type':'application/x-www-form-urlencoded'})
...
def main():
    ra,de=CENTER;adql=f"""SELECT TOP 30 gs.source_id,gs.ra,gs.dec,gs.phot_g_mean_mag,v.amplitude_estimate,v.frequency
