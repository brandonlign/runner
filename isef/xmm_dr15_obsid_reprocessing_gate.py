...
No source names/coordinates are emitted. We ask whether clean 5XMM sources that are
positionally absent from 4XMM carry an XMM ObsID already represented in the official
4XMM-DR14 catalogue. Exact ObsID overlap proves the underlying observation already
existed in the previous catalogue era; positional absence therefore isolates a
reprocessing/stacking recovery aperture rather than merely later XMM observing time.
"""
...
def tap():
 q=f'SELECT TOP 100000 ra,dec,obsid FROM xmmssc WHERE {BASE}'
 url=EP+'?'+urllib.parse.urlencode({'REQUEST':'doQuery','LANG':'ADQL','FORMAT':'votable','QUERY':q})
 req=urllib.request.Request(url,headers={'User-Agent':'ISEF-XMM-obsid-gate/1.1'})
 with urllib.request.urlopen(req,timeout=180) as r: raw=r.read()
 return Table.read(io.BytesIO(raw),format='votable')
def dl4():
 reqh=urllib.request.Request(U4,method='HEAD',headers={'User-Agent':'ISEF-XMM-obsid-gate/1.1'}); total=int(urllib.request.urlopen(reqh,timeout=30).headers['Content-Length'])
 if P4.exists() and P4.stat().st_size==total:return total
 req=urllib.request.Request(U4,headers={'User-Agent':'ISEF-XMM-obsid-gate/1.1'})
...
def main():
 try:
  t=tap(); size=dl4(); ra5=np.asarray(t['ra'],float); de5=np.asarray(t['dec'],float); obs5=np.asarray([norm(x) for x in t['obsid']])
  ...
  out={'success':True,'five_rows':len(t),'five_query_cap':100000,'four_rows':len(ra4),'four_catalog_bytes':size,'four_obsid_column':obscol,
  ...
  out['interpretation']='If a substantial conservative-unmatched cohort has a nonblank 5XMM ObsID already present in 4XMM-DR14, the reprocessing-recovery aperture is real. This 100k-row blind gate establishes existence, not population prevalence.'
  save(out)
