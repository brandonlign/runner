#!/usr/bin/env python3
"""Frozen odd-sector holdout replication of the 5XMM variability-loss mechanism."""
...
OUT=Path('results/xmm_holdout_variability_mechanism.json');OUT.parent.mkdir(parents=True,exist_ok=True)
...
def tap(ep,q,timeout=420):
 d=urllib.parse.urlencode({'REQUEST':'doQuery','LANG':'ADQL','QUERY':q}).encode();req=urllib.request.Request(ep,data=d,headers={'Content-Type':'application/x-www-form-urlencoded','User-Agent':'ISEF-XMM-holdout/1.0'})
...
def stacks(url,p):
 req=urllib.request.Request(url,headers={'User-Agent':'ISEF-XMM-holdout/1.0'})
...
def maps():
 s4,r4=stacks(U4,Path('/tmp/h4.gz'));s5,r5=stacks(U5,Path('/tmp/h5.gz'));bad4={a for v in r4.values() if len(v)>1 for a in v};bad5={a for v in r5.values() if len(v)>1 for a in v};by5=defaultdict(list)
...
def classify_loss(old,new_by_parent,m45,radius):
 p5=m45.get(old['parent']);pool=new_by_parent.get(p5,[])
 ...
 return int(float(np.asarray(sp.arcsec).reshape(-1)[0])>radius)
def raw_or(y,strong):
...
def fit_model(rows,radius,min_ml):
 z=[r for r in rows if r['ep_det_ml']>=min_ml];y=np.array([r[f'lost{int(radius)}'] for r in z],float);strong=np.array([int(r['fratio']>=5) for r in z],float);logml=np.log(np.array([r['ep_det_ml'] for r in z],float));ncon=np.log1p(np.array([r['n_contrib'] for r in z],float));perr=np.log1p(np.array([r['radec_err'] for r in z],float));secs=np.array([r['sector'] for r in z],int)
 ...
 try:
  fit=sm.GLM(y,X,family=sm.families.Binomial()).fit(cov_type='cluster',cov_kwds={'groups':groups});i=names.index('fratio_ge5');beta=float(fit.params[i]);se=float(fit.bse[i]);p=float(fit.pvalues[i]);lo=beta-1.959963984540054*se;hi=beta+1.959963984540054*se
  return {'success':True,'n':len(z),'lost':int(y.sum()),'loss_fraction':float(y.mean()),'strong_n':int(strong.sum()),'strong_fraction':float(strong.mean()),'clusters':len(set(groups)),'adjusted_or':math.exp(beta),'ci95':[math.exp(lo),math.exp(hi)],'p_value':p,'beta':beta,'se_cluster':se,'raw':raw_or(y,strong)}
 except Exception as e:return {'success':False,'error':f'{type(e).__name__}: {e}','n':len(z),'raw':raw_or(y,strong)}
def main():
 try:
  ...
  for r in old:
   r['lost5']=classify_loss(r,new,m45,5.);r['lost7']=classify_loss(r,new,m45,7.)
  primary=fit_model(old,5.,10.);ml15=fit_model(old,5.,15.);r7=fit_model(old,7.,10.);sectors={}
  for s in SECS:
  ...
  conditions={'primary_or_gt1':primary.get('adjusted_or',0)>1 if primary.get('success') else False,'primary_ci_low_gt1':primary.get('ci95',[0])[0]>1 if primary.get('success') else False,'ml15_direction_gt1':ml15.get('adjusted_or',0)>1 if ml15.get('success') else False,'r7_direction_gt1':r7.get('adjusted_or',0)>1 if r7.get('success') else False,'all_nonempty_sectors_direction_gt1':all((v['raw']['odds_ratio']>1) for v in sectors.values() if v['raw'] and min(v['raw']['strong_lost']+v['raw']['strong_retained'],v['raw']['weak_lost']+v['raw']['weak_retained'])>0),'no_top_truncation':not any(v['top_truncated'] for v in qcounts.values())}
  passed=all(conditions.values())
  out={'success':True,'status':'FROZEN_ODD_SECTOR_HOLDOUT','holdout_opened':True,'exact_pairs':len(m45),'fratio_threshold_frozen':5.0,'primary_loss_radius_arcsec_frozen':5.0,'query_counts':qcounts,'eligible_dr14_sources':len(old),'primary_ml10_r5':primary,'sensitivity_ml15_r5':ml15,'sensitivity_ml10_r7':r7,'sector_raw_effects':sectors,'replication_conditions':conditions,'decision':'VARIABILITY_MECHANISM_HOLDOUT_REPLICATED' if passed else 'VARIABILITY_MECHANISM_HOLDOUT_FAILED','note':'Frozen odd-sector replication. No source identities or coordinates emitted. No post-development tuning.'}
 except Exception as e:out={'success':False,'status':'FROZEN_ODD_SECTOR_HOLDOUT','holdout_opened':True,'error':f'{type(e).__name__}: {e}'}
