...
This program follows isef research freezes a468846f..., 411c8f..., and
813c599a.... It may read target pixels internally only to evaluate synthetic
injections and aggregate off-line nulls. It never emits coordinates, fluxes,
S/N, counts, or spectra of non-injected sources in the real [O III] window.
"""
...
DLAM=1.25; RAD=8/3600
# Full-mosaic WCS from the header-only pre-science probe.
...
def download_tile(i,ra,dec):
 # One fixed broad Stage-A band contains both null detectors, their continuum
 # sidebands, the target detector, and its continuum sidebands.
 ...
 for a in range(4):
  try:
   req=urllib.request.Request(url,headers={'User-Agent':'ISEF-NGC1427A-StageA/1.0'})
...
def wave_from(h,n):
 # ESO SODA cutout verified to have linear CD3_3=1.25 Angstrom.
...
def spec_map(data,var,wave,center):
 ...
 wt=np.where(good,1/vv,0.); sw=wt.sum(axis=0); c=np.divide((np.where(good,yy,0)*wt).sum(axis=0),sw,out=np.full(sw.shape,np.nan),where=sw>0); vc=np.divide(1.,sw,out=np.full(sw.shape,np.inf),where=sw>0)
 ...
 # Shared continuum estimate contributes correlated error across line pixels.
 coeff=np.divide((qq*inv).sum(axis=0),den,out=np.zeros(den.shape),where=den>0)
 va=np.divide(1.,den,out=np.full(den.shape,np.inf),where=den>0)+vc*coeff*coeff
 return amp,va
...
def positions(seed,valid,n=25):
 rng=np.random.default_rng(seed); ys,xs=np.where(valid); order=rng.permutation(len(xs)); out=[];mind=3*(FWHM_ARC/PIX_ARC)
 for j in order:
  x=int(xs[j]);y=int(ys[j])
  if all((x-a)**2+(y-b)**2>=mind**2 for a,b in out):out.append((x,y))
  if len(out)==n:return out
 raise RuntimeError(f'could place only {len(out)}/{n} injections')

def add_sources(base,pos,amp):
 z=base.copy();r=PR
 for x,y in pos:
  z[y-r:y+r+1,x-r:x+r+1]+=amp*P
 return z
...
def main():
 o={'status':'FROZEN_STAGEA_INJECTION_GATE','success':False,'decision':'NO_DECISION','stagea_freeze':'a468846f91e726139bf84e3f14b7138558748db6','sideband_clarification':'411c8f242b677df730d32286f11a397806c0229e','null_clarification':'813c599ad1443bd18f6a007c4f2216b8e0ddc323','real_candidate_outputs_emitted':False,'real_oiii_peak_count_emitted':False,'planned_oiii_A':LAM,'tile_results':[],'injection_domain':'spectrally compressed line-flux map; exact linear response to the frozen injected 2.6-A spectral template, before spatial matched filtering'}
 try:
  ...
  for ti,(ra,dec) in enumerate(CENTERS):
   ...
   try:
    with fits.open(p,memmap=False) as h:
     data=np.asarray(h['DATA'].data,dtype=np.float64);var=np.asarray(h['STAT'].data,dtype=np.float64);hdr=h['DATA'].header
   finally:p.unlink(missing_ok=True)
   wave=wave_from(hdr,data.shape[0]);
   if str(hdr.get('BUNIT','')).find('10**(-20)')<0: raise RuntimeError('unexpected DATA units')
   # STAT unit was transport-validated as squared DATA units; verify again.
   # Build target spectral map internally, but never emit any non-injected target statistic.
   base,basev=spec_map(data,var,wave,LAM)
   valid=np.isfinite(base)&np.isfinite(basev)&(basev>0)
   # Require full PSF kernel support via convolution of valid mask.
   ...
   for mi,m in enumerate(MAGS):
    hit=0;tot=0
    A=10**(-0.4*(m+13.74))/1e-20
    for rep in range(4):
     seed=14270000+1000*ti+10*mi+rep; pos=positions(seed,support,25); inj=add_sources(base,pos,A);snr,_=spatial_snr(inj,basev);pk=local_peak_mask(snr);rad=FWHM_ARC/PIX_ARC
     for x,y in pos:
      yy,xx=np.ogrid[:pk.shape[0],:pk.shape[1]];near=pk&((xx-x)**2+(yy-y)**2<=rad**2);hit+=int(np.any(near));tot+=1
  ...
  # Frozen 25-A target / 50-A combined null-width scaling, then sampled-area -> full mosaic.
  ...
  # At least 5/9 tiles = >=50% sampled usable area proxy.
  ...
  o['gate_recovery_27']=n27>=5;o['gate_recovery_27p5']=n275>=5;o['gate_false_positive_full_mosaic']=full_false<=1.0
  # Background-regime dominance cannot be fully assessed until we summarize variance strata; for 3x3 equal-radius tiles, require >=5 tiles with nonzero usable area as a minimal frozen operational check.
  o['gate_spatial_coverage']=sum(t['usable_pixels']>0 for t in o['tile_results'])>=5
