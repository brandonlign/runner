#!/usr/bin/env python3
"""Fast blinded old-era 5XMM vs 4XMM-DR14 slim cohort probe.

5XMM is filtered server-side by HEASARC TAP so only scientific columns needed
for the aggregate feasibility test are transferred. 4XMM DR14 slim is the
official unique-source catalogue. No source names/IDs/coordinates are emitted.
"""
...
def tap(adql):
    ...
    req=urllib.request.Request(EP,data=b,headers={'User-Agent':'ISEF-XMM-reprocessing-TAP/1.0','Content-Type':'application/x-www-form-urlencoded'})
...
def dl4():
    reqh=urllib.request.Request(U4,method='HEAD',headers={'User-Agent':'ISEF-XMM-reprocessing-TAP/1.0'})
    ...
    req=urllib.request.Request(U4,headers={'User-Agent':'ISEF-XMM-reprocessing-TAP/1.0'})
...
def main():
  try:
    # TOP protects against accidental unbounded schema mistakes; parent count is 370,948.
    t5=tap(f'SELECT TOP 500000 {COLS} FROM xmmssc WHERE {BASE}')
    dl4()
    with fits.open(P4,memmap=True) as h:
      ...
      # Fixed, interpretable enrichment counts chosen before any identity opening.
