...
BASE='s.sum_flag < 3 AND s.extent = 0 AND s.ep_det_ml >= 15'; CAP=100000; RADIUS=15.0
...
def tap(q,timeout=300):
    ...
    req=urllib.request.Request(url,headers={'User-Agent':'ISEF-XMM-eRASS1-reality/1.0'})
...
def refs():
    ...
    return SkyCoord(ra[ok]*u.deg,de[ok]*u.deg),old

def q5(lo,hi,depth=0):
    q=f'''SELECT TOP {CAP} s.srcid AS sid,s.ra AS sra,s.dec AS sdec,d.obsid AS dobsid,d.pps_srcnum AS pps
...
def qe(lo,hi,depth=0):
    q=f'''SELECT TOP {CAP} ra,dec FROM erass1main WHERE ra >= {lo:.8f} AND ra < {hi:.8f}'''
    t=tap(q)
    if len(t)>=CAP:
        if depth>=8: raise RuntimeError(f'eRASS1 row cap persists {lo}-{hi}')
        mid=(lo+hi)/2; return qe(lo,mid,depth+1)+qe(mid,hi,depth+1)
    return [t]
...
def main():
    try:
        ...
        for lo in range(0,180,5):
            t=cat_tabs(q5(float(lo),float(lo+5)))
            ...
            ids=list(by)
            if not ids: continue
            c=SkyCoord([by[s][0] for s in ids]*u.deg,[by[s][1] for s in ids]*u.deg)
            _,sep,_=c.match_to_catalog_sky(c4)
            cases=[ids[i] for i in range(len(ids)) if sep.arcsec[i]>20 and any(o in old for o in obs[ids[i]])]
            era_hi=min(360.0,float(lo+6.1))
            et=cat_tabs(qe(float(lo),era_hi))
            if len(et): ec=SkyCoord(np.asarray(et['ra'],float)*u.deg,np.asarray(et['dec'],float)*u.deg)
        ...
        n=totals['cases']; rm=totals['real_matches']; sm=totals['shift_matches']
        table=[[rm,n-rm],[sm,n-sm]]
        odds,p=fisher_exact(table,alternative='greater')
        ratio=(rm/sm) if sm else None
        gates={'g1_real_matches_ge_25':rm>=25,'g2_real_to_shift_ratio_ge_5':(sm==0 or rm>=5*sm),'g3_fisher_p_le_0p001':p<=0.001}
        out={'success':True,'hemisphere':'development','match_radius_arcsec':RADIUS,'shift_ra_deg':1.0,'totals':totals,
             'real_match_fraction':rm/n if n else None,'shift_match_fraction':sm/n if n else None,'real_to_shift_ratio':ratio,
             'fisher_odds_ratio':float(odds),'fisher_one_sided_p':float(p),'frozen_gates':gates,
             'science_status':'PASS' if all(gates.values()) else 'FAIL','bins':bins,
             'privacy':'Aggregate counts only; no source identities or coordinates emitted.'}
