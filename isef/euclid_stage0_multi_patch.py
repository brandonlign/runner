...
def fetch_cube(target,qs,shifts):
    routes,_=b.route_groups(qs,target,shifts);hs=b.epoch_headers(routes);ims=[None]*16;meta=[None]*16
...
def analyze_patch(idx,target,qs,shifts,lim):
    cube,hs,meta,routes=fetch_cube(target,qs,shifts);orig=[(int(m['x0']),int(m['y0'])) for m in meta];ra,de,peak=pd.sources(cube,hs,orig);stars=[];inj={a:[] for a in AMPS};meas=acc=0
