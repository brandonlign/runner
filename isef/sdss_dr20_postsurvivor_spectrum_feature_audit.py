...
def extract(raw):
 with fits.open(io.BytesIO(raw),memmap=False) as h:
  ...
  tab=None
  for x in h[1:]:
   if x.data is not None and hasattr(x.data,'names') and x.data.names:
    cmap={n.lower():n for n in x.data.names}
    if 'loglam' in cmap and 'flux' in cmap:tab=x.data;break
  if tab is None:raise RuntimeError('no flux/loglam table')
  cmap={n.lower():n for n in tab.names};wave=10**np.asarray(tab[cmap['loglam']],float);flux=np.asarray(tab[cmap['flux']],float);ivar=np.asarray(tab[cmap['ivar']],float) if 'ivar' in cmap else np.ones_like(flux)
  return wave,flux,ivar,meta
def feature(w,f,iv,rest,rv=-471.7881):
 expected=rest*(1+rv/C);m=(w>rest-18)&(w<rest+18)&np.isfinite(f)&(iv>0)
 if m.sum()<20:return {'expected_A':expected,'usable':False}
 ww=w[m];ff=f[m];vv=iv[m];outer=np.abs(ww-rest)>11
 if outer.sum()<6:outer=np.ones_like(ww,dtype=bool)
 p=np.polyfit(ww[outer],ff[outer],1,w=np.sqrt(np.maximum(vv[outer],1e-12)));nf=ff/np.polyval(p,ww)
 def minnear(center,half):
  q=np.abs(ww-center)<=half
  if q.sum()<3:return None
  j=np.flatnonzero(q)[np.argmin(nf[q])];lam=float(ww[j]);return {'lambda_A':lam,'norm_flux':float(nf[j]),'velocity_kms':float((lam/rest-1)*C),'offset_from_expected_A':float(lam-expected)}
 return {'expected_A':float(expected),'predicted_window_min':minnear(expected,4.0),'broad_window_min':minnear(rest,10.0),'usable':True}
