...
def exact_pairs():
 ...
 # embedded ObsID parent map is unique for every stack participating in exact cohort
...
def nearest_stats(d5,d4,p5,p4):
 # group DR14 coordinates by its parent stack
 g4=defaultdict(list)
 for r in d4:
  a=parent(r['srcid'],p4)
  if a:g4[a].append(r)
 seps=[];errscaled=[];shift_seps=[];tested=0;empty_parent=0
 for b,rows in group5(d5,p5).items():
  a_pair=p5.get(b); old=g4.get(a_pair,[])
  if not old:
  ...
  for j,(r,i,s) in enumerate(zip(rows,idx,ss)):
   e5=float(r['radec_err']) if r.get('radec_err') is not None else np.nan
   e4=float(old[int(i)]['radec_err']) if old[int(i)].get('radec_err') is not None else np.nan
   den=max(1.0,(e5 if np.isfinite(e5) else 0)+(e4 if np.isfinite(e4) else 0))
   errscaled.append(float(s)/den)
  # density null: deterministic 60-arcsec eastward true-angle shift, same parent catalogue
  ras=np.array([float(r['ra']) for r in rows]);decs=np.array([float(r['dec']) for r in rows])
  dra=(60./3600.)/np.maximum(np.cos(np.deg2rad(decs)),.0872); shifted=SkyCoord((ras+dra)%360*u.deg,decs*u.deg)
  _,sep_shift,_=shifted.match_to_catalog_sky(c4);shift_seps.extend(sep_shift.arcsec.tolist())
 return {'tested':tested,'parents_without_dr14_sources':empty_parent,'nearest_arcsec_quantiles':qtile(seps),'error_scaled_quantiles':qtile(errscaled),'random_shift_nearest_arcsec_quantiles':qtile(shift_seps),'counts_no_dr14_within_arcsec':{str(r):int(np.sum(np.asarray(seps)>r))+empty_parent for r in RADII},'fractions_no_dr14_within_arcsec':{str(r):(int(np.sum(np.asarray(seps)>r))+empty_parent)/(tested+empty_parent) if tested+empty_parent else None for r in RADII},'random_shift_fractions_within_arcsec':{str(r):float(np.mean(np.asarray(shift_seps)<=r)) if shift_seps else None for r in RADII}}

def group5(rows,emap):
 g=defaultdict(list)
 for r in rows:
  b=parent(r['srcid'],emap)
  if b:g[b].append(r)
 return g
...
def main():
 try:
  ...
  for sec in SECTORS:
   ...
   # Only exact-input, unambiguous stack cohort. Do not output identities.
   k5=[r for r in r5 if parent(r['srcid'],e5) in map5to4]
   k4=[r for r in r4 if parent(r['srcid'],e4) in map4to5]
   st=nearest_stats(k5,k4,map5to4,e4)
  ...
  # pooled recomputation is safe because sectors are separated by >=30 deg; duplicate margin old rows harmless within different parent groups
  pooled=nearest_stats(all5,all4,map5to4,e4)
