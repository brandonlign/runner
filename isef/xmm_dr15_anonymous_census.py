...
def q(adql):
    ...
    req=urllib.request.Request(EP,data=b,headers={'User-Agent':'ISEF-XMM-anon-census/1.0','Content-Type':'application/x-www-form-urlencoded'})
    with urllib.request.urlopen(req,timeout=60) as r:raw=r.read()
    txt=raw.decode('utf-8','replace')
    if 'QUERY_STATUS' in txt[:5000] and 'value="ERROR"' in txt[:5000]:raise RuntimeError(txt[:3000])
    t=Table.read(io.BytesIO(raw),format='votable');return t
def count(extra='1=1'):
...
def main():
    tests={
      ...
      'spec_good_high_nh':'spec_flag_pl = 0 AND spec_nh_pl >= 3',
    ...
    out={'success':True,'base':BASE,'counts':{}}
    ...
    try:
      ...
      # Distribution-only summaries; source identities/positions not requested.
      import numpy as np
      sm={}
      for c in t.colnames:
        try:
