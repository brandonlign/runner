#!/usr/bin/env python3
"""High-confidence Gaia DR3 short-period eclipsing-binary controls for Euclid Q2 Field 1."""
...
OUT=Path('results/euclid_gaia_eb_controls.json');OUT.parent.mkdir(parents=True,exist_ok=True);TAPS=['https://gea.esac.esa.int/tap-server/tap/sync','https://gaia.aip.de/tap/sync'];CENTER=(267.5945,-30.0074)
def parse(raw):
...
def tap(q):
 ...
 for u in TAPS:
  try:
   req=urllib.request.Request(u,data=body,headers={'User-Agent':'isef-euclid-eb-controls/1.0','Content-Type':'application/x-www-form-urlencoded'});raw=urllib.request.urlopen(req,timeout=20).read();return parse(raw),u
...
def measure(r,gm):
 ...
 fl=np.asarray(fl,float);valid=bool(np.all(np.isfinite(fl)&(fl>0)));o={'source_id':sid(r),'ra':ra,'dec':de,'routed':True,'valid_flux':valid,'gmag':float(r['gmag']),'frequency_per_day':float(r['freq']),'period_hours':24/float(r['freq']),'primary_depth_mag':float(r['depth']),'global_ranking':float(r['rank']),'route_diagnostics':diag}
 if valid:
 ...
 return o
def main():
 ra,de=CENTER;q=f"""SELECT TOP 40 gs.source_id AS sid,gs.ra AS cra,gs.dec AS cdec,gs.phot_g_mean_mag AS gmag,v.frequency AS freq,v.derived_primary_ecl_depth AS depth,v.global_ranking AS rank FROM gaiadr3.gaia_source AS gs JOIN gaiadr3.vari_eclipsing_binary AS v ON gs.source_id=v.source_id WHERE 1=CONTAINS(POINT('ICRS',gs.ra,gs.dec),CIRCLE('ICRS',{ra},{de},0.25)) AND gs.phot_g_mean_mag<18.5 AND v.frequency>=4.0 AND v.derived_primary_ecl_depth>=0.30 AND v.global_ranking>=0.5 ORDER BY v.derived_primary_ecl_depth DESC"""
 ...
 for r in rows:
  ...
  if sum(bool(x.get('routed')) for x in tested)>=8:break
 good=[x for x in tested if x.get('valid_flux')];o={'success':len(good)>0,'endpoint':endpoint,'query':q,'catalog_candidates_returned':len(rows),'selection':'Gaia DR3 EB; G<18.5; model frequency>=4/d; primary eclipse depth>=0.30 mag; global ranking>=0.5; ordered by published eclipse depth before Euclid measurement','tested':tested,'valid_controls':len(good)}
 if good:o['diagnostic_ranking_by_euclid_excursion']=sorted([{'source_id':x['source_id'],'max_abs_excursion':x['max_abs_excursion'],'peak_to_peak_fraction':x['peak_to_peak_fraction'],'period_hours':x['period_hours'],'primary_depth_mag':x['primary_depth_mag'],'global_ranking':x['global_ranking']} for x in good],key=lambda x:x['max_abs_excursion'],reverse=True)
