...
# HPSC1 has no standalone source-table FITS in the public directory; use the official ECSV.
FILES={'source':BASE+'hetdex_sc1_v3.2.ecsv','det':BASE+'hetdex_sc1_detinfo_v3.2.fits'}
# Published Indahl+ 2021 Table 1 coordinates; all are pre-existing positive controls.
CONTROLS=[
...
('O3ELG13',172.8717,51.2003,0.0650),('O3ELG14',176.4366,51.2577,0.0900),('O3ELG15',212.7970,51.2664,0.0270),
('O3ELG16',168.1299,51.8901,0.0710)]
...
def dl(url,p):
 ...
 req=urllib.request.Request(url,headers={'User-Agent':'ISEF-HETDEX-XMPG-dev/1.1'})
...
def nearest_line(rows,z,rest,tol=8.0):
 want=rest*(1+z);best=None;names=rows.dtype.names or ()
 for r in rows:
  try:w=float(r['wave'])
  except:continue
  d=abs(w-want)
  if d<=tol and (best is None or d<best['delta_A']):
   best={'delta_A':d,'wave':w,'line_id':val(r['line_id']) if 'line_id' in names else None,
         'sn':float(r['sn']) if 'sn' in names and np.isfinite(r['sn']) else None,
         'flux':float(r['flux']) if 'flux' in names and np.isfinite(r['flux']) else None,
         'chi2':float(r['chi2']) if 'chi2' in names and np.isfinite(r['chi2']) else None}
 return best
def main():
 out={'status':'DEVELOPMENT_ONLY','hpsc2_new_opened':False,'success':False,'controls':[],'primary_search_z_frozen':[0.005,0.085]}
 try:
  ps=Path('/tmp/hpsc1_source.ecsv');pd=Path('/tmp/hpsc1_det.fits');dl(FILES['source'],ps);dl(FILES['det'],pd)
  tab=Table.read(ps,format='ascii.ecsv')
  names=list(tab.colnames);ra=np.asarray(tab['RA'],float);dec=np.asarray(tab['DEC'],float);z=np.asarray(tab['z_hetdex'],float);stype=np.asarray(tab['source_type']).astype(str)
  good=np.isfinite(z)&(z>=0.005)&(z<=0.085)
  out['source_columns']=names;out['hpsc1_source_rows']=int(len(tab));out['hpsc1_lowz_005_0085_rows']=int(good.sum())
  ...
  for lab,cra,cdec,cz in CONTROLS:
   ...
   rec={'label':lab,'queryable':True,'nearest_sep_arcsec':sep,'matched':sep<=3.0,'published_z':cz,
        'inside_frozen_primary_z_domain':0.005<=cz<=0.085}
   if sep<=3.0:
  ...
  with fits.open(pd,memmap=True) as h:
   ...
   out['det_columns']=list(d.names);out['hpsc1_detection_rows']=int(len(d));by={r['source_id']:r for r in out['controls'] if r.get('matched')}
   ...
   zdet=np.asarray(d['z_hetdex'],float);wave=np.asarray(d['wave'],float);sn=np.asarray(d['sn'],float) if 'sn' in d.names else np.full(len(d),np.nan)
   ...
   out['anonymous_highsn_5007_detection_rows']=int(strong.sum());out['anonymous_highsn_5007_unique_sources']=int(len(np.unique(ids[strong])))
  out['matched_queryable_controls']=int(sum(1 for r in out['controls'] if r.get('matched')))
  out['matched_controls_inside_primary_z_domain']=int(sum(1 for r in out['controls'] if r.get('matched') and r['inside_frozen_primary_z_domain']))
  out['success']=True
