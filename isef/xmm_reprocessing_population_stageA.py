#!/usr/bin/env python3
"""Frozen Stage A census for the 5XMM reprocessing-recovery project.

Development hemisphere only: 0 <= RA < 180 deg.
No source identities or coordinates are emitted. 5-degree source+observation joins
are recursively subdivided only if HEASARC's 100k result cap is reached.
"""
...
OUT=Path('results/xmm_reprocessing_population_stageA.json'); OUT.parent.mkdir(parents=True,exist_ok=True)
...
def tap(q,timeout=300):
 ...
 req=urllib.request.Request(url,headers={'User-Agent':'ISEF-XMM-population-stageA/1.0'})
...
def old_positions():
 ...
 with fits.open(P4,memmap=True) as h:
  ...
  ra=np.asarray(d[nm['SC_RA']],float).copy(); de=np.asarray(d[nm['SC_DEC']],float).copy(); n=len(d)
 return ra,de,n
...
WHERE {BASE} AND s.ra >= {lo:.8f} AND s.ra < {hi:.8f}'''
 ...
 if len(t)>=CAP:
  ...
  mid=(lo+hi)/2
  a,la=query_interval(lo,mid,depth+1); b,lb=query_interval(mid,hi,depth+1)
  return (a,b),la+lb
...
def score_table(t,c4,oldset):
 ...
 c=SkyCoord(ra*u.deg,de*u.deg); _,sep,_=c.match_to_catalog_sky(c4); unmatched=sep.arcsec>20
 un=[ids[i] for i in np.flatnonzero(unmatched)]
...
def main():
 try:
  ra4,de4,n4=old_positions(); ok=np.isfinite(ra4)&np.isfinite(de4); c4=SkyCoord(ra4[ok]*u.deg,de4[ok]*u.deg)
  oldset,nobs=old_obsids()
  bins=[]; leaves=[]
  for lo in range(0,180,5):
   tree,meta=query_interval(float(lo),float(lo+5)); leaves.extend(meta)
   agg={'eligible':0,'positionally_unmatched':0,'reprocessing_recovered':0,'with_nonblank_obsid':0}
   for t in flatten(tree):
    s=score_table(t,c4,oldset)
    for k in agg: agg[k]+=s[k]
   bins.append({'ra_lo':lo,'ra_hi':lo+5,**agg})
   print(json.dumps({'progress_bin':f'{lo}-{lo+5}',**agg}),flush=True)
  total={k:sum(b[k] for b in bins) for k in ('eligible','positionally_unmatched','reprocessing_recovered','with_nonblank_obsid')}
  prev=total['reprocessing_recovered']/total['eligible'] if total['eligible'] else None
  g1=total['reprocessing_recovered']>=500; g2=prev is not None and prev>=0.001
  out={'success':True,'science_status':'PASS' if g1 and g2 else 'FAIL','hemisphere':'development','ra_range_deg':[0,180],
       'four_unique_source_rows':n4,'official_dr14_obslist_rows':nobs,'official_dr14_unique_obsids':len(oldset),
       'totals':total,'reprocessing_prevalence':prev,
       'frozen_gates':{'g1_reprocessing_recovered_ge_500':g1,'g2_prevalence_ge_0p001':g2},
       'bins':bins,'query_leaves':leaves,'privacy':'Only aggregate/bin counts emitted; no source identities or coordinates.'}
 except Exception as e:
  out={'success':False,'science_status':'INFRASTRUCTURE_FAILURE','error':f'{type(e).__name__}: {e}'}
