...
P4=Path('/tmp/4XMM_DR14cat_slim_v1.0.fits.gz'); P4OBS=Path('/tmp/4xmmdr14_obslist.fits'); OUT=Path('results/xmm_reprocessing_counterpart_bias_validation.json'); OUT.parent.mkdir(parents=True,exist_ok=True)
...
def tap(q,timeout=300):
 url=EP+'?'+urllib.parse.urlencode({'REQUEST':'doQuery','LANG':'ADQL','FORMAT':'votable','QUERY':q}); req=urllib.request.Request(url,headers={'User-Agent':'ISEF-XMM-counterpart-bias-validation/1.0'})
 with urllib.request.urlopen(req,timeout=timeout) as r: raw=r.read()
 return Table.read(io.BytesIO(raw),format='votable')
def dl(url,p):
 req=urllib.request.Request(url,headers={'User-Agent':'ISEF-XMM-counterpart-bias-validation/1.0'})
...
def qbin(lo,hi,depth=0):
 q=f'''SELECT TOP {CAP} s.srcid AS sid,s.ra AS sra,s.dec AS sdec,s.gaiadr3_source_id AS gid,s.gaia_match_prob AS gp,s.wise_name AS wname,s.wise_match_prob AS wp,s.classopt_class AS oclass,d.obsid AS dobsid,d.pps_srcnum AS pps FROM xmmssc AS s JOIN xmmstack AS d ON s.srcid=d.srcid WHERE {BASE} AND s.ra >= {lo:.8f} AND s.ra < {hi:.8f} AND d.pps_srcnum IS NOT NULL'''; t=tap(q)
 if len(t)>=CAP:
...
def main():
 try:
  ...
  for lo in range(180,360,5):
   tabs=qbin(float(lo),float(lo+5)); t=vstack(tabs,metadata_conflicts='silent') if len(tabs)>1 else tabs[0]; props={}; obs=defaultdict(set)
   for row in t:
    sid=norm(row['sid'])
    if sid not in props: props[sid]={'ra':num(row['sra']),'dec':num(row['sdec']),'gaia':validstr(row['gid']) and num(row['gp'])>=0.8,'wise':validstr(row['wname']) and num(row['wp'])>=0.8,'optclass':validstr(row['oclass'])}
    o=norm(row['dobsid']);
  ...
  outcomes={'gaia':lambda p:p['gaia'],'wise':lambda p:p['wise'],'any':lambda p:p['gaia'] or p['wise'],'optclass':lambda p:p['optclass']}; res={}; confirmed={}
  for name,fn in outcomes.items():
   ...
   odds,p=fisher_exact([[ca,cb],[xa,xb]],alternative='two-sided'); cf=ca/len(cases); xf=xa/len(ctrls)
   res[name]={'case_yes':ca,'case_no':cb,'control_yes':xa,'control_no':xb,'case_fraction':cf,'control_fraction':xf,'odds_ratio':float(odds),'raw_p':float(p)}
   confirmed[name]=bool(cf<xf and p<=0.01)
  out={'success':True,'hemisphere':'validation','matched_cases':len(cases),'unique_controls':len(ctrls),'excluded_cases':excluded,'probability_threshold':0.8,'outcomes':res,'validation_confirmed':confirmed,'privacy':'Aggregate counts only; no identities or coordinates emitted.'}
