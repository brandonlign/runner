...
OUT='/tmp/run_p13_matched_launcher_v4_inner.sh'
test -f "$SRC"
...
new_marker="""test \"${launcher_files[0]}\" = 'orbittrace_core_halo_p13_literature/LAUNCH_V4.md'\nlaunch_marker=\"$(git show \"$HEAD_SHA\":orbittrace_core_halo_p13_literature/LAUNCH_V4.md)\"\ntest \"$(printf '%s\\n' \"$launch_marker\" | sed -n '1p')\" = 'LAUNCH_P13_MATCHED_TECHNICAL_RECOVERY_V4'\ntest \"$(printf '%s\\n' \"$launch_marker\" | sed -n '2p')\" = '674'\ntest \"$(printf '%s\\n' \"$launch_marker\" | sed -n '3p')\" = '31322983205'\n"""
if src.count(old_marker)!=1:
    raise SystemExit(f'v4 marker anchor count={src.count(old_marker)}')
