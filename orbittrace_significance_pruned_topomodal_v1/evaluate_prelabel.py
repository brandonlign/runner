...
YEARS=(2022,2023); MONTH_KEYS=tuple(f"{y}-{m:02d}" for y in YEARS for m in range(1,13)); BLIND=(20.0,55.0); BUCKETS=(0,1,2,3); B=199; ALPHA=0.05
...
def main()->int:
    ...
    intrinsic=load_module(a.intrinsic_runner,"sig_eval_intrinsic"); req(sha256(a.quality_source)==QUALITY_SHA256 and sha256(a.v8_result_json)==V8_RESULT_SHA256,"frozen input")
    pre_sha=sha256(a.prelabel); pre=json.loads(a.prelabel.read_text()); req(pre["schema"]=="ORBITTRACE_SIGNIFICANCE_PRUNED_TOPOMODAL_V1_PRELABEL" and pre["scientific_role"]=="PRELABEL_SIGNIFICANCE_PRUNED_TOPOMODAL_V1","prelabel schema"); req(pre["structural_result_sha256"]==STRUCTURAL_RESULT_SHA256,"structural pin")
    cfg=pre["configuration"]; req(cfg["density"]=="GEO6_third_nearest_other_empirical_rank_q" and cfg["graph"]=="exact_1284_physical_radius_1" and cfg["hierarchy"]=="gudhi_3.12_manual_topomato_significance_simplified_flat_partition","method changed"); req(cfg["null"]=="permute_q_over_fixed_graph" and int(cfg["B"])==B and float(cfg["alpha"])==ALPHA and cfg["tau_rule"]=="10th_largest_of_199_null_maxima_strict_survival" and int(cfg["min_candidate_support"])==4,"null changed")
    req(pre["blind_exclusion"]==list(BLIND) and pre["shower_truth_used"] is False and pre["target_information_access"] is False and pre["target_region_events_accessed"] is False,"firewall")
    sg={str(k):bool(v) for k,v in pre["cross_scale"]["gates"].items()}; req(set(sg)=={"pooled_fine_to_coarse_mean_best_jaccard_strictly_greater","strict_bucket_wins_at_least_3_of_4"},"structural gates")
    ...
    for r in pre["subsets"]:
        req(len(r["null_max_prominences"])==B,"null count"); desc=sorted((float(x) for x in r["null_max_prominences"]),reverse=True); req(abs(float(r["successor_summary"]["tau"])-desc[9])<=1e-15,"tau recompute"); succ=r["successor_candidates"]; req([int(x["rank"]) for x in succ]==list(range(1,len(succ)+1)),"rank continuity")
        for x in succ:
            if not bool(x["is_root_survivor"]): req(float(x["finite_prominence"])>float(r["successor_summary"]["tau"]) and float(x["p_fwer"])<=ALPHA,"finite significance")

    parent=load_module(a.parent_runner,"sig_eval_parent"); req(tuple(parent.YEARS)==YEARS and tuple(parent.BLIND)==BLIND and int(parent.MIN_CLUSTER_SIZE)==10 and int(parent.MIN_SAMPLES)==10,"parent")
    q=load_module(a.quality_source,"sig_eval_gmn"); q.v1.mult.YEARS=YEARS; q.v1.mult.MONTH_KEYS=MONTH_KEYS; q.v1.mult.TOP_K=100; runtime=q.v1.mult.load_frozen_runtime(); support=runtime.load_support_module(a.support_source_parts); support.YEARS=YEARS; support.MONTH_KEYS=MONTH_KEYS; support.CORPUS="orbittrace-significance-pruned-topomodal-v1-evaluator"; support.RANKING_VARIANTS=("persistence",); req((float(support.BLIND_LOW),float(support.BLIND_HIGH))==BLIND,"firewall changed"); setattr(a,"fixed4_baseline_json",a.v8_result_json); _c,base,_s=support.load_sources(a); scan,_cal,hidden,sources=support.parse_catalogue(base); req(isinstance(hidden,dict) and sorted(scan)==list(YEARS) and [x["key"] for x in sources]==list(MONTH_KEYS),"truth source")
    events=[]
    ...
    for d in (128,1024):
        for b in BUCKETS:
            fr=frozen[(d,b)]; ii=intrinsic.selected_indices(hashes,d,b); sid=[ids[int(i)] for i in ii]; sy=np.asarray(yrs[ii]); req(len(sid)==int(fr["events_total"]) and hashlib.sha256("\n".join(sorted(sid)).encode()).hexdigest()==str(fr["event_universe_sha256"]),"event universe"); K=int(fr["equal_budget_k"]); par=fr["recurrent_candidates"]; succ_all=fr["successor_candidates"]; succ=succ_all[:min(K,len(succ_all))]; req(len(par)==K,"parent budget")
            for y in YEARS:
                annual={sid[int(i)] for i in np.flatnonzero(sy==y)}; pm=compact(parent.metrics(par,hidden,annual)); sm=compact(parent.metrics(succ,hidden,annual)); truth.append({"denominator":d,"bucket":b,"year":y,"equal_budget_k":K,"successor_candidates_available":len(succ_all),"successor_candidates_evaluated":len(succ),"parent":pm,"successor":sm,"qualified_nonlower":int(sm["qualified_matches"])>=int(pm["qualified_matches"]),"qualified_strict_win":int(sm["qualified_matches"])>int(pm["qualified_matches"])})
    scales={}
    for d in (128,1024):
    ...
    gates={"candidate_budget_sufficient_all_panels":not bool(pre["candidate_budget_shortage_any_panel"]),**{"structural_"+k:v for k,v in sg.items()},**tg}; verdict="PASS_SIGNIFICANCE_PRUNED_TOPOMODAL_V1" if all(gates.values()) else "FAIL_SIGNIFICANCE_PRUNED_TOPOMODAL_V1"
    out={"schema":"ORBITTRACE_SIGNIFICANCE_PRUNED_TOPOMODAL_V1","scientific_role":"TARGET_EXCLUDED_GMN_2022_2023_SPARSE_RECOVERY_AND_GENERALIZATION_DEVELOPMENT","verdict":verdict,"prelabel_sha256":pre_sha,"structural_source_run_id":31955621864,"structural_source_artifact_id":9265889512,"structural_result_sha256":STRUCTURAL_RESULT_SHA256,"configuration":cfg,"candidate_budget_shortage_any_panel":pre["candidate_budget_shortage_any_panel"],"cross_scale":pre["cross_scale"],"panels":truth,"scale_aggregates":scales,"gates":gates,"blind_exclusion":list(BLIND),"target_information_access":False,"target_region_events_accessed":False,"sonotaco_2013_2014_access":False,"asfn_event_level_access":False,"efn_event_level_access":False,"amos_scientific_access":False,"maarsy_scientific_access":False,"dms_scientific_access":False,"method_parameter_selection_from_result":False}
    p=a.output/"SIGNIFICANCE_PRUNED_TOPOMODAL_V1.json"; p.write_text(json.dumps(out,indent=2,sort_keys=True,allow_nan=False)+"\n"); print(json.dumps({"verdict":verdict,"prelabel_sha256":pre_sha,"candidate_budget_shortage":pre["candidate_budget_shortage_any_panel"],"cross_scale":pre["cross_scale"],"scales":scales,"gates":gates},indent=2,sort_keys=True),flush=True); return 0
