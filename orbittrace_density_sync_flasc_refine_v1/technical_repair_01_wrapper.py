...
class _ExactParentHDBSCAN(_OriginalHDBSCAN):
    """Suppress only the convenience cache flag during the prerequisite parent fit."""

    def __init__(self, *args, branch_detection_data=False, **kwargs):
        self._repair_branch_detection_requested = bool(branch_detection_data)
        super().__init__(*args, branch_detection_data=False, **kwargs)


def _repair_detect_branches(clusterer, cluster_labels=None, cluster_probabilities=None, **kwargs):
    if not isinstance(clusterer, _ExactParentHDBSCAN):
        raise RuntimeError("technical repair wrapper received unexpected clusterer class")
    if not clusterer._repair_branch_detection_requested:
