...
def adapt_bif_candidates(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Engineering-only interface adapter; preserves order and every scientific field."""
    out: list[dict[str, Any]] = []
    for row in rows:
        req("family_hash" in row and "event_ids" in row, "bif candidate missing frozen identity fields")
        req("family_id" not in row, "unexpected family_id already present in frozen bif candidate")
        adapted = dict(row)
        adapted["family_id"] = "BIF1_" + str(row["family_hash"])
        req(adapted["event_ids"] == row["event_ids"], "adapter changed event membership")
        out.append(adapted)
    req([r["family_hash"] for r in out] == [r["family_hash"] for r in rows], "adapter changed candidate order")
    return out
...
def main() -> int:
    ...
    req(sha256(a.prelabel) == PRELABEL_SHA, "immutable original prelabel changed")
    ...
    sparse = load_module(a.sparse_source, "bifrank_repair_sparse")
    parent = load_module(a.parent_runner, "bifrank_repair_parent")
    req(tuple(parent.YEARS) == YEARS and tuple(parent.BLIND) == BLIND, "parent constants changed")

    qmod = load_module(a.quality_source, "bifrank_repair_gmn_utility")
    ...
    support.CORPUS = "orbittrace-annual-density-bifiltration-gmn-ranking-v1-target-excluded-evaluation-repair1"
    ...
    for denominator in DENOMINATORS:
        for bucket in BUCKETS:
            ...
            frozen_bif = list(s["bifiltration_candidates"])
            bif_all = adapt_bif_candidates(frozen_bif)
    ...
    result = {
        ...
        "verdict": verdict,
        "repair_role": "ENGINEERING_ONLY_FAMILY_ID_SCHEMA_ADAPTER",
        "original_prelabel_sha256": PRELABEL_SHA,
    ...
    result_sha = dump(a.output / "BIFILTRATION_GMN_RANKING_V1_REPAIR1_RESULT.json", result)
    print(json.dumps({"verdict": verdict, "original_prelabel_sha256": PRELABEL_SHA, "result_sha256": result_sha, "scale_aggregates": scale_results, "gates": gates}, indent=2, sort_keys=True))
    return 0
