...
def combined_best_label(family: dict[str,Any],hidden: dict[str,str],eligible: dict[str,Counter[int]])->dict[str,Any]:
    # Exact v22 best-label semantics. Only the downstream scalar regression target changes in v23.
    ...
    for label,per_year in eligible.items():
        ...
        if ov<=0: continue
        total=int(sum(per_year.values())); precision=ov/max(len(ids),1); recall=ov/total
        f1=2*precision*recall/(precision+recall) if precision+recall else 0.0
        rows.append((f1,precision,ov,label,recall))
    if not rows:
        return {'best_label':None,'combined_f1':0.0,'combined_precision':0.0,'combined_overlap':0}
    f1,precision,ov,label,_recall=max(rows,key=lambda r:(r[0],r[1],r[2],r[3]))
    return {'best_label':str(label),'combined_f1':float(f1),'combined_precision':float(precision),'combined_overlap':int(ov)}


def year_f1_for_label(family: dict[str,Any],truth: dict[str,str],label: str)->float:
    truth_ids=set(truth)
    pred={eid for eid in map(str,family['event_ids']) if eid in truth_ids}
    actual={eid for eid,v in truth.items() if v==label}
    if not pred or not actual: return 0.0
    ov=len(pred&actual)
    if ov<=0: return 0.0
    precision=ov/len(pred); recall=ov/len(actual)
...
def main()->int:
    ...
    ranker=load_module(a.ranker_source,'frozen_839_v23_train')
    route_data={}; Xs=[]; targets=[]; groups=[]; route_offsets={}; cursor=0; target_diag={}
    for route in ROUTES:
        ...
        require(array_sha(X)==meta['feature_sha256'] and array_sha(C)==meta['centroid_sha256'],'pretruth array internal hash changed')

        by_year={y:truth_year[(route,y)] for y in YEARS}; eligible=eligible_from_year_truth(by_year)
        hidden={}; hidden.update(by_year[2013]); hidden.update(by_year[2014]); require(len(hidden)==len(by_year[2013])+len(by_year[2014]),f'{route} duplicate IDs across years')
        best=[combined_best_label(f,hidden,eligible) for f in fams]
        year_scores=[]; y=[]; gs=[]
        for i,(f,b) in enumerate(zip(fams,best)):
            label=b['best_label']
            if label is None:
                f13=f14=target=0.0; group=f'NEG/{route}/{ids[i]}'
            else:
                f13=year_f1_for_label(f,by_year[2013],label); f14=year_f1_for_label(f,by_year[2014],label)
                target=float(min(f13,f14)); group='SHOWER/'+str(label)
            y.append(target); gs.append(group); year_scores.append((f13,f14))
        yarr=np.asarray(y,dtype=np.float64)
        require(np.all(np.isfinite(yarr)) and np.all((yarr>=0.0)&(yarr<=1.0)),'invalid worst-year target')
        route_offsets[route]=(cursor,cursor+len(ids)); cursor+=len(ids); Xs.append(X); targets.append(yarr); groups.extend(gs)
        route_data[route]={'meta':meta,'families':fams,'ids':ids,'centroids':C,'best':best,'year_scores':year_scores,'eligible':eligible}
        target_diag[route]={
            ...
            'nonzero_worst_year_targets':int(np.sum(yarr>0)),
            'target_mean':float(np.mean(yarr)),'target_median':float(np.median(yarr)),'target_max':float(np.max(yarr)),
            'target_definition':'min(F1_2013,F1_2014) for exact v22 combined best_label',
        }

    Xall=np.vstack(Xs); yall=np.concatenate(targets); groups=list(map(str,groups))
    ...
    for fold in range(5):
        ...
        fold_diag.append({'fold':fold,'train_examples':int(tr.sum()),'test_examples':int(te.sum()),'train_groups':len(train_groups),'test_groups':len(test_groups),'test_nonzero_targets':int(np.sum(yall[te]>0))})
    ...
    for route in ROUTES:
        ...
        v19order=list(map(str,rd['meta']['v19_order'])); fused=list(v19.fusion_orders(qorder,v19order)['rank_sum'])
        variants[route]={'worst_year_oof_quality':rerank(rd['families'],qorder),'worst_year_oof_v19_rank_sum':rerank(rd['families'],fused),'v19_control':rerank(rd['families'],v19order)}
        order_diag[route]={'oof_prediction_sha256':array_sha(scores),'quality_order_sha256':hashlib.sha256('\n'.join(qorder).encode()).hexdigest(),'fused_order_sha256':hashlib.sha256('\n'.join(fused).encode()).hexdigest()}
        for year in YEARS:
    ...
    full_freeze={'verdict':'NOT_FROZEN_V23_OOF_FAIL','model_sha256':None}
    if passed:
        full=ranker.model(); full.fit(Xall,yall,sample_weight=weights); full.set_params(n_jobs=1)
        model_path=a.output/'v23_sonotaco_full_ranker.joblib'; joblib.dump(full,model_path)
        full_freeze={'verdict':'PASS_V23_FULL_SONOTACO_MODEL_FREEZE','model_sha256':sha(model_path),'feature_dimension':FEATURE_DIM,'training_examples':len(yall),'training_groups':len(set(groups)),'training_target_sha256':array_sha(yall),'training_feature_sha256':array_sha(Xall),'in_sample_full_fit_score_used_for_promotion':False}
