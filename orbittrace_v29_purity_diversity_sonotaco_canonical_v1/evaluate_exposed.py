...
def main()->int:
    ...
    summary=json.loads(a.summary.read_text()); require(summary['verdict']=='PASS_V29_CANONICAL_SONOTACO_PRETRUTH_CATALOGUE_FREEZE','pretruth catalogue not frozen'); require(summary['truth_accessed'] is False and summary['matched_comparator_rows_accessed'] is False,'pretruth boundary changed')
    catalogue=json.loads(a.catalogue.read_text()); require(sha(a.catalogue)==summary['primary_output_sha256'],'pretruth catalogue hash changed'); require(catalogue['input_role'].startswith('single canonical SonotaCo base pair'),'detector input role changed'); require(catalogue['truth_accessed'] is False and catalogue['matched_comparator_rows_accessed'] is False,'pretruth catalogue became truth-bearing')
    ...
    for route,year in PANELS:
        truth=json.loads((a.truth_root/f'truth_{route}_{year}.json').read_text()); frozen=json.loads((a.truth_root/f'evaluation_{route}_{year}.json').read_text())
    ...
    result={'scientific_stage':'V29_CANONICAL_SONOTACO_EXPOSED_LITERATURE_DEVELOPMENT','single_canonical_detector_catalogue':True,'panel_specific_candidate_generation':False,'panel_specific_ranking':False,'candidate_catalogue_sha256':sha(a.catalogue),'candidate_order_sha256':catalogue['v29_order_sha256'],'candidate_counts':catalogue['candidate_counts'],'membership_diagnostics':catalogue['membership_diagnostics'],'panels':panels,'panel_wins':wins,'verdict':'PASS_V29_CANONICAL_SONOTACO_ALL_PANEL_LITERATURE_SUPERIORITY_DEVELOPMENT' if passed else 'FAIL_V29_CANONICAL_SONOTACO_ALL_PANEL_LITERATURE_SUPERIORITY_DEVELOPMENT','sonotaco_role':'EXPOSED_DEVELOPMENT_ONLY','model_retrained_on_sonotaco':False,'matched_comparator_rows_used_as_detector_input':False,'parameter_search':False,'post_result_second_search':False,'maarsy_scientific_access':False,'dms_scientific_access':False,'target_information_access':False}
    (a.output/'V29_CANONICAL_SONOTACO_EXPOSED_RESULT.json').write_text(json.dumps(result,indent=2,sort_keys=True,allow_nan=False)+'\n')
