...
OUT_DIR = ROOT / "results" / "shared_flux_confirmation"
...
PRIMARY = "shared_flux"
ELIGIBLE_BASELINES = tuple(method for method in METHODS if method != PRIMARY)
RADII = (0.70, 0.90, 1.10, 1.30)
CALIBRATION_NULL_SCENES = 96
TEST_NULL_SCENES = 96
INJECTION_REPLICATES = 128
...
MODERATE_PATTERN = {"CAMS": 4, "GMN": 6, "EDMOND": 3, "SonotaCo": 3}
DOMINANT_PATTERN = {"CAMS": 2, "GMN": 8, "EDMOND": 2, "SonotaCo": 2}
THREE_NETWORK_PATTERN = {"CAMS": 4, "GMN": 5, "EDMOND": 0, "SonotaCo": 4}
...
def local_expected_counts(
    inner: np.ndarray,
    outer: np.ndarray,
    radius: float,
) -> np.ndarray:
    shell = np.maximum(outer - inner, 0).astype(np.float64)
    outer_radius = v1.OUTER_MULTIPLIER * radius
    volume_ratio = radius**3 / (outer_radius**3 - radius**3)
    return (shell + 0.5) * volume_ratio


def shared_flux_profile_llr(
    inner_matrix: np.ndarray,
    expected_matrix: np.ndarray,
) -> np.ndarray:
    counts = inner_matrix.astype(np.float64)
    background = np.maximum(expected_matrix.astype(np.float64), 1e-12)
    network_count, candidate_count = counts.shape
    weights = np.ones((network_count, 1), dtype=np.float64)

    support = np.sum(
        (counts >= v1.MIN_INNER_COUNT) & (counts > background),
        axis=0,
    ) >= 2

    derivative_zero = np.sum(weights * (counts / background - 1.0), axis=0)
    active = derivative_zero > 0.0
    lower = np.zeros(candidate_count, dtype=np.float64)
    upper = np.maximum(
        np.sum(np.maximum(counts - background, 0.0), axis=0) / np.sum(weights),
        1.0,
    )

    def derivative(rate: np.ndarray) -> np.ndarray:
        return np.sum(
            weights * (counts / (background + weights * rate.reshape(1, -1)) - 1.0),
            axis=0,
        )

    for _ in range(12):
        needs_growth = active & (derivative(upper) > 0.0)
    ...
    for _ in range(36):
        ...
        positive = active & (derivative(midpoint) > 0.0)
        lower[positive] = midpoint[positive]
        upper[active & ~positive] = midpoint[active & ~positive]

    rate = (lower + upper) / 2.0
    alternative = background + weights * rate.reshape(1, -1)
    llr = np.sum(
        counts * np.log(alternative / background) - weights * rate.reshape(1, -1),
        axis=0,
    )
    llr[~active] = 0.0
    llr[~support] = 0.0
...
def scan_at_radius(
    ...
    for name in network_names:
        ...
        inner_rows.append(np.asarray(inner, dtype=np.float64))
        expected_rows.append(local_expected_counts(inner, outer, radius))
        excess_rows.append(v1.poisson_excess_llr(inner, outer, radius))

    inner_matrix = np.vstack(inner_rows)
    expected_matrix = np.vstack(expected_rows)
    excess_matrix = np.vstack(excess_rows)

    shared_flux = shared_flux_profile_llr(inner_matrix, expected_matrix)
    positive_support = np.sum(excess_matrix > 0.0, axis=0)
    total = np.sum(excess_matrix, axis=0)
    largest = np.max(excess_matrix, axis=0)
    sorted_scores = np.sort(excess_matrix, axis=0)
...
def paired_bootstrap_interval(primary: np.ndarray, baseline: np.ndarray, seed: int) -> tuple[float, float]:
...
def evaluate_condition(
    ...
    for method in METHODS:
        ...
        summary[method] = {
            ...
            "recovery_wilson_95": list(wilson_interval(recovery_count, INJECTION_REPLICATES)),
...
def summarize_null(test_scores: dict[str, list[float]], thresholds: dict[str, float], total: int) -> dict[str, object]:
    result = {}
    for method in METHODS:
        count = int(np.sum(np.asarray(test_scores[method]) > thresholds[method]))
        result[method] = {
            "false_positive_count": count,
            "false_positive_rate": count / total,
            "wilson_95": list(wilson_interval(count, total)),
        }
    return result


def evaluate_m2026(data: dict[str, v1.NetworkData], thresholds: dict[str, float], seed: int) -> dict[str, object]:
    rng = np.random.default_rng(seed)
...
def combined_gain(
    ...
    primary = np.concatenate([
        np.asarray(conditions[name][PRIMARY]["located"], dtype=int)
        for name in names
    ])
    baselines = {
        method: np.concatenate([
            np.asarray(conditions[name][method]["located"], dtype=int)
            for name in names
        ])
        for method in ELIGIBLE_BASELINES
    }
    best_method = max(baselines, key=lambda method: float(np.mean(baselines[method])))
...
def run() -> dict[str, object]:
    ...
    calibration = make_null_scores(data, CALIBRATION_NULL_SCENES, seed=301001)
    thresholds = {method: empirical_threshold(calibration[method]) for method in METHODS}
    test_null = make_null_scores(data, TEST_NULL_SCENES, seed=302001)
    null_summary = summarize_null(test_null, thresholds, TEST_NULL_SCENES)

    conditions: dict[str, dict[str, object]] = {}
    grouped_names: list[str] = []
    condition_index = 0
    for dispersion_name, scatter in DISPERSIONS.items():
        for family_name, pattern in (("balanced", BALANCED_PATTERN), ("moderate", MODERATE_PATTERN)):
            name = f"{family_name}_{dispersion_name}"
            ...
            conditions[name] = evaluate_condition(
                ...
                pattern,
                scatter,
                seed=303001 + condition_index * 1000,
            )
            condition_index += 1

    conditions["dominant_nominal"] = evaluate_condition(
        data,
        thresholds,
        DOMINANT_PATTERN,
        DISPERSIONS["nominal"],
    ...
    conditions["three_network_nominal"] = evaluate_condition(
        data,
        thresholds,
        THREE_NETWORK_PATTERN,
        DISPERSIONS["nominal"],
        seed=311001,
    )
    ...
    dropout_null_summary = summarize_null(dropout_test, dropout_thresholds, TEST_NULL_SCENES)
    ...
    group = combined_gain(conditions, grouped_names, seed=317001)
    external = evaluate_m2026(data, thresholds, seed=318001)
    ...
    dominant_primary = conditions["dominant_nominal"][PRIMARY]["recovery_rate"]
    dominant_pooled = conditions["dominant_nominal"]["pooled"]["recovery_rate"]
    three_network = conditions["three_network_nominal"][PRIMARY]["recovery_rate"]
    artifact = conditions["gmn_only_artifact"][PRIMARY]["recovery_rate"]
    strong = conditions["strong_shared"][PRIMARY]["recovery_rate"]
    ...
    gates = {
        ...
        "combined_gain_ge_0_10": group["gain"] >= 0.10,
        "combined_bootstrap_lower_gt_0": group["paired_bootstrap_95"][0] > 0.0,
        "individual_not_inferior_by_0_10": individual_not_inferior,
        "dominant_gain_over_pooled_ge_0_05": dominant_primary - dominant_pooled >= 0.05,
        "three_network_recovery_ge_0_50": three_network >= 0.50,
        "gmn_only_artifact_recovery_le_0_10": artifact <= 0.10,
        "strong_shared_recovery_ge_0_90": strong >= 0.90,
    ...
    decision = {
        ...
        "combined_weak_conditions": group,
        "individual_comparisons": individual_comparisons,
        "dominant_primary_recovery": dominant_primary,
        "dominant_pooled_recovery": dominant_pooled,
        "three_network_recovery": three_network,
        "artifact_recovery": artifact,
        "strong_shared_recovery": strong,
        "dropout_primary_recovery": dropout_primary,
        "dropout_pooled_recovery": dropout_pooled,
        "gates": gates,
        "verdict": "CONTINUE_TO_KNOWN_STREAM_BENCHMARK"
        if all(gates.values())
        else "KILL_OR_REDESIGN_SHARED_FLUX_METHOD",
    }
    ...
    return {
        "configuration": {
            ...
            "patterns": {
                ...
                "three_network": THREE_NETWORK_PATTERN,
...
def markdown_report(payload: dict[str, object]) -> str:
    ...
    group = decision["combined_weak_conditions"]
    lines = [
        "# Shared latent-flux confirmation",
        "",
        f"Verdict: **{decision['verdict']}**",
        "",
        "The joint Poisson profile likelihood uses one shared nonnegative stream rate and separate network backgrounds. Candidate and scale searches are null-calibrated. GhostStream was excluded.",
        "",
        "## Primary summary",
        "",
        f"- independent-null FPR: {payload['null_summary'][PRIMARY]['false_positive_rate']:.3f}",
        f"- null Wilson 95%: {payload['null_summary'][PRIMARY]['wilson_95']}",
        f"- combined weak recovery: {group['primary_mean_recovery']:.3f}",
        f"- best baseline: {group['best_baseline']} at {group['best_baseline_mean_recovery']:.3f}",
        f"- gain: {group['gain']:.3f}; paired-bootstrap 95% {group['paired_bootstrap_95']}",
        f"- dominant shared recovery: {decision['dominant_primary_recovery']:.3f} versus pooled {decision['dominant_pooled_recovery']:.3f}",
        f"- three-network recovery: {decision['three_network_recovery']:.3f}",
