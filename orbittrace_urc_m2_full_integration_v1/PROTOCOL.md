...
This protocol is frozen before the scientific result of PR #846 is known.

It defines the **only** allowed full-catalogue integration of the M2 event-level P12 membership challenger admitted by the final GMN method-selection freeze in PR #848. It does not authorize any new GMN architecture search.

## Preconditions

The integration is scientifically authorized only if all of the following are true:

1. PR #846 returns `PASS_EVENT_LEVEL_P12_MEMBERSHIP_CALIBRATION_FEASIBILITY` under its already-frozen selector and at least three grid variants pass its feasibility gates.
2. The exact single model / probability threshold / additions-per-core cap selected by #846 then passes the already-frozen five-salt fixed-policy whole-shower stress in PR #850, with all five panels passing.
...
For GMN 2022/2023 scientific evaluation, M2 uses the exact **out-of-fold predictions** from #846's selected policy, reconstructed from the frozen #846 source and folds.
...
- the model family, threshold, and cap are exactly those selected by #846;
...
- the exact #846 selected hard-family historical macro F1;
- the exact #846 selected corrected qualified count;
- the exact #846 selected corrected recovery@100;
- the exact #846 selected corrected top-100 dominant precision;
...
1. use the exact model class/hyperparameters selected by #846;
2. reconstruct the exact 17,238 GMN training rows/features from the frozen #846 pipeline;
