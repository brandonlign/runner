...
GEN='orbittrace_support_safe_rank_p14/generate_matched_pretruth_direct_v2.py'
OUT='/tmp/p14_matched_pretruth_direct.sh'

test -f "$V3" -a -f "$GEN"
test "$(git hash-object "$V3")" = '1fb46484a51bb7d7edd60c865dcf5341550277a1'
python "$GEN" "$V3" "$OUT"
python -m py_compile "$GEN" orbittrace_support_safe_rank_p14/generate_matched_pretruth_direct.py orbittrace_core_halo_p13_literature/prepare_pretruth_panel_input_p14.py orbittrace_core_halo_p13_literature/finalize_pretruth_checkpoint_p14.py
bash -n "$OUT"
